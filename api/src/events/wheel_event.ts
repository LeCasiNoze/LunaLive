// api/src/events/wheel_event.ts
//
// Semaine de la roue — REDESIGN v2 (cf docs/events-design.md #2). Remplace
// l'ancien wheel_week.ts (scoring recompute-from-wallet_tx) : CHANGEMENT DE
// PARADIGME, le score s'ACCUMULE à chaque spin (event_scores.points +=),
// il n'est plus jamais recomputé depuis l'activité (cf engine.ts — plus
// d'appel recomputeWheelWeek). Systèmes GÉNÉRIQUES (paliers + boutique +
// inventaire de tickets + quêtes d'event) pensés pour être réutilisés par
...
/* ────────────────────────────────────────────────────────────────────────
   Roue — 11 segments physiques. Les 3 tranches à 300 pts représentent le
   "near-miss" collé au jackpot (cf docs/events-design.md — 3 slices
   visuelles pour 8% cumulés = 267+267+266). weightBp sur 10000 (même
   convention que economy.ts).
   ──────────────────────────────────────────────────────────────────────── */

export type WheelLot =
  | { type: "none" }
  | { type: "rubis"; amount: number }
  | { type: "xp"; amount: number }
  | { type: "ticket"; amount: number }
  | { type: "cosmetic"; code: string }
  | { type: "sub_ticket" }
  | { type: "jackpot"; rubis: number; cosmeticCode: string };

export type WheelSegment = {
  index: number;
  weightBp: number; // /10000
  points: number;
  lot: WheelLot;
  lotLabel: string;
};
...
/* ────────────────────────────────────────────────────────────────────────
   Paliers — points cumulés (event_scores.points), réclamés EN DIRECT.
   ──────────────────────────────────────────────────────────────────────── */

export type PalierReward = {
  rubis?: number;
  tickets?: number;
  xp?: number;
  subTicket?: boolean;
  cosmeticCode?: string;
  xpBoost?: boolean; // active le multiplicateur (lu via getXpBoostMultiplierBp)
};
...
/* ────────────────────────────────────────────────────────────────────────
   Boutique — le PUITS de rubis (P0 de l'éco, cf docs/events-design.md).
   ──────────────────────────────────────────────────────────────────────── */

export type ShopItemCode = "ticket" | "reroll" | "xp_boost" | "sub_ticket";
...
export type ShopItem = {
  ...
  capMode: "day" | "event";
  capPerPeriod: number;
  priceFor: (boughtInPeriod: number) => number; // prix du PROCHAIN achat
};
...
/* ────────────────────────────────────────────────────────────────────────
   Quêtes — donnent des tickets (user_event_tickets, cap 50). ⚠️
   event_quest_claims n'a pas de colonne "day" : les quêtes DAILY sont
   stockées sous une clé composite "<key>:<YYYY-MM-DD Paris>" pour rester
   réclamables chaque jour de l'event ; les WEEKLY utilisent la clé nue (une
   fois par event). Cf questStorageKey.
   ──────────────────────────────────────────────────────────────────────── */
...
export const QUESTS: { daily: Quest[]; weekly: Quest[] } = {
  daily: [
...
function questStorageKey(quest: Quest, todayDate: string): string {
...
export async function getLiveWheelEvent(db: Db) {
...
async function addEventTickets(client: PoolClient, userId: number, amount: number) {
  if (amount <= 0) return;
  await client.query(
    `
...
async function grantSubTicket(client: PoolClient, userId: number) {
  await client.query(
    `
...
async function getXpBoostMultiplierBp(db: Db, userId: number): Promise<number> {
  ...
  const last = r.rows?.[0]?.last_grant as Date | string | null;
...
async function applySegmentLot(
  client: PoolClient,
  eventId: number,
  userId: number,
  lot: WheelLot,
  xpMultiplierBp: number
) {
  switch (lot.type) {
    case "none":
      return;
...
async function applyPalierReward(client: PoolClient, eventId: number, userId: number, reward: PalierReward) {
  if (reward.rubis) {
...
async function claimPaliersTx(client: PoolClient, eventId: number, userId: number): Promise<Palier[]> {
  ...
  const claimedSet = new Set((already.rows || []).map((r: any) => Number(r.palier_index)));

  const newlyClaimed: Palier[] = [];
  for (const palier of PALIERS) {
    if (points < palier.threshold) continue;
    if (claimedSet.has(palier.index)) continue;

    const ins = await client.query(
      `INSERT INTO event_palier_claims (event_id, user_id, palier_index) VALUES ($1,$2,$3) ON CONFLICT DO NOTHING RETURNING palier_index`,
      [eventId, userId, palier.index]
    );
...
export async function claimPaliers(eventId: number, userId: number): Promise<Palier[]> {
...
async function performSpin(
  ...
  await client.query(
    `INSERT INTO event_wheel_spins (event_id, user_id, segment_index, points) VALUES ($1,$2,$3,$4)`,
...
export type SpinResult =
  | { ok: false; error: "no_event" | "no_ticket" }
  | {
      ok: true;
...
export async function spinWheel(userId: number): Promise<SpinResult> {
  ...
  try {
    ...
    const freeClaim = await client.query(
      `INSERT INTO event_quest_claims (event_id, user_id, quest_key) VALUES ($1,$2,$3)
       ON CONFLICT DO NOTHING RETURNING 1`,
      [Number(event.id), userId, `__free_spin:${todayDate}`]
    );
    const isFree = (freeClaim.rowCount ?? 0) > 0;

    if (!isFree) {
      const consumed = await client.query(
        `UPDATE user_event_tickets SET amount = amount - 1 WHERE user_id=$1 AND amount >= 1 RETURNING amount`,
        [userId]
      );
...
async function getQuestProgress(
  ...
  switch (quest.key) {
    case "spin_1": {
      const r = await db.query(
        `SELECT COUNT(*)::int AS n FROM event_wheel_spins WHERE event_id=$1 AND user_id=$2 AND (created_at AT TIME ZONE '${TZ}')::date = $3::date`,
        [eventId, userId, todayDate]
      );
      return Number(r.rows?.[0]?.n ?? 0);
    }
    case "watch_20": {
      const r = await db.query(
        `SELECT COUNT(DISTINCT svm.bucket_ts)::int AS n FROM stream_viewer_minutes svm WHERE svm.user_id=$1 AND (svm.bucket_ts AT TIME ZONE '${TZ}')::date = $2::date`,
        [userId, todayDate]
      );
      return Number(r.rows?.[0]?.n ?? 0);
    }
    case "chat_5": {
      const r = await db.query(
        `SELECT COUNT(*)::int AS n FROM chat_messages cm WHERE cm.user_id=$1 AND cm.deleted_at IS NULL AND (cm.created_at AT TIME ZONE '${TZ}')::date = $2::date`,
        [userId, todayDate]
      );
      return Number(r.rows?.[0]?.n ?? 0);
    }
    ...
    case "watch_2h": {
      const r = await db.query(
        `SELECT COUNT(DISTINCT svm.bucket_ts)::int AS n FROM stream_viewer_minutes svm WHERE svm.user_id=$1 AND svm.bucket_ts >= $2::timestamptz AND svm.bucket_ts < $3::timestamptz`,
        [userId, event.start_at, event.end_at]
      );
      return Number(r.rows?.[0]?.n ?? 0);
    }
    case "points_3000": {
    ...
    case "days_3": {
      const r = await db.query(
        `SELECT COUNT(DISTINCT (created_at AT TIME ZONE '${TZ}')::date)::int AS n FROM event_wheel_spins WHERE event_id=$1 AND user_id=$2`,
        [eventId, userId]
      );
      return Number(r.rows?.[0]?.n ?? 0);
    }
...
export type ClaimQuestResult =
  | { ok: false; error: "no_event" | "unknown_quest" | "not_done" | "already_claimed" }
  | { ok: true; ticketsAwarded: number; tickets: number };

export async function claimQuest(eventId: number, userId: number, questKey: string): Promise<ClaimQuestResult> {
  ...
  try {
    ...
    const already = await client.query(
      `SELECT 1 FROM event_quest_claims WHERE event_id=$1 AND user_id=$2 AND quest_key=$3`,
      [eventId, userId, storageKey]
    );
    ...
    const ins = await client.query(
      `INSERT INTO event_quest_claims (event_id, user_id, quest_key) VALUES ($1,$2,$3) ON CONFLICT DO NOTHING RETURNING 1`,
      [eventId, userId, storageKey]
    );
...
async function getBoughtCount(db: Db, eventId: number, userId: number, item: ShopItem, todayDate: string): Promise<number> {
  if (item.capMode === "day") {
    const r = await db.query(
      `SELECT COUNT(*)::int AS n FROM event_shop_purchases WHERE event_id=$1 AND user_id=$2 AND item_code=$3 AND day_date=$4::date`,
      [eventId, userId, item.code, todayDate]
    );
    return Number(r.rows?.[0]?.n ?? 0);
  }
  const r = await db.query(
    `SELECT COUNT(*)::int AS n FROM event_shop_purchases WHERE event_id=$1 AND user_id=$2 AND item_code=$3`,
    [eventId, userId, item.code]
  );
  return Number(r.rows?.[0]?.n ?? 0);
}

async function applyShopItemEffect(
  client: PoolClient,
  eventId: number,
  userId: number,
  item: ShopItem
...
export type BuyShopItemResult =
  | { ok: false; error: "no_event" | "unknown_item" | "cap_reached" | "not_enough_rubis" | "ticket_cap" }
  | {
      ok: true;
      spentRubis: number;
      rubis: number;
...
export async function buyShopItem(eventId: number, userId: number, itemCode: string): Promise<BuyShopItemResult> {
  const item = SHOP_ITEMS.find((i) => i.code === itemCode);
  if (!item) return { ok: false, error: "unknown_item" };

  const eventRes = await pool.query(`SELECT * FROM events WHERE id=$1 AND type='wheel_week' AND state='live'`, [eventId]);
  if (!eventRes.rows?.[0]) return { ok: false, error: "no_event" };
  ...
  try {
    ...
    await client.query(
      `INSERT INTO event_shop_purchases (event_id, user_id, item_code, day_date) VALUES ($1,$2,$3,$4::date)`,
      [eventId, userId, item.code, todayDate]
    );
...
export async function getWheelState(db: Db, eventId: number, userId?: number) {
  ...
  const leaderboard = (leaderboardRes.rows || []).map((r: any) => ({
    rank: Number(r.rank),
  ...
  if (userId) {
    ...
    const paliersClaimed = (claimsRes.rows || []).map((r: any) => Number(r.palier_index));

    const nextPalierDef =
      PALIERS.find((p) => !paliersClaimed.includes(p.index) && p.threshold > points) ??
    ...
    const freeRes = await db.query(
      `SELECT 1 FROM event_quest_claims WHERE event_id=$1 AND user_id=$2 AND quest_key=$3`,
      [eventId, userId, `__free_spin:${todayDate}`]
    );
  ...
  type QuestOut = { key: string; label: string; goal: number; progress: number; done: boolean; claimed: boolean };
  const questsOut: { daily: QuestOut[]; weekly: QuestOut[] } = { daily: [], weekly: [] };
