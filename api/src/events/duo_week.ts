...
export type SharedAudiencePair = {
  streamerAId: number;
  streamerBId: number;
 ...
 */
export async function computeSharedAudience(windowDays = SHARED_AUDIENCE_WINDOW_DAYS): Promise<SharedAudiencePair[]> {
 ...
 */
export async function proposeDuos(eventId: number) {
  const existing = await pool.query(`SELECT 1 FROM event_duos WHERE event_id=$1 LIMIT 1`, [eventId]);
  if ((existing.rowCount ?? 0) > 0) return { ok: true as const, skipped: "already_proposed" as const };
  ...
  const streamers = (sizeRes.rows || []).map((r: any) => ({ id: Number(r.id), size: Number(r.size || 0) }));

  const sharedPairs = await computeSharedAudience();
  const sharedMap = new Map<string, number>();
  for (const p of sharedPairs) sharedMap.set(`${Math.min(p.streamerAId, p.streamerBId)}-${Math.max(p.streamerAId, p.streamerBId)}`, p.sharedViewers);
  const sharedOf = (a: number, b: number) => sharedMap.get(`${Math.min(a, b)}-${Math.max(a, b)}`) ?? 0;
  ...
  const candidates: { a: number; b: number; shared: number; sizeDiff: number }[] = [];
  ...
  const duos: { a: number; b: number; shared: number }[] = [];
  ...
  return { ok: true as const, duos: duos.length, unmatched: unmatched.length };
}

// ── Quêtes/paliers du duo (remplace l'ancien barème d'activité brute) ────
// Un duo progresse à travers 5 paliers en accomplissant des quêtes. Chaque
// quête est soit "impartiale" (cat streamer/collab, indépendante de la
// taille de commu — exactement une par palier) soit dépendante de l'activité
// de la commu. Voir la règle de complétion dans evaluatePaliers ci-dessous.
export type DuoQuestCat = "streamer" | "commu" | "collab";
export type DuoQuestMetric =
  | "each_days"
  | "each_days_2h"
  | "each_days5_or_15h"
  | "each_open"
...
export type DuoQuestDef = {
  key: string;
  label: string;
  cat: DuoQuestCat;
  impartial: boolean;
  goal: number;
  metric: DuoQuestMetric;
  unit: string;
};

export type DuoPalierDef = {
  index: number;
  name: string;
  quests: DuoQuestDef[];
};

export const DUO_PALIERS: DuoPalierDef[] = [
  {
    index: 0,
    name: "Rencontre",
    quests: [
 ...
 */
export async function computeDuoMetrics(
  db: Db,
  aId: number,
  bId: number | null,
  start: Date | string,
  ...
  const [streamRes, watchRes, chestRes, opensRes, hostRes, subsRes] = await Promise.all([
    ...
    db.query(
      `SELECT COUNT(*)::int AS watch FROM stream_viewer_minutes
       WHERE streamer_id = ANY(ARRAY[$1,$2]::int[]) AND bucket_ts >= $3::timestamptz AND bucket_ts < $4::timestamptz`,
      [aId, bId, start, end]
    ),
    db.query(
      `SELECT COALESCE(SUM(amount),0)::int AS dep FROM wallet_tx
    ...
    db.query(
      `SELECT streamer_id, COUNT(*)::int AS n FROM streamer_chest_openings
    ...
    bId
      ? db.query(
          `SELECT
             ...
             EXISTS(SELECT 1 FROM streamer_hosts WHERE hoster_streamer_id=$2 AND target_streamer_id=$1) AS b_hosts_a`,
          [aId, bId]
        )
      : Promise.resolve({ rows: [{ a_hosts_b: false, b_hosts_a: false }] } as any),
    db.query(
      `SELECT COUNT(*)::int AS subs FROM sub_gift_claims
       WHERE streamer_id = ANY(ARRAY[$1,$2]::int[]) AND claimed_at >= $3::timestamptz AND claimed_at < $4::timestamptz`,
      [aId, bId, start, end]
    ),
  ]);

  const streamByStreamer = new Map<number, { days: number; hours: number; has2h: boolean }>();
  ...
  const opensByStreamer = new Map<number, number>();
...
function evaluateQuest(q: DuoQuestDef, m: DuoMetrics): { current: number; done: boolean } {
  switch (q.metric) {
    case "each_days":
    ...
    default: {
      const _exhaustive: never = q.metric;
      throw new Error(`metric duo inconnue: ${_exhaustive}`);
    }
  }
}

export type DuoQuestProgress = DuoQuestDef & { current: number; done: boolean };
export type DuoPalierProgress = { index: number; name: string; done: boolean; quests: DuoQuestProgress[] };
 ...
 */
export function evaluatePaliers(m: DuoMetrics): DuoPalierProgress[] {
 ...
 */
export async function recomputeDuoWeek(eventId: number) {
  ...
  if (String(e.type) !== "duo_week") return { ok: false as const, error: "wrong_type" as const };
  ...
  const duosRes = await pool.query(
    `SELECT id, streamer_a_id, streamer_b_id FROM event_duos WHERE event_id=$1`,
    [eventId]
  );
  ...
  for (const row of duosRes.rows || []) {
    ...
    await pool.query(
      ...
      [eventId, duoId, paliersDone, JSON.stringify({ paliers })]
    );
  }
...
export type RankedDuo = {
  rank: number;
  duoId: number;
  streamerAId: number;
  streamerASlug: string;
  streamerADisplayName: string;
  streamerAUserId: number | null;
  streamerBId: number | null;
  streamerBSlug: string | null;
  streamerBDisplayName: string | null;
  streamerBUserId: number | null;
  status: string;
  sharedViewers: number;
  refreshedCount: number;
  points: number;
  paliersDone: number;
  paliers: DuoPalierProgress[];
};
 ...
 */
export async function getRankedDuos(db: Db, eventId: number, limit = 20): Promise<RankedDuo[]> {
  ...
  return (r.rows || []).map((row: any, idx: number) => ({
    ...
    paliers: (row.detail?.paliers ?? []) as DuoPalierProgress[],
  }));
}

export async function acceptDuoForStreamer(eventId: number, streamerId: number) {
  const r = await pool.query(
    `
    UPDATE event_duos
    SET status='accepted'
...
export type RefreshDuoResult =
  | { ok: true; duo: any }
  | { ok: false; error: "no_duo" | "refresh_used" | "no_candidate" };
 ...
 */
export async function refreshDuoForStreamer(eventId: number, streamerId: number): Promise<RefreshDuoResult> {
  ...
  try {
    ...
    const dRes = await client.query(
      `SELECT * FROM event_duos WHERE event_id=$1 AND (streamer_a_id=$2 OR streamer_b_id=$2) LIMIT 1`,
      [eventId, streamerId]
    );
    ...
    const usedRes = await client.query(
      `SELECT streamer_a_id, streamer_b_id FROM event_duos WHERE event_id=$1 AND id <> $2`,
      [eventId, duo.id]
    );
    const used = new Set<number>([streamerId]);
    if (duo.streamer_a_id != null) used.add(Number(duo.streamer_a_id));
    if (duo.streamer_b_id != null) used.add(Number(duo.streamer_b_id));
    for (const row of usedRes.rows || []) {
      if (row.streamer_a_id != null) used.add(Number(row.streamer_a_id));
      if (row.streamer_b_id != null) used.add(Number(row.streamer_b_id));
    }
