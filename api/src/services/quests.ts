...
function parisDateParts(d: Date) {
...
export function getCurrentPeriodKey(type: QuestType, now: Date = new Date()): string {
  if (type === "daily") {
...
export function getPeriodStartSql(type: QuestType, now: Date = new Date()): string {
  if (type === "daily") {
    const { year, month, day } = parisDateParts(now);
...
export function getPeriodExpiresAtIso(type: QuestType, now: Date = new Date()): string {
...
export async function computeMetricProgress(
  client: PoolClient,
  userId: number,
  metric: string,
  type: QuestType
  ...
  switch (metric) {
    ...
    case "call": {
      const r = await client.query(
        `SELECT COUNT(*)::int AS n FROM calls_actions
          WHERE user_id = $1 AND created_at >= $2::timestamptz`,
        [userId, periodStart]
      );
      return Number(r.rows[0]?.n || 0);
    ...
    case "discord_claim": {
      const r = await client.query(
        `SELECT COUNT(*)::int AS n FROM wallet_tx
    ...
    case "discord_blackjack": {
      ...
      const r = await client.query(
        `SELECT COUNT(DISTINCT date_trunc('minute', created_at))::int AS n
           FROM wallet_tx
    ...
    case "chest_open": {
      ...
      const r = await client.query(
        `SELECT COUNT(*)::int AS n FROM streamer_chest_payouts
          WHERE user_id = $1 AND created_at >= $2::timestamptz`,
        [userId, periodStart]
      );
      return Number(r.rows[0]?.n || 0);
    ...
    case "daily_claim": {
      const r = await client.query(
        `SELECT COUNT(*)::int AS n FROM daily_bonus_claims
  ...
  try {
    ...
    for (const type of types) {
      ...
      const seeds = await client.query(
        `SELECT q.code, q.type, q.title, q.description, q.metric, q.target,
    ...
    return out.sort((a, b) => {
      const ord = { daily: 0, weekly: 1, monthly: 2 } as const;
...
export async function claimQuest(
  ...
  try {
    ...
    const q = await client.query(
      `SELECT code, type, metric, target, reward_rubis, reward_origin, reward_meta
         FROM quests WHERE code = $1 AND enabled = TRUE LIMIT 1`,
      [questCode]
    );
    ...
    const seed = await client.query(
      `SELECT 1 FROM quest_seeds WHERE quest_code = $1 AND period_key = $2 LIMIT 1`,
      [questCode, periodKey]
    );
    if (!seed.rows[0]) {
      await client.query("ROLLBACK");
    ...
    await client.query(
      `INSERT INTO quest_progress (user_id, quest_code, period_key, progress_value)
       VALUES ($1, $2, $3, 0)
       ON CONFLICT (user_id, quest_code, period_key) DO NOTHING`,
      [userId, questCode, periodKey]
    );
    const lock = await client.query(
      `SELECT progress_value, claimed_at FROM quest_progress
        WHERE user_id = $1 AND quest_code = $2 AND period_key = $3
        FOR UPDATE`,
      [userId, questCode, periodKey]
    );
    ...
    if (rewardMeta?.title_code) {
      await client.query(
        ...
        [userId, String(rewardMeta.title_code)]
      ).catch(() => {});
    }
    ...
    await client.query(
      `UPDATE quest_progress
          SET progress_value = $4,
              ...
              updated_at = NOW()
        WHERE user_id = $1 AND quest_code = $2 AND period_key = $3`,
      [userId, questCode, periodKey, progress]
    );
...
export async function seedActiveQuestsForPeriod(
  type: QuestType,
  periodKey: string,
  count: number
  ...
  try {
    ...
    const existing = await client.query(
      `SELECT 1 FROM quest_seeds WHERE type = $1 AND period_key = $2 LIMIT 1`,
      [type, periodKey]
    );
    if (existing.rows[0]) return { added: [] };

    const pool_ = await client.query(
      `SELECT code FROM quests WHERE type = $1 AND enabled = TRUE`,
      [type]
    );
    const codes = pool_.rows.map((r: any) => String(r.code));
    ...
    for (const code of picked) {
      await client.query(
        `INSERT INTO quest_seeds (type, period_key, quest_code)
         VALUES ($1, $2, $3)
         ON CONFLICT DO NOTHING`,
        [type, periodKey, code]
      );
...
export async function seedAllPeriodsIfNeeded(): Promise<{
  daily: string[];
  weekly: string[];
  monthly: string[];
}> {
