...
export interface DealParams {
  ...
  ersAgencyPercent: number | null; // % agency keeps of rs_amount
}
...
export interface AdjustedDelta {
  signups: number;
  ftd: number;
  sumDep: number;
  totalDeposits: number;
...
export interface PerSnapshotAgg {
...
export interface PeriodTotals {
  ...
  totalDeposits: number;
  rsAmount: number;
  cpaEarnings: number; // streamer's CPA earnings for the period
  rsEarnings: number;  // streamer's RS earnings for the period
...
export interface AggregateResult {
  rawTotals: PeriodTotals;
  adjustedTotals: PeriodTotals;
  agencyTotals: {
  ...
  perSnapshot: PerSnapshotAgg[];
}

// ── Helpers internes ─────────────────────────────────────────────────────────

function cpaStreamerUnit(deal: DealParams): number {
  const cpa = Number(deal.cpaAmount ?? 0);
  const cut = Number(deal.cpaAgencyCut ?? 0);
  return Math.max(cpa - cut, 0);
}

function rsStreamerPct(deal: DealParams): number {
  const agencyPct = Number(deal.ersAgencyPercent ?? 0);
  return Math.max(0, Math.min(100, 100 - agencyPct));
}

function rsAgencyPct(deal: DealParams): number {
  return Number(deal.ersAgencyPercent ?? 0);
}

function computeSnapshotDelta(snap: SnapshotRaw, prev: SnapshotRaw | null): RawDelta {
  return {
...
export function computeBonusSplit(params: {
  ...
  deltaFtd: number;
  deltaSumDep: number;
  deltaTotalDeposits: number;
  deltaRsAmount: number;
 ...
 */
export function applyBonusToDelta(
  rawDelta: RawDelta,
  bonusComputed: BonusSplitResult
): AdjustedDelta {
 ...
 */
export function aggregateSnapshotsForPeriod(
  snapshots: SnapshotRaw[],
  baseline: SnapshotRaw | null,
  deal: DealParams
): AggregateResult {
  ...
  const perSnapshot: PerSnapshotAgg[] = [];
  const rawDeltas: RawDelta[] = [];
  const adjustedDeltas: AdjustedDelta[] = [];
  ...
  for (let i = 0; i < snapshots.length; i++) {
    ...
    const bonusComputed: BonusSplitResult = {
  ...
  const rawTotals: PeriodTotals = { ...rawTotalsPre, bonusTotal: totalBonusAmount };

  const adjustedTotalsPre = buildPeriodTotals(adjustedDeltas, cpuUnit, rsPct);
  const adjustedTotals: PeriodTotals = { ...adjustedTotalsPre, bonusTotal: totalBonusAmount };
