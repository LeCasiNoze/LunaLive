...
export async function migrate() {
  ...
  `);
  // ──────────────────────────────────────────────────────────
  // 🎁 STREAMER CHEST (coffre)
  // - Persiste entre streams
  // - Tout ce qui "sort" a un weight <= 0.20 (2000 bp)
  // - Le streamer owner ne peut pas participer
  // ──────────────────────────────────────────────────────────
  await pool.query(`
    ...
    CREATE TABLE IF NOT EXISTS streamer_chest_payouts (
      ...
      user_id INT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      amount INT NOT NULL,
      breakdown JSONB NOT NULL DEFAULT '{}'::jsonb, -- { "2000": 12, "1000": 3 }
    ...
    -- Auto-mint state (3 rubis / 5 viewer-minutes global, avec carry)
