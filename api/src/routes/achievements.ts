...
type Metrics = {
  ...
  // Calls
  callsSentTotal: number;
  callsPaidTotal: number;
  callsMaxMultiplier: number; // pay/bet max observé — 0 si pas de données

  // Blackjack
  bjHandsTotal: number;
  bjHasNatural: boolean;
  bjHasBrelan: boolean; // 21+3 Trips ou mieux
  bjConsecutiveWins: number; // max séquence gagnante (fenêtre des 20 dernières mains)
  bjNetGainTotal: number;

  // Économie
  userRubisBalance: number;
  lifetimeRubisEarned: number;
  shopSpentRubis: number;
  hasPreviouslyHadRubis: boolean; // proxy pour succès "broke"

  // Quêtes
  questDailyClaimedTotal: number;
  questWeeklyClaimedTotal: number;
  questMonthlyClaimedTotal: number;

  // Daily & Wheel lifetime
  dailyClaimsTotal: number;
  wheelSpinsLifetimeTotal: number;

  // Chat extras
  chatNightOwlCount: number; // messages entre 2h-5h Europe/Paris

  // Watch (déjà watchMinutesTotal mais en alias explicite)
  // watchMinutesTotal = déjà dans Metrics, réutilisé

  // Discord
  discordLinked: boolean;
  discordCommandDistinct: number; // COUNT(DISTINCT command) dans discord_command_uses
  discordSlotUses: number;
  discordClaimUses: number;

  // XP / Levels
  userXp: number;
  xpThisWeek: number; // XP gagné sur la semaine calendaire courante (Paris)
...
async function getMetrics(userId: number): Promise<Metrics> {
  ...
  const bjHasNatural = hasBlackjackHands
    ? (await safeCount(
        `SELECT COUNT(*)::int AS n FROM blackjack_hands WHERE user_id=$1 AND result='blackjack'`,
        [userId]
      )) > 0
    : false;
  ...
  if (hasBlackjackHands) {
    try {
      const r = await q<{ n: string | number }>(
        `SELECT COUNT(*)::int AS n
  ...
  if (hasBlackjackHands && bjHandsTotal > 0) {
    try {
      const r = await q<{ result: string }>(
        `SELECT result FROM blackjack_hands WHERE user_id=$1 ORDER BY created_at DESC LIMIT 50`,
        [userId]
      );
      let cur = 0;
      let max = 0;
  ...
  const bjNetGainTotal = hasBlackjackHands
    ? await safeSum(
        `SELECT COALESCE(SUM(net_gain),0)::numeric AS s FROM blackjack_hands WHERE user_id=$1`,
        [userId]
      )
    : 0;
  ...
  const lifetimeRubisEarned = hasRubisTx
    ? await safeSum(
        `SELECT COALESCE(SUM(amount),0)::int AS s
         FROM rubis_tx
         WHERE to_user_id=$1 AND kind='mint' AND status='succeeded'`,
        [userId]
      )
    : 0;
  ...
  const shopSpentRubis = hasRubisTx
    ? await safeSum(
        `SELECT COALESCE(SUM(amount),0)::int AS s
         FROM rubis_tx
         WHERE from_user_id=$1 AND kind='sink' AND purpose='shop_purchase' AND status='succeeded'`,
        [userId]
      )
    : 0;
  ...
  const questDailyClaimedTotal =
    hasQuestProgress && hasQuests
      ? await safeCount(
          `SELECT COUNT(*)::int AS n
           FROM quest_progress qp
           JOIN quests q ON q.code = qp.quest_code
           WHERE qp.user_id=$1 AND qp.claimed_at IS NOT NULL AND q.type='daily'`,
          [userId]
        )
      : 0;

  const questWeeklyClaimedTotal =
    hasQuestProgress && hasQuests
      ? await safeCount(
          `SELECT COUNT(*)::int AS n
           FROM quest_progress qp
           JOIN quests q ON q.code = qp.quest_code
           WHERE qp.user_id=$1 AND qp.claimed_at IS NOT NULL AND q.type='weekly'`,
          [userId]
        )
      : 0;

  const questMonthlyClaimedTotal =
    hasQuestProgress && hasQuests
      ? await safeCount(
          `SELECT COUNT(*)::int AS n
           FROM quest_progress qp
           JOIN quests q ON q.code = qp.quest_code
           WHERE qp.user_id=$1 AND qp.claimed_at IS NOT NULL AND q.type='monthly'`,
          [userId]
        )
      : 0;
  ...
  const discordCommandDistinct = hasDiscordCommandUses
    ? await safeCount(
        `SELECT COUNT(DISTINCT command)::int AS n FROM discord_command_uses WHERE user_id=$1`,
        [userId]
      )
    : 0;
  ...
  const discordSlotUses = hasDiscordCommandUses
    ? await safeCount(
        `SELECT COUNT(*)::int AS n FROM discord_command_uses WHERE user_id=$1 AND command='slot'`,
        [userId]
      )
    : 0;
  ...
  const discordClaimUses = hasDiscordCommandUses
    ? await safeCount(
        `SELECT COUNT(*)::int AS n FROM discord_command_uses WHERE user_id=$1 AND command='claim'`,
        [userId]
      )
    : 0;
  ...
  const xpThisWeek = hasXpEvents
    ? await safeSum(
        `SELECT COALESCE(SUM(delta),0)::int AS s
         FROM xp_events
         WHERE user_id=$1
           AND created_at >= date_trunc('week', now() AT TIME ZONE 'Europe/Paris') AT TIME ZONE 'Europe/Paris'`,
        [userId]
      )
    : 0;
  ...
  const firstChatterCount = hasStreamFirstChatters
    ? await safeCount(
        `SELECT COUNT(*)::int AS n FROM stream_first_chatters WHERE user_id=$1`,
        [userId]
      )
    : 0;
...
achievementsRouter.get("/", async (req, res) => {
  ...
  try {
    ...
    if (hasUnlocksTable && unlockedIds.length > 0) {
      ...
      await pool.query(
        `INSERT INTO user_achievements_unlocks (user_id, achievement_id)
      ...
      const claimedRows = await pool.query<{ achievement_id: string }>(
        `UPDATE user_achievements_unlocks
         SET xp_awarded = -1
      ...
      if (claimedRows.rows.length > 0) {
        ...
        for (const d of defs) tierById[d.id] = d.tier;
        ...
        try {
          ...
          for (const row of claimedRows.rows) {
            ...
            if (xpAmount > 0) {
              ...
              await client.query(
                `UPDATE user_achievements_unlocks SET xp_awarded = $3
                 WHERE user_id = $1 AND achievement_id = $2`,
                [userId, row.achievement_id, xpAmount]
              );
