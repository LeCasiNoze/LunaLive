...
import { requireAdminKey } from "../auth.js"; // on ne l'utilise pas ici (on fait interne)
 ...
 */
function requireInternalBot(req: any, res: any, next: any) {
  const got = String((req.headers as any)["x-access-token"] || "");
  const want = String(process.env.INTERNAL_BOT_TOKEN || "");
  if (!want || got !== want) return res.status(401).json({ ok: false, error: "unauthorized" });
  next();
}
...
type CreatePayload = {
  ...
  // infos compactes (on stocke ce qu'on peut)
  dliveUrl?: string | null;
  otherLinks?: string | null;
  experience?: string | null;
};

internalBotStreamerRequestsRouter.post(
  "/internal/bot/streamer-requests/create",
  requireInternalBot,
  a(async (req, res) => {
    ...
    if (!discordUserId || !discordGuildId) return res.status(400).json({ ok: false, error: "bad_input" });
    ...
    if (!userId) return res.status(409).json({ ok: false, error: "not_linked" });
    ...
    try {
      await pool.query(
        `UPDATE streamer_requests SET notes = $2 WHERE user_id = $1`,
        [userId, JSON.stringify({ otherLinks, experience })]
      );
    } catch {
 ...
 */
internalBotStreamerRequestsRouter.post(
  "/internal/bot/streamer-requests/:id/approve",
  requireInternalBot,
  a(async (req, res) => {
    ...
    try {
      ...
      await client.query(
        `UPDATE streamers SET suspended_until=NULL, updated_at=NOW() WHERE user_id=$1`,
        [userId]
      );
...
internalBotStreamerRequestsRouter.post(
  "/internal/bot/streamer-requests/:id/reject",
  requireInternalBot,
  a(async (req, res) => {
    const id = Number(req.params.id);
    if (!id) return res.status(400).json({ ok: false, error: "bad_id" });
