  ...
  a(async (req, res) => {
    ...
    if (!source) {
      ...
      const archive = await pool.query(
        `SELECT video_id, title, thumbnail_url, started_at, ended_at, duration_sec,
      ...
      if (vods.length === 0) {
        const rumbleRow = await pool.query(
          `SELECT live_id, title, thumbnail_url, live_started_at, vod_mp4_url, vod_hls_url
