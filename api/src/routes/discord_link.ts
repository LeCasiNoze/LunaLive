...
discordLinkRouter.post("/link/consume", requireAuth, async (req, res) => {
  const userId = Number(req.user?.id || 0);
  if (!userId) return res.status(401).json({ ok: false, error: "unauthorized" });

  const code = normCode((req.body as any)?.code);
  if (!code || code.length < 6) return res.status(400).json({ ok: false, error: "bad_code" });
  ...
  try {
    ...
    // consume
    await client.query(
      `UPDATE discord_link_codes SET consumed_at = NOW() WHERE id = $1`,
      [row.id]
    );
    ...
    // réponse simple (le bot fera la sync)
