...
type RefStatus = "all" | "pending" | "validated";
...
adminReferralsRouter.get(
  ...
  a(async (req, res) => {
    ...
    const statusRaw = String((req.query as any)?.status ?? "all").toLowerCase();
    const status: RefStatus =
      statusRaw === "pending" ? "pending" : statusRaw === "validated" ? "validated" : "all";
