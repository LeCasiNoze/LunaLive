...
function normText(v: any, max = 200) {
  const s = String(v ?? "").trim();
  return s ? s.slice(0, max) : "";
}

function clampInt(n: any, a: number, b: number) {
...
adminReportsRouter.get("/", async (req, res) => {
  try {
    ...
    for (const row of c.rows) {
      ...
      if (st === "open") counts.open = Number(row.n || 0);
      if (st === "closed") counts.closed = Number(row.n || 0);
    }
