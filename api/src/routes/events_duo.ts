...
async function getOwnStreamerId(userId: number) {
...
eventsDuoRouter.get(
  ...
  a(async (req: any, res) => {
    ...
    let myDuo: ReturnType<typeof mapDuo> | null = null;
    if (userId) {
...
eventsDuoRouter.post(
  "/events/duo/accept",
  requireAuth,
  a(async (req: any, res) => {
    const userId = Number(req.user?.id || 0);
    const streamerId = await getOwnStreamerId(userId);
    if (!streamerId) return res.status(403).json({ ok: false, error: "not_a_streamer" });

    const event = await getLiveDuoEvent();
    if (!event) return res.status(400).json({ ok: false, error: "no_active_event" });

    const duo = await acceptDuoForStreamer(Number(event.id), streamerId);
    if (!duo) return res.status(404).json({ ok: false, error: "no_duo" });
