// api/src/routes/me_discord_link.ts
...
meDiscordLinkRouter.post("/me/discord-link/sync", requireAuth, async (req, res) => {
  const userId = Number(req.user!.id);

  const r = await pool.query(
    `SELECT discord_user_id FROM discord_links WHERE user_id = $1 LIMIT 1`,
    [userId]
  );
  const row = r.rows?.[0];
  if (!row) return res.status(404).json({ ok: false, error: "not_linked" });
