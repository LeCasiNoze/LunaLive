  ...
  a(async (req: any, res) => {
    ...
    try {
      ...
      if (!alreadyVoted) {
        ...
        await client.query(
          `INSERT INTO clip_likes (clip_id, user_id, created_ts) VALUES ($1,$2,$3)
           ON CONFLICT (clip_id, user_id) DO NOTHING`,
          [clipId, userId, Date.now()]
        );
      }
...
eventsClipRaceRouter.get(
  ...
  a(async (req: Request, res) => {
    ...
    if (userId) {
      ...
      myVotedClipIds = (voted.rows || []).map((r: any) => Number(r.clip_id));
    }
