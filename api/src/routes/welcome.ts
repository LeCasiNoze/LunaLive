...
welcomeRouter.post(
  ...
  a(async (req: any, res) => {
    ...
    if (already.rows?.[0]) return res.json({ ok: true, already: true });
    ...
    try {
      ...
      const already2 = await client.query(
        `SELECT 1 FROM welcome_rewards WHERE user_id=$1 LIMIT 1`,
        [userId]
      );
      if (already2.rows?.[0]) {
      ...
      await earnRubisTx(
        ...
        "event_platform",
        50,
        { purpose: "welcome_rewards", kind: "welcome", promoDays: 7 }
      );
      ...
      const viewerTxId: number | null = null;

      // mark welcome rewarded
      await client.query(
        `INSERT INTO welcome_rewards(user_id, viewer_tx_id)
         VALUES ($1, $2)
      ...
      let streamerTxId: number | null = null;
      ...
      if (ref.rows?.[0]?.streamer_id) {
        ...
        const ru = await client.query(
          `SELECT user_id FROM streamers WHERE id=$1 LIMIT 1`,
          [streamerId]
        );
        ...
        if (streamerUserId > 0) {
          ...
          const cap = await client.query(
            `SELECT COUNT(*)::int AS n
             ...
             WHERE streamer_id=$1 AND week_start=$2::date`,
            [streamerId, weekStart]
          );
          ...
          if (n < STREAMER_WEEKLY_CAP) {
            ...
            await earnRubisTx(
              ...
              "paid_topup",
              25,
              { purpose: "referral_streamer_reward", referredUserId: userId, weekStart }
            );
            ...
            await client.query(
              `INSERT INTO referral_streamer_payouts(streamer_id, referred_user_id, week_start, amount, tx_id)
               VALUES ($1,$2,$3::date,25,$4)
               ON CONFLICT (referred_user_id) DO NOTHING`,
              [streamerId, userId, weekStart, streamerTxId]
            );
