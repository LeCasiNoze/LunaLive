...
/* ──────────────────────────────────────────
   ✅ PUBLIC overlay endpoint — head + next call (no auth)
   Utilisé par l'OverlayPage pour afficher la zone "call" sans token.
   GET /public/calls/:slug/queue → { ok, head, next, count }
   ────────────────────────────────────────── */
hunt2Router.get("/public/calls/:slug/queue", async (req: any, res) => {
  try {
    ...
    console.error("[public/calls/queue]", e);
    return res.status(500).json({ ok: false, error: "server_error" });
  }
});
