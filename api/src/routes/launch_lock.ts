...
import {
  ...
  LAUNCH_LOCK_TARGET,
  userPrereqs,
} from "../events/launch_lock.js";

export const launchLockRouter = Router();

launchLockRouter.get(
  "/public/launch-lock",
  a(async (req: any, res) => {
    ...
    let me: { clicked: boolean; eligible: boolean; requirements: Array<{ key: string; label: string; done: boolean }> } | null = null;
    try {
...
launchLockRouter.post(
  "/public/launch-lock/click",
  requireAuth,
  a(async (req: any, res) => {
