...
referralRouter.post(
  ...
  a(async (req: any, res) => {
    ...
    if (!slug) return res.status(400).json({ ok: false, error: "missing_code" });

    const st = await pool.query(
      `SELECT id, slug FROM streamers WHERE lower(slug)=lower($1) LIMIT 1`,
      [slug]
    );
    ...
    const cur = await pool.query(
      `SELECT streamer_id FROM user_referrals WHERE user_id=$1 LIMIT 1`,
      [req.user!.id]
    );
