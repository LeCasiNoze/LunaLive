// api/src/discord/games_claim.ts
...
export type ClaimResult =
  | { ok: true; amount: number; countThisMonth: number; bonus: number; nextAt: Date; monthKey: string }
  | { ok: false; error: "cooldown"; remainingMs: number; nextAt: Date; monthKey: string }
  | { ok: false; error: "db" };

function bonusForCount(n: number) {
  if (n === 10) return 10;
  if (n === 20) return 20;
  if (n === 30) return 30;
  return 0;
 ...
 */
export async function discordDailyClaimTxClient(
  client: PoolClient,
  discordUserId: string
): Promise<ClaimResult & { monthKey?: string }> {
  try {
    const now = new Date();
    ...
    if (!row) {
      ...
      if (String(row.month_key) !== mk) count = 1;
      else count = Number(row.claim_count || 0) + 1;
    ...
    return { ok: true, amount, countThisMonth: count, bonus, nextAt, monthKey: mk } as any;
  } catch {
    return { ok: false, error: "db" } as any;
  }
}
