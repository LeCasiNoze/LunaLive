...
function roleRank(role: string | null | undefined) {
  ...
  if (r === "admin") return 3;
  if (r === "streamer") return 2;
  if (r === "moderator" || r === "mod") return 1;
  return 0; // viewer
}

function minRoleRank(minRole: string | null | undefined) {
...
function canSee(userRole: string | null | undefined, minRole: string | null | undefined) {
...
publicContentRouter.get(
  ...
  a(async (req, res) => {
    ...
    const items = (rows || []).filter((it: any) => canSee(userRole, it?.min_role));
