...
adminRouter.post(
  ...
  a(async (req, res) => {
    ...
    if (!apiKey) return res.status(400).json({ ok: false, error: "apiKey_required" });
    if (!streamKey) return res.status(400).json({ ok: false, error: "streamKey_required" });

    try {
      const { rows } = await pool.query(
        `INSERT INTO rumble_accounts (username, api_key, rtmp_url, stream_key, assigned_to_streamer_id, assigned_at)
         VALUES ($1,$2,$3,$4,$5,NOW())
         RETURNING id, username, rtmp_url AS "rtmpUrl", assigned_at AS "assignedAt"`,
        [username, apiKey, rtmpUrl, streamKey, assignedToStreamerId]
      );
...
adminRouter.post(
  "/admin/rumble-accounts/:id/assign",
  requireAdminKey,
  a(async (req, res) => {
    const id = Number(req.params.id);
    const streamerId = Number(req.body.streamerId);
...
adminRouter.post(
  "/admin/rumble-accounts/:id/release",
  requireAdminKey,
  a(async (req, res) => {
    const id = Number(req.params.id);
    if (!id) return res.status(400).json({ ok: false, error: "bad_id" });
    ...
    const { rowCount } = await pool.query(
      `UPDATE rumble_accounts
       ...
       WHERE id=$1 AND assigned_to_streamer_id IS NOT NULL`,
      [id]
    );
    if (!rowCount) return res.status(404).json({ ok: false, error: "not_found" });
    res.json({ ok: true });
