...
async function spendToken(userId: number, token: string, amount: number) {
  ...
  try {
    ...
    if (cur < amount) throw new Error("insufficient_tokens");
...
shopRouter.get(
  ...
  a(async (req: any, res) => {
    ...
    const u = await pool.query<{ rubis: number }>(
...
shopRouter.post(
  ...
  a(async (req: any, res) => {
    ...
    const it = COSMETICS_CATALOG.find(
      (x) => x.active && x.kind === kind && x.code === code
    );
    ...
    const priceRubis =
      typeof it.priceRubis === "number" ? it.priceRubis : null;
    const pricePrestige =
      typeof (it as any).pricePrestige === "number"
        ? (it as any).pricePrestige
        : null;
