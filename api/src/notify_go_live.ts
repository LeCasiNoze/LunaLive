...
async function sendGenericGoLiveAnnouncement(
  discordClient: Client | null | undefined,
  opts: { displayName: string; title: string; slug: string; webBase: string; thumbnailUrl: string | null }
) {
  ...
  try {
    ...
    if (opts.title) embed.setDescription(`**${opts.title}**`);
    if (opts.thumbnailUrl) embed.setThumbnail(opts.thumbnailUrl);

    const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder().setStyle(ButtonStyle.Link).setLabel("Regarder sur LunaLive").setURL(streamUrl)
    );

    const sendPayload: any = { embeds: [embed], components: [row] };
    if (roleId) {
...
function buildGoLiveEmailHtml(displayName: string, title: string, slug: string): string {
...
async function sendGoLiveEmails(
  streamerId: number,
  opts: { displayName: string; title: string; slug: string }
) {
  if (!isMailReady()) return;
  ...
  const text =
    `${opts.displayName} vient de lancer son live sur LunaLive !` +
...
export async function notifyFollowersGoLive(io: IOServer | undefined, streamerId: number) {
  ...
  if (!claimed) {
    console.log(
      `[notify_go_live] notifs Discord/email skipped for streamer ${streamerId} (cooldown ${getDiscordNotifCooldownMin()}min)`
    );
  } else {
    ...
    const discordClient = (global as any).discordClient as Client | null | undefined;
