...
export function ensureDliveBridge(opts: {
  ...
  let streamerMeta: { id: number; ownerUserId: number | null } | null = null;
  ...
  const broadcast = (m: {
    ...
    if (bang && streamerMeta) {
      handleCallsCommand({
        ...
        console.warn("[dlive_bridge] handleCallsCommand error", opts.slug, e?.message || e)
      );
    }
  };
      ...
      if (!streamerMeta) {
        try {
          const r = await opts.pool.query(
            `SELECT id, user_id AS owner_user_id FROM streamers WHERE slug = $1 LIMIT 1`,
            [opts.slug]
          );
          const row = r.rows[0];
          if (row) {
