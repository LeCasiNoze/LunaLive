...
import { findBestMatch, type SearchContext } from "./support_search.js";
...
import { type KbRole, type KbBlock } from "./kb/index.js";
import { type KbEntry } from "./support_kb.js";
import {
  CID_SUPPORT_CLOSE,
...
async function logUnanswered(opts: {
  ...
  probableSubject?: string;
}) {
  try {
    await pool.query(
      `INSERT INTO support_unanswered
...
function buildRatingRow(ticketId: number, entryId: string) {
 ...
 */
async function detectUserRole(lunaUserId: number): Promise<KbRole> {
...
export async function handleSupportMessage(message: Message, ctx: BotCtx) {
  ...
  try {
    ...
    const isStaff =
      !masterOverride && message.member
        ? hasAnyRole(message.member as GuildMember, STAFF_ROLE_IDS)
        : false;

    let kbRole: KbRole | undefined;
    ...
    const recentMessages = await fetchRecentUserMessages(
      message.channel as TextChannel,
      String(ticket.discord_user_id),
      String(message.id)
    );
    ...
    if (!result || !result.confident) {
      await logUnanswered({
        ...
        bestMatchId: result?.primary ? (result.primary as any).id : undefined,
    ...
    const entryId = (result.primary as KbBlock | KbEntry).id;
    const sensitivity = (result.primary as any).sensitivity as string;
    const confidence = (result.primary as any).confidence as string;

    // ── Cas sensible non confirmé → escalade directe ─────────────
    if (sensitivity === "high" && confidence !== "confirmed") {
      const sensitiveDesc = result.isBlock
        ? (result.primary as KbBlock).summary
        : (result.primary as KbEntry).answer;
    ...
    if (result.isBlock) {
      ...
      const block = result.primary as KbBlock;
      ...
      const entry = result.primary as KbEntry;
    ...
    await logMessage(
      Number(ticket.id),
      "bot",
      `Réponse: ${entryId} (score=${result.score.toFixed(2)}, intent=${result.intent}, block=${result.isBlock}, role=${kbRole ?? "unknown"}, master=${masterOverride})`
    );
  } catch (e: any) {
...
export async function handleSupportRate(interaction: any, ctx: BotCtx) {
  ...
  try {
    await pool.query(
      `INSERT INTO support_ratings (ticket_id, message_id, kb_entry_id, rating, discord_user_id)
       VALUES ($1, $2, $3, $4, $5)
       ON CONFLICT (ticket_id, message_id) DO UPDATE SET rating = EXCLUDED.rating`,
      [ticketId, String(interaction.message?.id ?? ""), entryId, rating, String(interaction.user.id)]
    );
  } catch {
    // non-bloquant
  }
