...
import { safeDm, type BotCtx } from "./utils.js";
...
export function isRestricted(member: GuildMember) {
  return member.roles.cache.has(ROLE_RESTRICTED_ID);
}

export async function applyRolesAndNick(client: Client | null, guild: Guild, member: GuildMember, userRow: any, ctx: BotCtx) {
...
export async function syncUserEverywhere(client: Client | null, discordUserId: string, ctx: BotCtx) {
  if (!client) return;
