...
export type SearchResult = {
  primary: KbBlock | KbEntry;
  score: number;
  confident: boolean;
  /** Intent détecté dans la question de l'utilisateur */
  intent: KbIntent;
  /** true si primary est un KbBlock (nouveau format enrichi) */
  isBlock: boolean;
  /** Entrée complémentaire si pertinente (sujet différent, score suffisant) */
  secondary?: KbBlock | KbEntry;
  secondaryIsBlock?: boolean;
};
...
const INTENT_SIGNALS: [KbIntent, string[]][] = [
...
export function detectIntent(query: string): KbIntent {
  const n = normalize(query);
  for (const [intent, signals] of INTENT_SIGNALS) {
    if (signals.some((s) => n.includes(s))) return intent;
  }
  return "definition";
}
...
function blockSearchText(block: KbBlock): string {
  const parts: string[] = [block.summary];
  if (block.what_it_is) parts.push(block.what_it_is);
  ...
  if (block.troubleshooting) parts.push(...block.troubleshooting);
...
function baseScoreEntry(tokens: string[], entry: KbEntry): number {
...
function baseScoreBlock(tokens: string[], block: KbBlock, bodyTokens: string[]): number {
  if (tokens.length === 0) return 0;
...
type IndexItem =
  | { kind: "block"; item: KbBlock; bodyTokens: string[] }
  | { kind: "entry"; item: KbEntry };

function buildIndex(): IndexItem[] {
  const blockIds = new Set(KB_BLOCKS.map((b) => b.id));

  const blockItems: IndexItem[] = KB_BLOCKS.map((block) => ({
    kind: "block",
    item: block,
  ...
  const entryItems: IndexItem[] = KB_ENTRIES
...
export function findBestMatch(query: string, ctx?: SearchContext): SearchResult | null {
  ...
  type Scored = {
    ...
    score: number;
    isBlock: boolean;
    subject: string; // pour comparer la diversité sujet (secondary)
  };

  const scored: Scored[] = INDEX.map((idx) => {
    if (idx.kind === "block") {
      return {
        idx,
  ...
  let secondary: KbBlock | KbEntry | undefined;
  let secondaryIsBlock: boolean | undefined;
