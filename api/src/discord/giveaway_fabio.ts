...
export async function ensureGiveawaySetup(client: Client, ctx: BotCtx) {
  ...
  let ch = channels?.find(
    (c) => c?.parentId === FABIO_CAT_INFOS_ID && c?.name?.includes("giveaway")
  );
  ...
  try {
    ...
    await rest.post(
      Routes.applicationGuildCommands(client.user.id, FABIO_GUILD_ID) as any,
      { body: GIVEAWAY_COMMAND }
    );
...
export function parseDuration(input: string): Date | null {
  ...
  const absMatch =
    str.match(/^(\d{4})-(\d{2})-(\d{2})\s+(\d{2}):(\d{2})$/) ||
  ...
  if (!relMatch || !str) return null;

  const [, weeks, days, hours, minutes] = relMatch;
  const ms =
    (parseInt(weeks  || "0") * 7 * 24 * 3600 +
...
function discordTimestamp(date: Date, style: "R" | "F" | "D" = "R"): string {
...
function buildGiveawayComponents(giveawayId: string, entryCount: number, ended = false) {
  if (ended) return [];
  return [{
    type: 1,
    components: [{
...
// ─── Handler : /giveaway ──────────────────────────────────────────────────────
export async function handleGiveawayCommand(interaction: any, ctx: BotCtx) {
  ...
  const prize      = interaction.options.getString("prize", true) as string;
  const winners    = interaction.options.getInteger("winners", true) as number;
  const durationStr = interaction.options.getString("duration", true) as string;
  const role1      = interaction.options.getRole("role1") as any;
  const role2      = interaction.options.getRole("role2") as any;
  const role3      = interaction.options.getRole("role3") as any;
  const conditions = interaction.options.getString("conditions") as string | null;
  ...
  const eligibleRoleIds = [role1?.id, role2?.id, role3?.id].filter(Boolean) as string[];
  ...
  try {
    ...
    const r = await pool.query(
      `INSERT INTO discord_giveaways
       ...
       RETURNING id`,
      [FABIO_GUILD_ID, channelId, prize, winners, endsAt, eligibleRoleIds, conditions, interaction.user.id]
    );
    const giveawayId = r.rows[0].id as string;

    // Poster dans le salon giveaway
    const ch = await interaction.client.channels.fetch(channelId).catch(() => null) as TextChannel | null;
    if (!ch) {
    ...
    await pool.query(
      `UPDATE discord_giveaways SET message_id = $1 WHERE id = $2`,
      [msg.id, giveawayId]
    );
...
export async function handleGiveawayEnter(interaction: any, giveawayId: string, ctx: BotCtx) {
  await interaction.deferReply({ ephemeral: true });

  try {
    const r = await pool.query(
      `SELECT * FROM discord_giveaways WHERE id = $1 LIMIT 1`,
      [giveawayId]
    );
    ...
    if (giveaway.eligible_role_ids?.length > 0) {
      const member = interaction.member as GuildMember;
      const hasRole = giveaway.eligible_role_ids.some((id: string) => member.roles.cache.has(id));
      if (!hasRole) {
        const roleList = giveaway.eligible_role_ids.map((id: string) => `<@&${id}>`).join(", ");
        await interaction.editReply({
    ...
    const existing = await pool.query(
      `SELECT 1 FROM discord_giveaway_entries WHERE giveaway_id = $1 AND discord_user_id = $2`,
    ...
    if ((existing.rowCount ?? 0) > 0) {
      // Retrait
      await pool.query(
        `DELETE FROM discord_giveaway_entries WHERE giveaway_id = $1 AND discord_user_id = $2`,
        [giveawayId, interaction.user.id]
      );
      const countRes = await pool.query(
        `SELECT COUNT(*) AS c FROM discord_giveaway_entries WHERE giveaway_id = $1`,
        [giveawayId]
      );
    ...
    // Inscription
    await pool.query(
      `INSERT INTO discord_giveaway_entries (giveaway_id, discord_user_id) VALUES ($1, $2)`,
      [giveawayId, interaction.user.id]
    );
...
async function updateGiveawayMessage(
  ...
  try {
    const ch = await client.channels.fetch(giveaway.channel_id).catch(() => null) as TextChannel | null;
    if (!ch || !giveaway.message_id) return;
...
export async function handleGiveawayCancel(interaction: any, giveawayId: string, ctx: BotCtx) {
  const member = interaction.member as GuildMember;
  if (!isMod(member)) {
    await interaction.reply({ ephemeral: true, content: "❌ Réservé aux modérateurs." });
    return;
  ...
  try {
    const r = await pool.query(
      `UPDATE discord_giveaways SET state = 'cancelled' WHERE id = $1 AND state = 'active' RETURNING *`,
      [giveawayId]
    );
    if (!r.rowCount) {
...
export async function checkExpiredGiveaways(client: Client, ctx: BotCtx) {
  try {
    ...
    for (const giveaway of expired.rows) {
      try {
        // Marquer terminé
        await pool.query(
          `UPDATE discord_giveaways SET state = 'ended' WHERE id = $1`,
          [giveaway.id]
        );

        // Tirer les gagnants au sort
        const entries = await pool.query(
          `SELECT discord_user_id FROM discord_giveaway_entries WHERE giveaway_id = $1 ORDER BY random() LIMIT $2`,
          [giveaway.id, giveaway.winner_count]
        );
        const winnerIds = entries.rows.map((r: any) => r.discord_user_id as string);
        ...
        await pool.query(
          `UPDATE discord_giveaways SET winner_ids = $1 WHERE id = $2`,
          ...
          if (winnerIds.length === 0) {
            ...
            await ch.send({
              ...
              embeds: [{
                ...
                description:
                  `${winnerMentions} ${plural ? "ont remporté" : "a remporté"} **${giveaway.prize}** ! 🎁\n\n` +
