...
import { ChannelType, type Client, type GuildMember, type PartialGuildMember, type TextChannel } from "discord.js";
...
import {
  ...
  FABIO_ROLE_MOD_ID,
  FABIO_ROLE_CHEF_ID,
  FABIO_ROLE_MEMBRE_ID,
} from "./constants.js";

// ─── State ────────────────────────────────────────────────────────────────────
let welcomeChannelId: string | null = null;
let goodbyeChannelId: string | null = null;
...
// ─── Ensure channels ──────────────────────────────────────────────────────────
export async function ensureFabioWelcomeChannels(client: Client, ctx: BotCtx) {
  ...
  let welcome = channels.find(
    (c) => c?.parentId === FABIO_CAT_INFOS_ID && c?.name?.includes("bienvenue")
  );
  if (!welcome) {
  ...
  if (welcome) welcomeChannelId = welcome.id;

  // ── Salon départs (privé, catégorie MODÉRATEURS) ──────────────────────────
  let goodbye = channels.find(
    (c) => c?.parentId === FABIO_CAT_MODO_ID && c?.name?.includes("départ")
  );
  if (!goodbye) {
  ...
  if (goodbye) goodbyeChannelId = goodbye.id;

  ctx.log(`[welcome_fabio] channels ready — welcome:${welcomeChannelId} goodbye:${goodbyeChannelId}`);
}

// ─── Membre rejoint ───────────────────────────────────────────────────────────
export async function handleFabioMemberJoin(member: GuildMember, client: Client, ctx: BotCtx) {
  if (!welcomeChannelId) return;

  try {
    const ch = await client.channels.fetch(welcomeChannelId).catch(() => null) as TextChannel | null;
    if (!ch) return;
    ...
    const offresText = offers.length
      ? offers.map((o) =>
          `▸ **${o.label}** — Dépôt min. **${o.min_deposit}€** → Remboursement **${o.reimburse_amt}€**`
        ).join("\n")
    ...
    await ch.send({
      ...
      embeds: [{
        ...
        description:
          `Tu es le **${memberCount}${ordinal} membre** du serveur.\n\n` +
...
// ─── Membre parti ─────────────────────────────────────────────────────────────
export async function handleFabioMemberLeave(
  ...
  try {
    const ch = await client.channels.fetch(goodbyeChannelId).catch(() => null) as TextChannel | null;
    if (!ch) return;
    ...
    const roles = member.roles instanceof Map
      ? [...(member.roles as any).values()]
          .filter((r: any) => r.id !== member.guild.roles.everyone.id && !r.managed)
          .map((r: any) => `\`${r.name}\``)
          .join(", ")
      : "—";
