...
import type { KbBlock, KbIntent, KbRole, KbStep } from "./kb/index.js";
...
export type BlockResponse = {
  ...
  description: string;
  fields: EmbedField[];
  /** true si le bloc recommande une escalade */
  shouldEscalate: boolean;
  /** footer contextuel selon les flags du bloc */
  footerHint?: string;
};
...
function cap(text: string, max = 1024): string {
  return text.length > max ? text.slice(0, max - 1) + "…" : text;
}

/** Filtre les étapes selon le rôle détecté */
function filterSteps(steps: KbStep[], role: KbRole | undefined): KbStep[] {
  return steps.filter((s) => !s.role || !role || s.role === role || role === "staff");
}

/** Formate les étapes en texte numéroté */
function formatSteps(steps: KbStep[]): string {
  return steps
    .map((s, i) => {
      let line = `${i + 1}. ${s.text}`;
      if (s.note) line += `\n   ↳ *${s.note}*`;
