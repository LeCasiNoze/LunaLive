...
export type BotCtx = { log: (msg: string) => void };

export function maskSecret(v: any) {
...
export function getLinkCodeSecret(ctx?: BotCtx) {
...
export function hashCode(code: string, ctx?: BotCtx) {
...
export async function safeDm(client: Client | null, userId: string, content: string, ctx: BotCtx) {
  if (!client) return;
  try {
...
export function normalizeAccept(v: string) {
  return String(v || "")
    .trim()
    .toUpperCase()
    .replace(/\s+/g, "");
