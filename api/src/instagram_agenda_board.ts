...
type ReelRow = {
  ...
  finished_at: Date;
  streamerSlug: string;
  // Live stats (fetched at refresh time)
  likes?: number;
  comments?: number;
  views?: number;
...
async function loadRecentReels(): Promise<ReelRow[]> {
...
type ScheduledRow = { jobId: number; clipTitle: string; streamerSlug: string; scheduledAt: Date };

async function loadScheduled(): Promise<ScheduledRow[]> {
  const r = await pool.query(
    `
    SELECT
      pj.id::int                 AS job_id,
...
async function enrichWithMetaStats(reels: ReelRow[]): Promise<void> {
  ...
  for (let i = 0; i < reels.length; i += concurrency) {
    ...
    await Promise.all(batch.map(async (r) => {
      ...
      const plays = ins?.data?.find((x: any) => x.name === "plays")?.values?.[0]?.value;
      if (typeof plays === "number") r.views = plays;
    }));
  }
}
...
function shortCaption(text: string | null | undefined, max = 80): string {
  if (!text) return "_(sans titre)_";
...
function buildEmbed(reels: ReelRow[], scheduled: ScheduledRow[], hasLogo: boolean): EmbedBuilder {
  ...
  if (lastWithThumb?.thumbnailUrl) embed.setImage(lastWithThumb.thumbnailUrl);
...
export async function refreshInstagramAgenda(): Promise<void> {
  if (_inFlight) return;
  _inFlight = true;
  try {
    ...
    if (reels.length > 0) await enrichWithMetaStats(reels);
    ...
    const sent = await (channel as any).send({ embeds: [embedCreate], components: [row], files });
    await pool.query(
      `
...
export async function handleIgAgendaRefreshButton(interaction: ButtonInteraction): Promise<void> {
  try {
    ...
    try {
      if (interaction.deferred) await interaction.editReply({ content: "❌ " + (e?.message || e) });
    } catch {}
  }
}

export function startInstagramAgendaBoard(): void {
