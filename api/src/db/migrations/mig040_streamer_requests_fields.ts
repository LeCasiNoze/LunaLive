...
export async function mig040_streamer_requests_fields(pool: Pool) {
  ...
  await pool.query(
    `ALTER TABLE streamer_requests ADD COLUMN IF NOT EXISTS has_channel BOOLEAN NOT NULL DEFAULT FALSE;`
  );
  await pool.query(
    `ALTER TABLE streamer_requests ADD COLUMN IF NOT EXISTS has_dlive BOOLEAN NOT NULL DEFAULT FALSE;`
  );

  await pool.query(`ALTER TABLE streamer_requests ADD COLUMN IF NOT EXISTS dlive_displayname TEXT;`);
  await pool.query(
    `ALTER TABLE streamer_requests ADD COLUMN IF NOT EXISTS rules_accepted BOOLEAN NOT NULL DEFAULT FALSE;`
  );

  await pool.query(
    `ALTER TABLE streamer_requests ADD COLUMN IF NOT EXISTS payload JSONB NOT NULL DEFAULT '{}'::jsonb;`
  );
