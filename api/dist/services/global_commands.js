...
export function isGlobalCommand(cmd) {
...
async function getUserStats(pool, userId, streamerId) {
    // 1) rubis + xp
    const u = await pool.query(`SELECT username, rubis, xp::bigint AS xp FROM users WHERE id = $1 LIMIT 1`, [userId]);
    const row = u.rows[0];
    if (!row)
        return null;
    ...
    let watchMins = 0;
    try {
        const r = await pool.query(`SELECT COUNT(DISTINCT bucket_ts)::int AS n
         FROM stream_viewer_minutes
        WHERE user_id = $1 AND streamer_id = $2`, [userId, streamerId]);
    ...
    let followDate = null;
    try {
        const r = await pool.query(`SELECT created_at FROM streamer_follows
        WHERE user_id = $1 AND streamer_id = $2 LIMIT 1`, [userId, streamerId]);
    ...
    try {
        const r = await pool.query(`SELECT COUNT(*)::int AS n FROM user_entitlements WHERE user_id = $1`, [userId]);
    ...
    try {
        const r = await pool.query(`SELECT display_name, slug FROM streamers WHERE id = $1 LIMIT 1`, [streamerId]);
...
export async function handleGlobalCommand(ctx) {
    ...
    if (cmd === "solde") {
        await sendBotChat(ctx.pool, ctx.io, optsBot, `💎 ${u} → ${stats.rubis.toLocaleString("fr-FR")} rubis · Niveau ${stats.level} (${stats.title})`);
        return { handled: true };
    }
    if (cmd === "watch") {
        ...
        await sendBotChat(ctx.pool, ctx.io, optsBot, `👁️ ${u} → ${fmtDuration(stats.watchMins * 60)} cumulé sur ${stats.streamerName} · ${since}`);
        return { handled: true };
    }
    if (cmd === "succes" || cmd === "succès") {
        ...
        if (summary) {
            ...
            await sendBotChat(ctx.pool, ctx.io, optsBot, `🏆 ${u} — 🥉 ${t.bronze.unlocked}/${t.bronze.total} · 🥈 ${t.silver.unlocked}/${t.silver.total} · 🥇 ${t.gold.unlocked}/${t.gold.total} · 👑 ${t.master.unlocked}/${t.master.total} (${summary.totalUnlocked}/${summary.totalAll})`);
        }
        else {
            await sendBotChat(ctx.pool, ctx.io, optsBot, `🏆 ${u} → ${stats.entitlementsCount} récompenses · lunalive.win/profile`);
        }
        return { handled: true };
    }
    if (cmd === "profil") {
