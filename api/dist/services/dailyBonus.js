...
function rewardByIsoDow(isodow) {
    // ISO: 1=Lun ... 7=Dim
    switch (isodow) {
...
async function getParisNow(client) {
...
async function mintRubis(client, userId, amount, origin, weight_bp) {
    ...
    await client.query(`INSERT INTO rubis_tx_entries (tx_id, entity, user_id, delta) VALUES ($1, 'user', $2, $3);`, [txId, userId, amount]);
...
async function countClaimsThisMonth(client, userId, monthStart) {
    const r = await client.query(`
...
async function milestoneClaimed(client, userId, monthStart, milestone) {
    const r = await client.query(`
    SELECT EXISTS(
...
async function markMilestoneGranted(client, userId, monthStart, milestone, granted) {
    await client.query(`
...
async function claimedToday(client, userId, day) {
    const r = await client.query(`SELECT EXISTS(SELECT 1 FROM daily_bonus_claims WHERE user_id=$1 AND day=$2::date) AS exists;`, [userId, day]);
    return Boolean(r.rows?.[0]?.exists);
}
async function getWeekClaims(client, userId, weekStart) {
    const r = await client.query(`
...
async function getTokens(client, userId) {
    const r = await client.query(`SELECT token, amount FROM user_tokens WHERE user_id=$1;`, [userId]);
    const out = {};
    for (const row of r.rows || [])
        out[row.token] = Number(row.amount ?? 0);
    return out;
}
export async function getDailyBonusState(pool, userId) {
    ...
    try {
        ...
        for (let d = 1; d <= 7; d++) {
            ...
            const rr = await client.query(`SELECT ($1::date + ($2::int - 1) * INTERVAL '1 day')::date::text AS date;`, [weekStart, d]);
            const date = String(rr.rows?.[0]?.date);
            let status = "future";
            const isToday = date === day;
            const isPast = date < day;
            if (isToday)
                status = todayClaimed ? "today_claimed" : "today_claimable";
            else if (isPast)
                status = weekClaims.has(date) ? "claimed" : "missed";
            else
                status = "future";
            week.push({
                isodow: d,
...
export async function claimDailyBonusToday(pool, userId) {
    const client = await pool.connect();
    try {
...
export async function claimDailyBonusMilestone(pool, userId, milestone) {
    const client = await pool.connect();
    try {
        await client.query("BEGIN");
