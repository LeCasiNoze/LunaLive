...
/* ─────────────────────────────────────────────
   ...
   ───────────────────────────────────────────── */
async function hasActivePremiumViewer(client, userId) {
...
function doubleRewardIfPremium(rw, premium) {
    if (!premium)
        return rw;
    if (rw.type === "rubis") {
...
async function getTokens(client, userId) {
    const r = await client.query(`SELECT token, amount FROM user_tokens WHERE user_id=$1;`, [
        userId,
    ]);
    const out = {};
    for (const row of r.rows || [])
