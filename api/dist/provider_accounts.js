export async function getConnectionForStreamerId(client, streamerId) {
    const { rows } = await client.query(`SELECT provider,
 ...
 */
export async function ensureAssignedDliveAccount(client, streamerId) {
    ...
    const { rows } = await client.query(`WITH picked AS (
     ...
     RETURNING pa.provider,
               ...
               pa.stream_key AS "streamKey"`, [streamerId]);
    return rows[0] || null;
}
export async function releaseAccountForStreamerId(client, streamerId) {
    await client.query(`UPDATE provider_accounts
     SET assigned_to_streamer_id = NULL,
         released_at = NOW()
     WHERE assigned_to_streamer_id = $1`, [streamerId]);
}
