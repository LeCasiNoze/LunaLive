...
async function autoMintTick() {
    ...
    if (!toTs)
        return;
    const live = await pool.query(`SELECT id
     FROM streamers
     WHERE is_live=TRUE`);
    for (const row of live.rows || []) {
        ...
        try {
         ...
         FOR UPDATE`, [streamerId]);
           ...
           ON CONFLICT (streamer_id) DO NOTHING`, [streamerId, toTs]);
         ...
         WHERE streamer_id=$1
           ...
           AND bucket_ts <= $3::timestamptz`, [streamerId, lastBucketTs, toTs]);
         ...
         SET last_bucket_ts=$2::timestamptz,
             carry_minutes=$3,
             updated_at=NOW()
         WHERE streamer_id=$1`, [streamerId, toTs, newCarry]);
            if (minted > 0) {
