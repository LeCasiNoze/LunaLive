...
function viewersToRubis(viewers) {
    if (viewers <= 0)
        return 0;
    if (viewers <= 10)
        return 2;
...
async function autoMintTick() {
    ...
    const live = await pool.query(`SELECT id, user_id AS "userId"
     FROM streamers
     WHERE is_live=TRUE`);
    for (const row of live.rows || []) {
        ...
        try {
            ...
            const st = await client.query(`SELECT last_bucket_ts AS "lastBucketTs", carry_minutes AS "carryMinutes"
         FROM streamer_chest_auto_state
         WHERE streamer_id=$1
         FOR UPDATE`, [streamerId]);
            // init state
            if (!st.rows?.[0]) {
                await client.query(`INSERT INTO streamer_chest_auto_state (streamer_id, last_bucket_ts, carry_minutes)
           VALUES ($1, $2::timestamptz, 0)
           ON CONFLICT (streamer_id) DO NOTHING`, [streamerId, toTsIso]);
            ...
            if (diffMin <= 0) {
                await client.query(`UPDATE streamer_chest_auto_state
           SET last_bucket_ts=$2::timestamptz, updated_at=NOW()
           WHERE streamer_id=$1`, [streamerId, toTsIso]);
                await client.query("COMMIT");
                continue;
            }
            // cumule
           ...
           SET last_bucket_ts=$2::timestamptz,
               carry_minutes=$3,
               updated_at=NOW()
           WHERE streamer_id=$1`, [streamerId, toTsIso, remainingCarry]);
                await client.query("COMMIT");
                continue;
            }
             ...
             */
            const viewersRes = await client.query(`SELECT COUNT(DISTINCT viewer_key)::int AS n
         FROM stream_viewer_minutes
         WHERE streamer_id=$1
           AND bucket_ts > $2::timestamptz
           AND bucket_ts <= $3::timestamptz`, [streamerId, windowFromIso, windowToIso]);
...
async function closeExpiredOpenings(io) {
    const r = await pool.query(`SELECT o.id, s.slug
    ...
    const closeFn = mod.closeOpeningAndPayout;
...
export function startChestJobs(io) {
    // fermeture auto
