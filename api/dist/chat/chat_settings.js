...
export async function patchChatSettings(pool, streamerId, patch, actorUserId) {
    ...
    const dliveUsername = dliveUsernameProvided ? toTextOrNull(patch.dliveUsername) : undefined;
    const noBoolChange = allowLinks == null &&
        followOnly == null &&
        subOnly == null &&
...
export function formatSettingsChangeMessage(opts) {
    ...
    if (opts.changed.dliveSyncPublic != null) {
        parts.push(opts.changed.dliveSyncPublic ? "a activé la sync DLive (public)" : "a désactivé la sync DLive (public)");
    }
    if (opts.changed.dliveSyncPopup != null) {
        parts.push(opts.changed.dliveSyncPopup ? "a activé la sync DLive (popup)" : "a désactivé la sync DLive (popup)");
    }
    if (Object.prototype.hasOwnProperty.call(opts.changed, "dliveUsername")) {
        const v = opts.changed.dliveUsername;
