...
lunaclipRouter.get("/logs", async (req, res) => {
    ...
    const r = await pool.query(`SELECT ts, slug, source, msg
     FROM lunaclip_admin_logs
     WHERE ${where}
     ORDER BY id DESC
     LIMIT ${limit}`, args);
    const logs = r.rows.reverse().map((x) => ({
...
lunaclipRouter.post("/control", async (req, res) => {
    ...
    await pool.query(`INSERT INTO lunaclip_admin_commands (action, payload)
     VALUES ($1, $2::jsonb)`, [String(action), JSON.stringify(payload)]);
    res.json({ ok: true });
});
// Historique DB (inchangé)
lunaclipRouter.get("/sessions", async (_req, res) => {
    const r = await pool.query(`SELECT ls.id, ls.hls_url, ls.provider, ls.status,
...
lunaclipRouter.get("/sessions/:id/events", async (req, res) => {
    const r = await pool.query(`SELECT * FROM lunaclip_events WHERE session_id=$1 ORDER BY triggered_at ASC`, [req.params.id]);
    res.json({ ok: true, events: r.rows });
});
lunaclipRouter.get("/sessions/:id/frames", async (req, res) => {
    const limit = Math.min(Number(req.query.limit ?? 500), 2000);
    const r = await pool.query(`SELECT ts_sec, multiplier, multiplier_source, in_bonus,
            win_numeric, win_total_numeric, bet_numeric, free_spins
     FROM lunaclip_frames
     WHERE session_id=$1 AND multiplier IS NOT NULL
     ORDER BY captured_at ASC LIMIT $2`, [req.params.id, limit]);
    res.json({ ok: true, frames: r.rows });
});
lunaclipRouter.get("/clips", async (req, res) => {
    const limit = Math.min(Number(req.query.limit ?? 100), 500);
    const r = await pool.query(`SELECT bc.id, bc.title, bc.author, bc.at_sec,
...
lunaclipRouter.get("/events/recent", async (req, res) => {
    ...
    const r = await pool.query(`SELECT le.*, ls.streamer_id,
            s.slug AS streamer_slug, s.display_name AS streamer_name
     FROM lunaclip_events le
     JOIN lunaclip_sessions ls ON ls.id = le.session_id
     LEFT JOIN streamers s ON s.id = ls.streamer_id
     ORDER BY le.triggered_at DESC LIMIT $1`, [limit]);
    res.json({ ok: true, events: r.rows });
});
