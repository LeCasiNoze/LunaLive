...
async function canControlStreamer(streamerId, ownerUserId, actor) {
...
async function ensureSession(streamerId) {
    await ensureSchema();
    await pool.query(`INSERT INTO calls_hunt_sessions(streamer_id) VALUES($1)
     ON CONFLICT (streamer_id) DO NOTHING`, [streamerId]);
}
async function getSession(streamerId) {
    await ensureSession(streamerId);
    const r = await pool.query(`SELECT mode, opened, start, bet_default, archive_id FROM calls_hunt_sessions WHERE streamer_id=$1`, [streamerId]);
    const row = r.rows?.[0] || {};
    const mode = (row.mode === "open" ? "open" : "farm");
    const opened = !!row.opened;
    const start = row.start === null || typeof row.start === "undefined" ? null : Number(row.start);
    const betDefault = row.bet_default === null || typeof row.bet_default === "undefined"
        ? null
    ...
    return {
        ...
        betDefault: Number.isFinite(betDefault) ? betDefault : null,
        archive_id,
    };
}
async function setSession(streamerId, patch) {
    await ensureSession(streamerId);
    ...
    const nextBetDefault = typeof patch.betDefault !== "undefined" ? patch.betDefault : cur.betDefault;
    const nextArchiveId = typeof patch.archive_id !== "undefined" ? patch.archive_id : cur.archive_id;
    await pool.query(`UPDATE calls_hunt_sessions
     SET mode=$2, opened=$3, start=$4, bet_default=$5, archive_id=$6, updated_at=NOW()
     WHERE streamer_id=$1`, [streamerId, nextMode, nextOpened, nextStart, nextBetDefault, nextArchiveId]);
}
async function loadQueue(streamerId) {
 ...
 */
callsHuntRouter.get("/:slug/hunt/state", async (req, res) => {
    try {
 ...
 */
callsHuntRouter.post("/:slug/hunt/close", async (req, res) => {
    try {
        const slug = String(req.params.slug || "");
        const streamer = await getStreamerBySlug(slug);
        if (!streamer)
 ...
 */
callsHuntRouter.post("/:slug/hunt/revert", async (req, res) => {
    try {
        const slug = String(req.params.slug || "");
        const streamer = await getStreamerBySlug(slug);
        if (!streamer)
 ...
 */
callsHuntRouter.post("/:slug/hunt/open", async (req, res) => {
    try {
        const slug = String(req.params.slug || "");
        const streamer = await getStreamerBySlug(slug);
        if (!streamer)
 ...
 */
callsHuntRouter.post("/:slug/hunt/start", async (req, res) => {
    try {
        const slug = String(req.params.slug || "");
        const streamer = await getStreamerBySlug(slug);
        if (!streamer)
 ...
 */
callsHuntRouter.post("/:slug/hunt/bet", async (req, res) => {
    try {
        const slug = String(req.params.slug || "");
        const streamer = await getStreamerBySlug(slug);
        if (!streamer)
 ...
 */
callsHuntRouter.post("/:slug/hunt/pass", async (req, res) => {
    const slug = String(req.params.slug || "");
    try {
        const streamer = await getStreamerBySlug(slug);
        if (!streamer)
 ...
 */
callsHuntRouter.post("/:slug/hunt/bonus", async (req, res) => {
    try {
        const slug = String(req.params.slug || "");
        const streamer = await getStreamerBySlug(slug);
        if (!streamer)
        ...
        if (!curCall)
            return res.json({ ok: true });
        await ensureCallsQueueIsBonusCol();
        await pool.query(`UPDATE calls_queue
       SET bet=$3, is_bonus=TRUE
       WHERE streamer_id=$1 AND id=$2`, [streamer.id, curCall.id, bet]);
 ...
 */
callsHuntRouter.patch("/:slug/hunt/bonus/:id", express.json(), async (req, res) => {
    try {
        const slug = String(req.params.slug || "");
 ...
 */
callsHuntRouter.delete("/:slug/hunt/bonus/:id", async (req, res) => {
    try {
        const slug = String(req.params.slug || "");
        const id = String(req.params.id || "");
        const streamer = await getStreamerBySlug(slug);
        ...
        if (!okCtl)
            return res.status(403).json({ ok: false, error: "forbidden" });
        await ensureCallsQueueIsBonusCol();
        const r = await pool.query(`DELETE FROM calls_queue
        WHERE streamer_id=$1
          AND id=$2
          AND (COALESCE(is_bonus,FALSE)=TRUE OR (bet IS NOT NULL AND bet > 0))
        `, [streamer.id, id]);
 ...
 */
callsHuntRouter.post("/:slug/hunt/pay", async (req, res) => {
    try {
        const slug = String(req.params.slug || "");
        const streamer = await getStreamerBySlug(slug);
        if (!streamer)
 ...
 */
callsHuntRouter.post("/:slug/hunt/reset", async (req, res) => {
    try {
        const slug = String(req.params.slug || "");
        const streamer = await getStreamerBySlug(slug);
        if (!streamer)
