...
function baseSelectSql(where, limitParamIndex) {
    return `
    SELECT
      c.id::text AS id,
 ...
 */
adminCasinoCommentsRouter.get("/pending", requireAdminKey, a(async (req, res) => {
 ...
 */
adminCasinoCommentsRouter.get("/published", requireAdminKey, a(async (req, res) => {
    const limit = parseLimit(req.query.limit, 50);
    const cursor = req.query.cursor ? String(req.query.cursor) : null;
    const q = String(req.query.q ?? "").trim().toLowerCase();
    const casinoId = req.query.casinoId ? Number(req.query.casinoId) : null;
 ...
 */
adminCasinoCommentsRouter.patch("/:commentId", requireAdminKey, a(async (req, res) => {
