...
publicCallsRouter.get("/api/public/calls/:slug/queue", async (req, res) => {
    try {
        ...
        const stR = await pool.query(`SELECT id FROM streamers WHERE slug=$1 LIMIT 1`, [slug]);
       ...
       LIMIT 2`, [streamer.id]);
        ...
        const cR = await pool.query(`SELECT COUNT(*)::text AS count
         FROM calls_queue
        WHERE streamer_id=$1
          AND pay IS NULL`, [streamer.id]);
