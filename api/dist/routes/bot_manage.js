...
async function getStreamerByOwnerUserId(userId) {
    const r = await pool.query(`SELECT id, slug, appearance, is_live AS "isLive"
     FROM streamers
     WHERE user_id=$1
     LIMIT 1`, [userId]);
    const row = r.rows?.[0];
...
async function listModStreamers(userId) {
    const r = await pool.query(`SELECT s.id, s.slug
     ...
     ORDER BY sm.created_at DESC NULLS LAST, s.id DESC`, [userId]);
    return (r.rows || []).map((x) => ({ id: Number(x.id), slug: String(x.slug) }));
}
/**
 ...
 *              sinon -> 400 streamer_required
 */
async function resolveBotManageTarget(req) {
    const u = req.user;
...
async function getBotSettings(streamerId) {
    // si pas encore upsert => defaults
    const r = await pool.query(`SELECT enabled, prefix, live_only AS "liveOnly"
...
async function assertLiveIfNeeded(isLive) {
    if (!isLive) {
        return { ok: false, status: 409, error: "live_only" };
    }
    return { ok: true };
}
// helper pour dupliquer routes /me/bot/* + /api/my/bot/*
function mountAliases(method, path, ...handlers) {
    // ex path="/overview" => /me/bot/overview + /api/my/bot/overview
    botManageRouter[method](`/me/bot${path}`, ...handlers);
    botManageRouter[method](`/api/my/bot${path}`, ...handlers);
}
/* ──────────────────────────────────────────
   OVERVIEW (counts + settings + rules + soon)
────────────────────────────────────────── */
const overviewHandler = a(async (req, res) => {
    const s = await resolveBotManageTarget(req);
    if (!s.ok)
        return res.status(s.status).json({ ok: false, error: s.error, ...(s.detail ? { detail: s.detail } : {}) });
...
/* ──────────────────────────────────────────
   COMMANDS
────────────────────────────────────────── */
const listCommandsHandler = a(async (req, res) => {
    const s = await resolveBotManageTarget(req);
    if (!s.ok)
        return res.status(s.status).json({ ok: false, error: s.error, ...(s.detail ? { detail: s.detail } : {}) });
...
const createCommandHandler = a(async (req, res) => {
    ...
    if (!trigger)
        return res.status(400).json({ ok: false, error: "bad_trigger" });
    if (!response)
        return res.status(400).json({ ok: false, error: "bad_response" });
    if (trigger.length > 32)
        return res.status(400).json({ ok: false, error: "trigger_too_long" });
    if (response.length > 600)
        return res.status(400).json({ ok: false, error: "response_too_long" });
    ...
    const exists = await pool.query(`SELECT 1 FROM bot_commands WHERE streamer_id=$1 AND lower(trigger)=lower($2) LIMIT 1`, [s.streamerId, trigger]);
    if (exists.rows?.[0])
        return res.status(409).json({ ok: false, error: "trigger_exists" });
    const ins = await pool.query(`INSERT INTO bot_commands(streamer_id, trigger, response, enabled, cooldown_sec)
     VALUES ($1,$2,$3,$4,$5)
     RETURNING
       id::text AS id,
       trigger,
       ...
       cooldown_sec AS "cooldownSec",
       created_at AS "createdAt",
       updated_at AS "updatedAt"`, [s.streamerId, trigger, response, enabled, cooldownSec]);
    res.json({ ok: true, command: ins.rows[0] });
});
mountAliases("post", "/commands", requireAuth, createCommandHandler);
const patchCommandHandler = a(async (req, res) => {
    const s = await resolveBotManageTarget(req);
    if (!s.ok)
        return res.status(s.status).json({ ok: false, error: s.error, ...(s.detail ? { detail: s.detail } : {}) });
    ...
    if ("trigger" in (req.body ?? {}))
        patch.trigger = normTrigger(req.body.trigger);
    ...
    if ("cooldownSec" in (req.body ?? {}))
        patch.cooldownSec = clampInt(req.body.cooldownSec, 0, 3600);
    if ("trigger" in patch && !patch.trigger)
        return res.status(400).json({ ok: false, error: "bad_trigger" });
    if ("trigger" in patch && String(patch.trigger).length > 32)
        return res.status(400).json({ ok: false, error: "trigger_too_long" });
    if ("response" in patch && !patch.response)
        return res.status(400).json({ ok: false, error: "bad_response" });
    if ("response" in patch && String(patch.response).length > 600)
        return res.status(400).json({ ok: false, error: "response_too_long" });
    const cur = await pool.query(`SELECT id, trigger FROM bot_commands WHERE id=$1 AND streamer_id=$2 LIMIT 1`, [id, s.streamerId]);
    if (!cur.rows?.[0])
        return res.status(404).json({ ok: false, error: "not_found" });
    if (patch.trigger) {
        const dup = await pool.query(`SELECT 1 FROM bot_commands WHERE streamer_id=$1 AND lower(trigger)=lower($2) AND id<>$3 LIMIT 1`, [s.streamerId, patch.trigger, id]);
        if (dup.rows?.[0])
            return res.status(409).json({ ok: false, error: "trigger_exists" });
    }
    const upd = await pool.query(`UPDATE bot_commands
     SET
...
const deleteCommandHandler = a(async (req, res) => {
    const s = await resolveBotManageTarget(req);
    if (!s.ok)
        return res.status(s.status).json({ ok: false, error: s.error, ...(s.detail ? { detail: s.detail } : {}) });
    const id = String(req.params.id || "").trim();
...
/* ──────────────────────────────────────────
   AUTOPOSTS
────────────────────────────────────────── */
const listAutopostsHandler = a(async (req, res) => {
    const s = await resolveBotManageTarget(req);
    if (!s.ok)
        return res.status(s.status).json({ ok: false, error: s.error, ...(s.detail ? { detail: s.detail } : {}) });
    const r = await pool.query(`SELECT
...
const createAutopostHandler = a(async (req, res) => {
    const s = await resolveBotManageTarget(req);
    if (!s.ok)
        return res.status(s.status).json({ ok: false, error: s.error, ...(s.detail ? { detail: s.detail } : {}) });
    ...
    const ins = await pool.query(`INSERT INTO bot_autoposts(streamer_id, message, every_sec, enabled)
     VALUES ($1,$2,$3,$4)
     RETURNING
       id::text AS id,
       message,
       ...
       enabled,
       created_at AS "createdAt",
       updated_at AS "updatedAt"`, [s.streamerId, message, Math.floor(everySec), enabled]);
    res.json({ ok: true, autopost: ins.rows[0] });
});
mountAliases("post", "/autoposts", requireAuth, createAutopostHandler);
const patchAutopostHandler = a(async (req, res) => {
    const s = await resolveBotManageTarget(req);
    if (!s.ok)
        return res.status(s.status).json({ ok: false, error: s.error, ...(s.detail ? { detail: s.detail } : {}) });
    const id = String(req.params.id || "").trim();
    ...
    if ("message" in (req.body ?? {}))
        patch.message = String(req.body.message ?? "").replace(/\r/g, "").trim();
    if ("everySec" in (req.body ?? {}))
        patch.everySec = Number(req.body.everySec ?? 0);
    if ("enabled" in (req.body ?? {}))
        patch.enabled = Boolean(req.body.enabled);
    if ("message" in patch && !patch.message)
        return res.status(400).json({ ok: false, error: "bad_message" });
    if ("message" in patch && String(patch.message).length > 600)
        return res.status(400).json({ ok: false, error: "message_too_long" });
    if ("everySec" in patch && (!Number.isFinite(patch.everySec) || patch.everySec < 60)) {
        return res.status(400).json({ ok: false, error: "bad_every_sec" });
    }
    const upd = await pool.query(`UPDATE bot_autoposts
     SET
...
const deleteAutopostHandler = a(async (req, res) => {
    const s = await resolveBotManageTarget(req);
    if (!s.ok)
        return res.status(s.status).json({ ok: false, error: s.error, ...(s.detail ? { detail: s.detail } : {}) });
    const id = String(req.params.id || "").trim();
...
/* ──────────────────────────────────────────
   LOGS
────────────────────────────────────────── */
const listLogsHandler = a(async (req, res) => {
    const s = await resolveBotManageTarget(req);
    if (!s.ok)
        return res.status(s.status).json({ ok: false, error: s.error, ...(s.detail ? { detail: s.detail } : {}) });
...
────────────────────────────────────────── */
const testSendHandler = a(async (req, res) => {
    const s = await resolveBotManageTarget(req);
    if (!s.ok)
        return res.status(s.status).json({ ok: false, error: s.error, ...(s.detail ? { detail: s.detail } : {}) });
    const liveCheck = await assertLiveIfNeeded(!!s.isLive);
    if (!liveCheck.ok)
        return res.status(liveCheck.status).json({ ok: false, error: liveCheck.error });
    ...
    const ins = await pool.query(`INSERT INTO chat_messages (streamer_id, user_id, username, body)
     VALUES ($1,$2,$3,$4)
     RETURNING id, created_at AS "createdAt"`, [s.streamerId, botUserId, botUsername, body]);
    ...
    if (io)
        io.to(`chat:${s.slug}`).emit("chat:message", msg);
