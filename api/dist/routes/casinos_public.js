...
casinosPublicRouter.get("/casinos", a(async (req, res) => {
    ...
    const C = rows.length > 0
        ? rows.reduce((acc, x) => acc + (Number(x.avgRating) || 0), 0) / rows.length
        : 0;
    const podium = [...rows]
        .sort((a, b) => topScore(b.avgRating, b.ratingsCount, C) - topScore(a.avgRating, a.ratingsCount, C))
        .slice(0, 3);
    const watchlist = rows
        .filter((x) => x.watchLevel === "watch" || x.watchLevel === "avoid")
        .sort((a, b) => {
        ...
        .slice(0, 6);
    const casinos = [...rows].sort((a, b) => {
        if (sort === "new")
            return +new Date(b.createdAt) - +new Date(a.createdAt);
        if (sort === "rating")
            return (b.avgRating - a.avgRating) || (b.ratingsCount - a.ratingsCount);
        if (sort === "reviews")
            return (b.ratingsCount - a.ratingsCount) || (b.avgRating - a.avgRating);
        if (sort === "featured") {
...
// GET /casinos/:slug
casinosPublicRouter.get("/casinos/:slug", a(async (req, res) => {
    ...
    const linksRows = await pool.query(`
      ...
      `, [Number(casino.id)]);
    const links = linksRows.rows.map((l) => {
    ...
    const bonusLink = links.find((x) => x.kind === "bonus") ??
        links.find((x) => x.ownerUserId == null) ??
        [...links].sort((a, b) => (a.pinnedRank ?? 999999) - (b.pinnedRank ?? 999999))[0] ??
        null;
    res.json({
        ok: true,
        casino,
...
casinosPublicRouter.get("/casinos/:slug/comments", a(async (req, res) => {
    const slug = String(req.params.slug || "").trim();
    ...
    const params = [Number(casino.id), limit];
    let cursorSql = "";
    if (cursor) {
        params.push(cursor);
        cursorSql = ` AND c.created_at < $${params.length}`;
    }
    const order = sort === "useful"
    ...
    const imagesBy = {};
    if (ids.length) {
        const imgs = await pool.query(`SELECT comment_id::text AS "commentId", url, w, h, size_bytes AS "sizeBytes"
         FROM casino_comment_images
         WHERE comment_id = ANY($1::bigint[])`, [ids.map((x) => Number(x))]);
    ...
    const myBy = {};
    if (me?.id && ids.length) {
        const r = await pool.query(`SELECT comment_id::text AS "commentId", kind
         FROM casino_comment_reactions
         WHERE user_id=$1 AND comment_id = ANY($2::bigint[])`, [Number(me.id), ids.map((x) => Number(x))]);
        for (const rr of r.rows)
            myBy[rr.commentId] = rr.kind;
    }
...
casinosPublicRouter.get("/go/casino/:casinoId/link/:linkId", a(async (req, res) => {
    const casinoId = Number(req.params.casinoId);
    const linkId = Number(req.params.linkId);
    const link = await pool.query(`SELECT id, casino_id, target_url, enabled
       FROM casino_affiliate_links
       WHERE id=$1 AND casino_id=$2
       LIMIT 1`, [linkId, casinoId]);
    const row = link.rows?.[0];
    if (!row || !row.enabled)
        return res.status(404).send("Not found");
    const me = tryGetAuthUser(req);
    await pool.query(`INSERT INTO casino_affiliate_clicks (casino_id, link_id, visitor_user_id, ref)
       VALUES ($1,$2,$3,$4)`, [casinoId, linkId, me?.id ?? null, req.get("referer") ?? null]);
