...
function normStatus(v) {
    ...
    if (s === "published" || s === "hidden" || s === "disabled")
        return s;
    return null;
}
function normWatch(v) {
    const s = String(v || "").trim();
    if (s === "none" || s === "watch" || s === "avoid")
        return s;
    return null;
}
// ✅ GET /admin/casinos?q=
...
adminCasinosRouter.patch("/:id", async (req, res) => {
    ...
    const fields = [];
    if ("name" in patch)
        fields.push({ col: "name", val: String(patch.name || "").trim() });
    if ("slug" in patch)
        fields.push({ col: "slug", val: String(patch.slug || "").trim().toLowerCase() });
    if ("logoUrl" in patch)
        fields.push({ col: "logo_url", val: patch.logoUrl ? String(patch.logoUrl) : null });
    ...
    if ("pros" in patch)
        fields.push({ col: "pros", val: Array.isArray(patch.pros) ? JSON.stringify(patch.pros) : JSON.stringify([]), castJsonb: true });
    if ("cons" in patch)
        fields.push({ col: "cons", val: Array.isArray(patch.cons) ? JSON.stringify(patch.cons) : JSON.stringify([]), castJsonb: true });
    if ("sections" in patch)
        fields.push({ col: "sections", val: Array.isArray(patch.sections) ? JSON.stringify(patch.sections) : JSON.stringify([]), castJsonb: true });
    ...
    if ("teamReview" in patch)
        fields.push({ col: "team_review", val: patch.teamReview ? String(patch.teamReview) : null });
    if ("featuredRank" in patch)
        fields.push({ col: "featured_rank", val: patch.featuredRank === null || patch.featuredRank === "" ? null : Number(patch.featuredRank) });
    if ("watchLevel" in patch) {
        const wl = normWatch(patch.watchLevel);
        if (!wl)
            return res.status(400).json({ ok: false, error: "bad watchLevel" });
        fields.push({ col: "watch_level", val: wl });
    }
    if ("watchReason" in patch)
        fields.push({ col: "watch_reason", val: patch.watchReason ? String(patch.watchReason) : null });
    if ("status" in patch) {
...
adminCasinosRouter.post("/:id/links", async (req, res) => {
    ...
    if (!(kind === "bonus" || kind === "streamer"))
        return res.status(400).json({ ok: false, error: "bad kind" });
    if (!targetUrl)
        return res.status(400).json({ ok: false, error: "targetUrl required" });
    const { rows } = await pool.query(`
...
adminCasinosRouter.patch("/links/:linkId", async (req, res) => {
    ...
    if ("kind" in patch) {
        ...
        if (!(k === "bonus" || k === "streamer"))
            return res.status(400).json({ ok: false, error: "bad kind" });
        fields.push({ col: "kind", val: k });
    }
    if ("label" in patch)
        fields.push({ col: "label", val: patch.label ? String(patch.label) : null });
    if ("targetUrl" in patch)
        fields.push({ col: "target_url", val: patch.targetUrl ? String(patch.targetUrl) : "" });
    if ("enabled" in patch)
        fields.push({ col: "enabled", val: Boolean(patch.enabled) });
    if ("pinnedRank" in patch)
        fields.push({ col: "pinned_rank", val: patch.pinnedRank === null || patch.pinnedRank === "" ? null : Number(patch.pinnedRank) });
    if ("ownerUserId" in patch)
        fields.push({ col: "owner_user_id", val: patch.ownerUserId === null || patch.ownerUserId === "" ? null : Number(patch.ownerUserId) });
    if ("streamerId" in patch)
        fields.push({ col: "streamer_id", val: patch.streamerId === null || patch.streamerId === "" ? null : Number(patch.streamerId) });
    if (fields.length === 0)
        return res.json({ ok: true });
