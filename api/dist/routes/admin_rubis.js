...
adminRubisRouter.get("/admin/users/search", requireAdminKey, async (req, res, next) => {
    try {
        ...
        const { rows } = await pool.query(`SELECT id, username, role, rubis
       FROM users
...
adminRubisRouter.post("/admin/rubis/mint", requireAdminKey, async (req, res, next) => {
    ...
    try {
        ...
        if (weightBp === null)
            return res.status(400).json({ ok: false, error: "bad_weightBp" });
        ...
        // create lot
        const lot = await client.query(`INSERT INTO rubis_lots (user_id, origin, weight_bp, amount_total, amount_remaining, meta)
       VALUES ($1,$2,$3,$4,$4,$5::jsonb)
       RETURNING id`, [userId, origin, weightBp, amount, JSON.stringify({ by: "admin", note: req.body?.note ?? null })]);
        // tx ledger
        const tx = await client.query(`INSERT INTO rubis_tx (
         kind, purpose, status,
       ...
       VALUES ($1, 'user', $2, $3)`, [Number(tx.rows[0].id), userId, amount]);
