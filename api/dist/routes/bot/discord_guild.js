 ...
 */
botDiscordGuildRouter.get("/discord/guild", requireAuth, a(async (req, res) => {
    ...
    const r = await pool.query(`SELECT guild_id, claimed_at, updated_at, config FROM bot_discord_guilds WHERE streamer_id=$1 LIMIT 1`, [streamerId]);
    if (!r.rows?.[0])
        return res.json({ ok: true, guild: null });
    return res.json({
        ok: true,
 ...
 */
botDiscordGuildRouter.post("/discord/unclaim", requireAuth, a(async (req, res) => {
    const userId = Number(req.user?.id || 0);
    if (!userId)
        return res.status(401).json({ ok: false, error: "unauthorized" });
    const s = await pool.query(`SELECT id FROM streamers WHERE user_id=$1 LIMIT 1`, [userId]);
