// api/src/routes/bot/bot_discord_guild.ts
 ...
 * -> renvoie le serveur Discord claim par ce streamer (si existant)
 */
botDiscordGuildRouter.get("/bot/discord/guild", requireAuth, a(async (req, res) => {
    ...
    const r = await pool.query(`
      SELECT guild_id, claimed_at, updated_at, config
      FROM bot_discord_guilds
      WHERE streamer_id=$1
      LIMIT 1
      `, [streamerId]);
    if (!r.rows?.[0])
 ...
 * -> délier le serveur Discord claim par ce streamer
 */
botDiscordGuildRouter.post("/bot/discord/unclaim", requireAuth, a(async (req, res) => {
    const userId = Number(req.user?.id || 0);
    if (!userId)
        return res.status(401).json({ ok: false, error: "unauthorized" });
    const s = await pool.query(`SELECT id FROM streamers WHERE user_id=$1 LIMIT 1`, [userId]);
