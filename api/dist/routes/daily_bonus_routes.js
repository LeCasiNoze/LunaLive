...
import { getDailyBonusState, claimDailyBonusToday, claimDailyBonusMilestone, } from "../services/dailyBonus.js";
...
dailyBonusRoutes.post("/claim-milestone", async (req, res) => {
    ...
    try {
        const r = await claimDailyBonusMilestone(pool, userId, milestone);
        res.json(r);
    }
    catch (e) {
        res.status(400).json({ ok: false, error: String(e?.message || "Erreur") });
