...
supportRouter.post("/support/sub", requireAuth, async (req, res) => {
    ...
    const s = await pool.query(`SELECT id, slug, display_name AS "displayName"
     FROM streamers
    ...
    try {
        ...
        const sub = await client.query(`INSERT INTO streamer_subscriptions (streamer_id, user_id, started_at, expires_at)
       ...
       DO UPDATE SET
         ...
         updated_at = NOW()
       RETURNING expires_at`, [streamerId, userId, qty]);
        await client.query("COMMIT");
        // ✅ Message système dans le chat (après commit)
        const io = req.app.locals.io;
        emitSystemChat(io, String(streamer.slug), formatSubSystemMessage({
            user: username,
