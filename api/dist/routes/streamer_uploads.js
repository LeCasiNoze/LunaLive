...
async function getMyStreamerRow(userId) {
    const r = await pool.query(`SELECT id, slug, offline_bg_path
     FROM streamers
     WHERE user_id=$1
     LIMIT 1`, [userId]);
    return r.rows?.[0] || null;
}
function buildOfflineBgUrl(req, filename) {
    if (!filename)
        return null;
 ...
 */
streamerUploadsRouter.post("/streamer/me/offline-bg", requireAuth, upload.single("image"), async (req, res) => {
 ...
 */
streamerUploadsRouter.delete("/streamer/me/offline-bg", requireAuth, async (req, res) => {
    if (req.user?.role !== "streamer" && req.user?.role !== "admin") {
        return res.status(403).json({ ok: false, error: "forbidden" });
    }
    const me = await getMyStreamerRow(req.user.id);
