...
casinosMeRouter.post(COMMENTS_PATHS, requireAuth, upload.array("images", 3), a(async (req, res) => {
    ...
    for (const f of files) {
        if (!MIME_EXT[f.mimetype])
            return res.status(400).json({ ok: false, error: "bad_image_type" });
    }
    const hasImages = files.length > 0;
    const status = hasImages ? "pending" : "published";
    const ins = await pool.query(`INSERT INTO casino_comments (casino_id, user_id, body, status, has_images)
       VALUES ($1,$2,$3,$4,$5)
    ...
    if (hasImages) {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            await pool.query(`INSERT INTO casino_comment_images (comment_id, url, w, h, size_bytes)
           VALUES ($1,$2,$3,$4,$5)`, [Number(commentId), outRel, w, h, sizeBytes]);
        }
    }
