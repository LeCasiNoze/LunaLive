...
// PUT rating
casinosMeRouter.put(RATING_PATHS, requireAuth, a(async (req, res) => {
    const casinoId = toInt(req.params.casinoId);
    if (casinoId == null)
        return res.status(400).json({ ok: false, error: "bad_casino_id" });
...
casinosMeRouter.post(COMMENTS_PATHS, requireAuth, upload.array("images", 3), a(async (req, res) => {
    const casinoId = toInt(req.params.casinoId);
    if (casinoId == null)
        return res.status(400).json({ ok: false, error: "bad_casino_id" });
    ...
    for (const f of files) {
        ...
        if (!ALLOWED_MIME.has(mt))
            return res.status(400).json({ ok: false, error: "bad_image_type" });
    }
    ...
    try {
        ...
        const ins = await client.query(`INSERT INTO casino_comments (casino_id, user_id, body, status, has_images)
         VALUES ($1,$2,$3,$4,$5)
         RETURNING id::text AS id`, [casinoId, Number(req.user.id), body, status, hasImages]);
        ...
        if (hasImages) {
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                await client.query(`INSERT INTO casino_comment_images (comment_id, url, w, h, size_bytes, mime, bytes)
             VALUES ($1,$2,$3,$4,$5,$6,$7)`, [Number(commentId), outRel, w, h, sizeBytes, mime, outBuf]);
            }
        }
...
// POST reaction
casinosMeRouter.post(REACT_PATHS, requireAuth, a(async (req, res) => {
    const commentId = toInt(req.params.commentId);
    if (commentId == null)
        return res.status(400).json({ ok: false, error: "bad_comment_id" });
