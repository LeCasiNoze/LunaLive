...
function normText(v, max = 3000) {
    const s = String(v ?? "").trim();
    if (!s)
        return "";
    return s.slice(0, max);
}
function safeCategory(kind, v) {
...
reportsRouter.post("/", async (req, res) => {
    try {
        ...
        const kind = kindRaw === "feedback" ? "feedback" : "report";
        ...
        const q = await pool.query(`INSERT INTO reports(
...
/** Admin list */
reportsRouter.get("/admin/list", async (req, res) => {
    try {
        const me = req.user;
        if (!isAdmin(me))
...
reportsRouter.post("/admin/:id", async (req, res) => {
    try {
        const me = req.user;
        if (!isAdmin(me))
            return res.status(403).json({ ok: false, error: "forbidden" });
        ...
        const q = await pool.query(`UPDATE reports
       SET status = COALESCE($2, status),
           admin_notes = COALESCE($3, admin_notes),
           handled_by = $4,
           handled_at = now()
       WHERE id=$1
       RETURNING id, status, admin_notes, handled_by, handled_at`, [id, nextStatus, admin_notes, Number(me?.id || 0) || null]);
