...
adminCasinosSetupRouter.get("/admin/casinos/listings", requireAdminKey, a(async (_req, res) => {
...
adminCasinosSetupRouter.post("/admin/casinos/listings", requireAdminKey, a(async (req, res) => {
    const slug = String(req.body?.slug ?? "").trim().toLowerCase();
    const name = String(req.body?.name ?? "").trim();
    if (!slug || !name)
        return res.status(400).json({ ok: false, error: "slug_name_required" });
...
adminCasinosSetupRouter.patch("/admin/casinos/listings/:id", requireAdminKey, a(async (req, res) => {
    const id = Number(req.params.id);
    if (!Number.isFinite(id))
        return res.status(400).json({ ok: false, error: "bad_id" });
    // pros/cons transform (si présents)
    const body = { ...req.body };
    if (Object.prototype.hasOwnProperty.call(body, "pros"))
        body.pros = JSON.stringify(normalizeStringArray(body.pros));
    if (Object.prototype.hasOwnProperty.call(body, "cons"))
        body.cons = JSON.stringify(normalizeStringArray(body.cons));
    // si watch_level/raison bouge, on update watch_updated_at
    const touchWatch = Object.prototype.hasOwnProperty.call(body, "watch_level") ||
    ...
    const mapped = {};
    if ("slug" in body)
        mapped.slug = body.slug ? String(body.slug).trim().toLowerCase() : null;
    if ("name" in body)
        mapped.name = body.name;
    if ("logoUrl" in body)
        mapped.logo_url = body.logoUrl;
    if ("status" in body)
        mapped.status = body.status;
    if ("featuredRank" in body)
        mapped.featured_rank = body.featuredRank === "" ? null : body.featuredRank;
    if ("bonusHeadline" in body)
        mapped.bonus_headline = body.bonusHeadline;
    if ("description" in body)
        mapped.description = body.description;
    if ("pros" in body)
        mapped.pros = body.pros; // json string
    if ("cons" in body)
        mapped.cons = body.cons; // json string
    if ("teamRating" in body)
        mapped.team_rating = body.teamRating === "" ? null : body.teamRating;
    if ("teamReview" in body)
        mapped.team_review = body.teamReview;
    if ("watchLevel" in body)
        mapped.watch_level = body.watchLevel;
    if ("watchReason" in body)
        mapped.watch_reason = body.watchReason;
...
adminCasinosSetupRouter.get("/admin/casinos/listings/:id/links", requireAdminKey, a(async (req, res) => {
...
adminCasinosSetupRouter.post("/admin/casinos/listings/:id/links", requireAdminKey, a(async (req, res) => {
    const casinoId = Number(req.params.id);
    const kind = String(req.body?.kind ?? "bonus");
    const targetUrl = String(req.body?.targetUrl ?? "").trim();
    if (!targetUrl)
        return res.status(400).json({ ok: false, error: "targetUrl_required" });
    let ownerUserId = null;
    if (kind === "streamer") {
        const streamerSlug = String(req.body?.streamerSlug ?? "").trim();
        if (!streamerSlug)
            return res.status(400).json({ ok: false, error: "streamerSlug_required" });
        ...
        if (!uid)
            return res.status(400).json({ ok: false, error: "streamer_has_no_owner" });
...
adminCasinosSetupRouter.patch("/admin/casinos/links/:linkId", requireAdminKey, a(async (req, res) => {
    const linkId = Number(req.params.linkId);
    const mapped = {};
    if ("label" in req.body)
        mapped.label = req.body.label;
    if ("targetUrl" in req.body)
        mapped.target_url = req.body.targetUrl;
    if ("enabled" in req.body)
        mapped.enabled = req.body.enabled;
    if ("pinnedRank" in req.body)
        mapped.pinned_rank = req.body.pinnedRank === "" ? null : req.body.pinnedRank;
