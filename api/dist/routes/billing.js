...
if (!STRIPE_WEBHOOK_SECRET)
    console.warn("[billing] STRIPE_WEBHOOK_SECRET missing");
if (!PRICE_VIEWER)
...
async function upsertSubscriptionFromStripe(sub, plan, userId) {
...
async function addCouponTx(client, userId, code, delta) {
    await client.query(`
...
async function markOnce(key) {
    ...
    const r = await pool.query(`INSERT INTO billing_idempotency (id) VALUES ($1) ON CONFLICT DO NOTHING RETURNING id`, [key]);
    return (r.rowCount || 0) > 0;
}
async function grantCycleBenefits(userId, plan) {
...
billingRouter.post("/checkout-session", express.json({ limit: "64kb" }), requireAuth, a(async (req, res) => {
    const u = req.user;
    const plan = req.body?.plan;
    if (!isPlan(plan))
        return res.status(400).json({ ok: false, error: "invalid_plan" });
    if (plan === "streamer" && !isStreamerRole(u.role)) {
    ...
    try {
        ...
        if (event.type === "invoice.paid") {
            ...
            const subId = String(inv?.subscription || "") ||
                String(inv?.parent?.subscription_details?.subscription || "") ||
                String(inv?.lines?.data?.[0]?.subscription || "") ||
                "";
            if (!subId)
                return res.json({ received: true });
