...
pushRouter.post("/push/subscribe", requireAuth, async (req, res) => {
    ...
    await pool.query(`INSERT INTO push_subscriptions (user_id, endpoint, p256dh, auth, user_agent)
    VALUES ($1,$2,$3,$4,$5)
    ...
    DO UPDATE SET
        ...
        updated_at=NOW()`, [Number(req.user.id), endpoint, p256dh, auth, ua]);
    res.json({ ok: true });
});
...
pushRouter.post("/push/test", requireAuth, async (req, res) => {
    try {
        ...
        const { rows } = await pool.query(`SELECT id, endpoint, p256dh, auth FROM push_subscriptions WHERE user_id=$1 ORDER BY updated_at DESC`, [uid]);
