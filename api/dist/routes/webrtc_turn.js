...
async function fetchCloudflareIceServers() {
    if (!CF_TURN_TOKEN_ID || !CF_TURN_API_TOKEN)
        return null;
    const url = `https://rtc.live.cloudflare.com/v1/turn/keys/${encodeURIComponent(CF_TURN_TOKEN_ID)}/credentials/generate-ice-servers`;
    let resp;
    try {
    ...
    if (!resp.ok) {
        ...
        console.error("[webrtc_turn] CF API error:", resp.status, txt.slice(0, 500));
        return null;
    }
...
webrtcTurnRouter.get("/me/webrtc/ice-servers", requireAuth, requireFsbAccess, a(async (_req, res) => {
