...
function isActiveSub(expiresAt) {
    if (!expiresAt)
        return false;
...
function emitSystemToChat(io, streamerSlug, body) {
    if (!io)
        return;
 ...
 */
async function autoEquipSubBadgeIfEmpty(userId, streamerSlug) {
    const slugLower = normSlug(streamerSlug);
    if (!slugLower)
        return;
    const badgeCode = `badge_sub_${slugLower}`;
    await pool.query(`INSERT INTO user_equipped_cosmetics (user_id)
     VALUES ($1)
     ON CONFLICT (user_id) DO NOTHING`, [userId]);
    await pool.query(`UPDATE user_equipped_cosmetics
     SET badge_code = $2
     WHERE user_id = $1
       AND (badge_code IS NULL OR badge_code = 'none' OR badge_code = '')`, [userId, badgeCode]);
}
async function getStreamerBySlug(slug) {
    const s = await pool.query(`SELECT id, slug, display_name AS "displayName", user_id AS "ownerUserId", appearance
     FROM streamers
...
async function getSubRow(streamerId, userId) {
...
async function upsertSub(streamerId, userId) {
    const r = await pool.query(`INSERT INTO streamer_subscriptions (streamer_id, user_id, started_at, expires_at, created_at, updated_at)
     ...
     ON CONFLICT (streamer_id, user_id) DO UPDATE
       SET started_at = CASE
           ...
           updated_at = NOW()
     RETURNING expires_at`, [streamerId, userId, SUB_DURATION_DAYS]);
    return r.rows?.[0]?.expires_at ?? null;
}
// version TX de upsertSub (pour claim atomique)
async function upsertSubTx(client, streamerId, userId) {
    const r = await client.query(`INSERT INTO streamer_subscriptions (streamer_id, user_id, started_at, expires_at, created_at, updated_at)
     VALUES ($1,$2,NOW(), NOW() + ($3 * INTERVAL '1 day'), NOW(), NOW())
     ON CONFLICT (streamer_id, user_id) DO UPDATE
       SET started_at = CASE
                         WHEN streamer_subscriptions.expires_at < NOW() THEN NOW()
...
async function autoEquipSubBadgeIfEmptyTx(client, userId, streamerSlug) {
    const slugLower = normSlug(streamerSlug);
    if (!slugLower)
        return;
    const badgeCode = `badge_sub_${slugLower}`;
    await client.query(`INSERT INTO user_equipped_cosmetics (user_id)
     VALUES ($1)
     ON CONFLICT (user_id) DO NOTHING`, [userId]);
    await client.query(`UPDATE user_equipped_cosmetics
     SET badge_code = $2
     WHERE user_id = $1
       AND (badge_code IS NULL OR badge_code = 'none' OR badge_code = '')`, [userId, badgeCode]);
}
// ✅ consume coupon atomique
async function consumeCouponTx(client, userId, code, qtyToConsume) {
    const qty = Math.max(0, Math.floor(Number(qtyToConsume || 0)));
    if (qty <= 0)
        return { ok: true, consumed: 0 };
    const cur = await client.query(`SELECT qty
     FROM user_coupons
     WHERE user_id=$1 AND code=$2
     FOR UPDATE`, [userId, code]);
    const have = Number(cur.rows?.[0]?.qty ?? 0);
    if (have < qty) {
        return { ok: false, error: "not_enough_tickets", have };
    }
    const upd = await client.query(`UPDATE user_coupons
     SET qty = qty - $3,
         updated_at = NOW()
     WHERE user_id=$1 AND code=$2
     RETURNING qty`, [userId, code, qty]);
...
subscriptionsRouter.post("/streamers/:slug/subscribe", requireAuth, async (req, res) => {
    ...
    const viewerUsername = String(req.user?.username || "");
    ...
    try {
        ...
        const tpl = (streamer.appearance?.chat?.subMessageTemplate && String(streamer.appearance.chat.subMessageTemplate)) ||
        ...
        emitSystemChat(io, streamer.slug, formatSubSystemMessage({
            user: viewerUsername || "Quelqu’un",
            streamer: streamer.displayName || streamer.slug,
            months: 1,
            origin: "self",
...
subscriptionsRouter.post("/streamers/:slug/gift-sub", requireAuth, async (req, res) => {
    ...
    const gifterUsername = String(req.user?.username || "");
    ...
    try {
        ...
        const newBalance = Number(u.rows?.[0]?.rubis ?? 0);
        const tpl = (streamer.appearance?.chat?.giftSubMessageTemplate && String(streamer.appearance.chat.giftSubMessageTemplate)) ||
...
subscriptionsRouter.post("/streamers/:slug/gift-subs", requireAuth, async (req, res) => {
    ...
    const useTicketsRaw = Number(req.body?.useTickets ?? 0);
    ...
    try {
        ...
        // crée le pool
        const ins = await client.query(`INSERT INTO sub_gift_pools (streamer_id, gifter_user_id, total, remaining, expires_at, meta)
       ...
       RETURNING id, remaining, expires_at`, [
        ...
        const newBalance = Number(u.rows?.[0]?.rubis ?? 0);
        const io = req.app.locals.io;
        emitSystemChat(io, streamer.slug, `🎁 ${gifterUsername || "Quelqu’un"} a offert ${count} sub${count > 1 ? "s" : ""} à la commu !`);
        return res.json({
            ok: true,
            newBalance,
            usedTickets,
...
subscriptionsRouter.get("/streamers/:slug/gift-subs/status", requireAuth, async (req, res) => {
    ...
    const q = await pool.query(`SELECT COALESCE(SUM(remaining),0)::int AS remaining,
            MIN(expires_at) AS next_expires_at
     FROM sub_gift_pools
     WHERE streamer_id=$1 AND ${poolActiveWhere()}`, [streamer.id]);
...
subscriptionsRouter.post("/streamers/:slug/gift-subs/claim", requireAuth, async (req, res) => {
    ...
    try {
        await client.query("BEGIN");
        const pick = await client.query(`SELECT id
       FROM sub_gift_pools
       ...
       LIMIT 1`, [streamer.id]);
        ...
        const claim = await client.query(`INSERT INTO sub_gift_claims (streamer_id, pool_id, user_id)
       VALUES ($1,$2,$3)
       ON CONFLICT (streamer_id, user_id) DO NOTHING
       RETURNING id`, [streamer.id, poolId, userId]);
        if (!claim.rows?.[0]?.id) {
            await client.query("ROLLBACK");
            return res.status(400).json({ ok: false, error: "already_claimed" });
        }
        const upd = await client.query(`UPDATE sub_gift_pools
       SET remaining = remaining - 1
       WHERE id=$1 AND remaining > 0
       RETURNING remaining, expires_at`, [poolId]);
        if (!upd.rows?.[0]) {
            await client.query("ROLLBACK");
