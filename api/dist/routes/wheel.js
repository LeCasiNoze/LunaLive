...
wheelRouter.get("/wheel/me", requireAuth, async (req, res) => {
    ...
    const check = await pool.query(`SELECT 1 FROM daily_wheel_spins WHERE user_id=$1 AND day=$2::date LIMIT 1`, [Number(req.user.id), day]);
...
wheelRouter.post("/wheel/spin", requireAuth, async (req, res) => {
    ...
    try {
        ...
        if (!god) {
            const exists = await client.query(`SELECT 1 FROM daily_wheel_spins WHERE user_id=$1 AND day=$2::date LIMIT 1`, [userId, day]);
        ...
        const tx = await client.query(`INSERT INTO rubis_tx (kind, purpose, status, to_user_id, amount, meta)
       VALUES ('mint','wheel_daily','succeeded',$1,$2,$3::jsonb)
       RETURNING id`, [userId, reward, JSON.stringify(txMeta)]);
        const txId = Number(tx.rows[0].id);
        await client.query(`INSERT INTO rubis_tx_entries (tx_id, entity, user_id, delta)
       VALUES ($1,'user',$2,$3)`, [txId, userId, reward]);
       ...
       VALUES ($1,'wheel_daily',$2,$3,$3,$4::jsonb)`, [userId, weightBp, reward, JSON.stringify(lotMeta)]);
        const u = await client.query(`UPDATE users SET rubis = rubis + $1 WHERE id=$2 RETURNING id, username, rubis`, [reward, userId]);
        if (!god) {
            await client.query(`INSERT INTO daily_wheel_spins (user_id, day, segment_index, reward_rubis, tx_id)
         VALUES ($1,$2::date,$3,$4,$5)`, [userId, day, segmentIndex, reward, txId]);
        }
