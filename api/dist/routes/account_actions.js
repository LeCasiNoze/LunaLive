...
async function getUserEmail(userId) {
    const { rows } = await pool.query(`SELECT email, email_verified
     FROM users
     WHERE id=$1
     LIMIT 1`, [userId]);
    return {
...
async function invalidatePreviousCodesByUser(userId, kind) {
    await pool.query(`UPDATE account_action_codes
     SET consumed_at=NOW()
     WHERE user_id=$1 AND kind=$2 AND consumed_at IS NULL`, [userId, String(kind)]);
}
async function invalidatePreviousCodesByEmail(email, kind) {
    await pool.query(`UPDATE account_action_codes
     SET consumed_at=NOW()
     WHERE lower(email)=lower($1) AND kind=$2 AND consumed_at IS NULL`, [String(email || "").trim(), String(kind)]);
}
async function insertCode(p) {
    ...
    await pool.query(`INSERT INTO account_action_codes (user_id, email, kind, code_hash, expires_at, created_ip)
     VALUES ($1,$2,$3,$4,$5,$6)`, [p.userId ?? null, p.email ?? null, p.kind, codeHash, expiresAt, p.ip ?? null]);
    return { minutes, expiresAt };
}
async function getLatestCodeForUserTx(client, userId, kind) {
    const MAX_ATTEMPTS = 6;
    const { rows } = await client.query(`SELECT id, code_hash, attempts, expires_at
     FROM account_action_codes
     WHERE user_id=$1 AND kind=$2 AND consumed_at IS NULL
     ORDER BY created_at DESC
     LIMIT 1
     FOR UPDATE`, [userId, String(kind)]);
    const row = rows[0];
    if (!row)
...
async function verifyConsumeCodeByEmail(email, kind, code) {
    const MAX_ATTEMPTS = 6;
    const { rows } = await pool.query(`SELECT id, code_hash, attempts, expires_at
     FROM account_action_codes
     WHERE lower(email)=lower($1) AND kind=$2 AND consumed_at IS NULL
     ORDER BY created_at DESC
     LIMIT 1`, [String(email || "").trim(), String(kind)]);
    const row = rows[0];
    if (!row)
        throw new Error("no_code");
    if (Number(row.attempts || 0) >= MAX_ATTEMPTS)
...
───────────────────────────────────────────── */
accountActionsRouter.post("/me/rename/request-code", requireAuth, a(async (req, res) => {
    const userId = Number(req.user.id);
    const { email, emailVerified } = await getUserEmail(userId);
    if (!email)
        return res.status(400).json({ ok: false, error: "email_missing" });
    if (!emailVerified)
...
accountActionsRouter.post("/me/rename/confirm", requireAuth, a(async (req, res) => {
    const userId = Number(req.user.id);
    ...
    try {
        ...
        if (msg === "insufficient_rubis")
            return res.status(400).json({ ok: false, error: "insufficient_rubis" });
...
───────────────────────────────────────────── */
accountActionsRouter.post("/me/password/request-code", requireAuth, a(async (req, res) => {
    const userId = Number(req.user.id);
    const { email, emailVerified } = await getUserEmail(userId);
    if (!email)
        return res.status(400).json({ ok: false, error: "email_missing" });
...
accountActionsRouter.post("/me/password/confirm", requireAuth, a(async (req, res) => {
    const userId = Number(req.user.id);
    const code = String(req.body.code || "").trim();
...
───────────────────────────────────────────── */
accountActionsRouter.post("/auth/forgot/request-code", a(async (req, res) => {
    const email = String(req.body.email || "").trim();
    const IS_DEV = (process.env.NODE_ENV || "development") !== "production";
    const neutralOk = (devCode) => res.json(devCode ? { ok: true, devCode } : { ok: true });
    if (!email)
        return neutralOk();
    const { rows } = await pool.query(`SELECT id, email
       FROM users
    ...
    try {
        ...
        if (IS_DEV)
            return neutralOk(code);
        return neutralOk();
    }
    return neutralOk();
}));
accountActionsRouter.post("/auth/forgot/confirm", a(async (req, res) => {
    const email = String(req.body.email || "").trim();
    const code = String(req.body.code || "").trim();
    const newPassword = String(req.body.newPassword || "");
    if (!email)
    ...
    const { rows } = await pool.query(`SELECT id, username, rubis, role
       FROM users
       WHERE lower(email)=lower($1)
       LIMIT 1`, [email]);
    const u = rows[0];
