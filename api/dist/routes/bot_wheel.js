...
async function requireStreamer(req, res, next) {
    ...
    if (!streamerId)
        return res.status(403).json({ ok: false, error: "not_streamer" });
...
async function sendBotChatMessage(streamerId, bodyRaw) {
    ...
    const s = await pool.query(`SELECT slug, appearance, is_live AS "isLive"
     FROM streamers
     WHERE id=$1
     LIMIT 1`, [streamerId]);
    const meta = s.rows?.[0];
    ...
    const io = botWheelRouter.app?.locals?.io; // fallback si appelé autrement
...
botWheelRouter.get("/state", requireStreamer, async (req, res) => {
    const streamerId = Number(req.streamerId);
    await ensureCfg(streamerId);
    const cfgR = await pool.query(`SELECT enroll_open
     FROM bot_wheel_cfg
     WHERE streamer_id=$1
     LIMIT 1`, [streamerId]);
    const entriesR = await pool.query(`SELECT username
     FROM bot_wheel_entries
     WHERE streamer_id=$1
     ORDER BY created_at ASC, id ASC`, [streamerId]);
    res.json({
        ok: true,
        cfg: { enroll_open: Boolean(cfgR.rows?.[0]?.enroll_open) },
        entries: entriesR.rows.map((r) => ({ username: String(r.username) })),
    });
});
// POST /me/bot/bot_wheel/enroll { open: boolean }
botWheelRouter.post("/enroll", requireStreamer, express.json(), async (req, res) => {
    const streamerId = Number(req.streamerId);
    await ensureCfg(streamerId);
    const open = Boolean(req.body?.open);
    // read previous state
    const prevR = await pool.query(`SELECT enroll_open
     FROM bot_wheel_cfg
     WHERE streamer_id=$1
     LIMIT 1`, [streamerId]);
    const prevOpen = Boolean(prevR.rows?.[0]?.enroll_open);
    await pool.query(`UPDATE bot_wheel_cfg
     SET enroll_open=$2, updated_at=NOW()
     WHERE streamer_id=$1`, [streamerId, open]);
    ...
    if (prevOpen !== open) {
        ...
        if (out?.slug && out?.msg) {
            ...
            if (io)
                io.to(`chat:${out.slug}`).emit("chat:message", out.msg);
        }
    }
    res.json({ ok: true });
});
// POST /me/bot/bot_wheel/add { username }
botWheelRouter.post("/add", requireStreamer, express.json(), async (req, res) => {
    const streamerId = Number(req.streamerId);
    ...
    await ensureCfg(streamerId);
    await pool.query(`INSERT INTO bot_wheel_entries(streamer_id, username, username_lc)
     VALUES ($1, $2, $3)
     ON CONFLICT (streamer_id, username_lc) DO NOTHING`, [streamerId, username, usernameLc]);
    res.json({ ok: true });
});
// POST /me/bot/bot_wheel/remove { username }
botWheelRouter.post("/remove", requireStreamer, express.json(), async (req, res) => {
    const streamerId = Number(req.streamerId);
    const username = normName(req.body?.username);
    const usernameLc = normKey(username);
    if (!username)
        return res.status(400).json({ ok: false, error: "username_required" });
    await pool.query(`DELETE FROM bot_wheel_entries
     WHERE streamer_id=$1 AND username_lc=$2`, [streamerId, usernameLc]);
    res.json({ ok: true });
});
// POST /me/bot/bot_wheel/clear
botWheelRouter.post("/clear", requireStreamer, async (req, res) => {
    const streamerId = Number(req.streamerId);
    await pool.query(`DELETE FROM bot_wheel_entries WHERE streamer_id=$1`, [streamerId]);
    res.json({ ok: true });
});
// POST /me/bot/bot_wheel/draw
botWheelRouter.post("/draw", requireStreamer, async (req, res) => {
    const streamerId = Number(req.streamerId);
    const r = await pool.query(`SELECT id, username
     FROM bot_wheel_entries
     WHERE streamer_id=$1
     ORDER BY created_at ASC, id ASC`, [streamerId]);
    const list = r.rows.map((x) => ({ id: Number(x.id), username: String(x.username) }));
    if (!list.length)
        return res.json({ ok: true, winner: null, index: null });
    const index = Math.floor(Math.random() * list.length);
    const winner = list[index]?.username ?? null;
    res.json({ ok: true, winner, index });
});
// GET /me/bot/bot_wheel/public?slug=...
botWheelRouter.get("/public", async (req, res) => {
    const slug = String(req.query?.slug || "").trim();
    if (!slug)
        return res.status(400).json({ ok: false, error: "bad_slug" });
    const metaR = await pool.query(`SELECT id
     FROM streamers
    ...
    const countR = await pool.query(`SELECT COUNT(*)::int AS n
     FROM bot_wheel_entries
    ...
    if (u?.id) {
        ...
        try {
            const jr = await pool.query(`SELECT 1 FROM bot_wheel_entries WHERE streamer_id=$1 AND user_id=$2 LIMIT 1`, [streamerId, userId]);
            joined = !!jr.rows?.[0];
        }
        catch {
            const usernameLc = String(u.username || "").trim().toLowerCase();
            if (usernameLc) {
                const jr = await pool.query(`SELECT 1 FROM bot_wheel_entries WHERE streamer_id=$1 AND username_lc=$2 LIMIT 1`, [streamerId, usernameLc]);
                joined = !!jr.rows?.[0];
            }
        }
    }
...
botWheelRouter.post("/join", requireAuth, express.json(), async (req, res) => {
    ...
    let already = false;
    try {
        const ins = await pool.query(`INSERT INTO bot_wheel_entries(streamer_id, user_id, username, username_lc)
       VALUES ($1,$2,$3,$4)
       ON CONFLICT (streamer_id, user_id) DO NOTHING
       RETURNING id`, [streamerId, userId, username, usernameLc]);
        joined = !!ins.rows?.[0];
        already = !joined;
    }
    catch {
        const ins = await pool.query(`INSERT INTO bot_wheel_entries(streamer_id, username, username_lc)
       VALUES ($1,$2,$3)
       ON CONFLICT (streamer_id, username_lc) DO NOTHING
       RETURNING id`, [streamerId, username, usernameLc]);
        joined = !!ins.rows?.[0];
        already = !joined;
    }
