...
meProfileRouter.get("/me/stats", requireAuth, async (req, res) => {
    ...
    const [followingQ, chatQ, watchQ, topWatchQ, topMsgQ, activityHourQ, activityDowQ, earnedQ, spentQ, supportQ, burnQ, wheelQ, bonusQ, achQ, entQ, chestQ, giftsQ,] = await Promise.all([
        pool.query(`SELECT COUNT(*)::int AS n FROM streamer_follows WHERE user_id=$1`, [userId]),
        pool.query(`SELECT COUNT(*)::int AS n FROM chat_messages WHERE user_id=$1 AND deleted_at IS NULL`, [userId]),
    ...
    return res.json({
        ...
        topStreamerByWatch: topWatch
            ...
            : null,
        topStreamersByWatch: (topWatchQ.rows ?? []).map((r) => ({
            slug: r.slug,
