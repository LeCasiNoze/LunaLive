...
import { addCall, deleteCallById, getCallsSettings, reorderCalls, setCallBet, setCallPay, setCallBounty, } from "../calls/queue.js";
function num(v) {
...
function scoreName(q, name) {
    ...
    if (!qParts.length || !nParts.length)
        return 0;
    let hit = 0;
    for (const p of qParts) {
        if (nParts.some((x) => x.startsWith(p) || x.includes(p)))
            hit++;
    }
...
async function getBetDefault(userId) {
...
async function setBetDefault(userId, bet) {
    await ensureSession(userId);
    await pool.query(`UPDATE hunt_sessions SET bet_default=$2, updated_at=NOW() WHERE user_id=$1`, [userId, bet]);
}
async function getMyStreamerMeta(userId) {
    const r = await pool.query(`SELECT id, user_id AS "ownerUserId" FROM streamers WHERE user_id=$1 LIMIT 1`, [userId]);
    const row = r.rows?.[0];
    if (!row)
        return null;
    return { streamerId: Number(row.id), ownerUserId: Number(row.ownerUserId) };
}
async function isSyncActiveForUser(userId) {
...
async function getState(userId) {
    ...
    const itemsQ = await pool.query(`SELECT id, pos, name, provider, image_url, bet, pay, bounty, caller
     FROM hunt_session_items
     WHERE user_id=$1
     ORDER BY pos ASC, id ASC`, [userId]);
...
async function writeSessionMeta(userId, patch) {
    ...
    await pool.query(`UPDATE hunt_sessions
     SET phase=$2, opened=$3, start=$4, archive_id=$5, updated_at=NOW()
     WHERE user_id=$1`, [userId, phase, opened, start, archiveId]);
}
async function suggestSlots(q, limit = 12) {
    const qq = String(q || "").trim();
    if (qq.length < 2)
        return [];
    const lim = clamp(Number(limit || 12), 1, 20);
    const raw = await searchSlotsCatalog(pool, qq, lim);
    const scored = (raw || [])
        .map((it) => ({
        name: String(it?.name || ""),
...
async function requireStreamerControl(req, res, slug) {
...
async function getHeadCall(streamerId) {
...
async function getFirstUnpaidOpenItem(streamerId) {
    const r = await pool.query(`
    SELECT
      q.id::text AS id,
      q.slot_name AS name,
...
/* ──────────────────────────────────────────
   ...
   ────────────────────────────────────────── */
function registerCallsCompat(prefix) {
    // STATE
    hunt2Router.get(`${prefix}/:slug/hunt/state`, requireAuth, async (req, res) => {
        try {
    ...
    // OPEN
    hunt2Router.post(`${prefix}/:slug/hunt/open`, requireAuth, async (req, res) => {
        try {
            const slug = String(req.params.slug || "");
            const meta = await requireStreamerControl(req, res, slug);
            if (!meta)
            ...
            if (!(start > 0))
                return res.status(400).json({ ok: false, error: "start_required" });
            if (!st.items?.length)
            ...
            if (!hasBonus)
                return res.status(400).json({ ok: false, error: "no_bonus_drops" });
    ...
    // PASS
    hunt2Router.post(`${prefix}/:slug/hunt/pass`, requireAuth, async (req, res) => {
        try {
            const slug = String(req.params.slug || "");
            const meta = await requireStreamerControl(req, res, slug);
            if (!meta)
    ...
    hunt2Router.post(`${prefix}/:slug/hunt/bonus`, requireAuth, async (req, res) => {
        try {
            const slug = String(req.params.slug || "");
            const meta = await requireStreamerControl(req, res, slug);
            if (!meta)
            ...
            if (!(bet > 0))
                return res.status(400).json({ ok: false, error: "no_bet_set" });
            const head = await getHeadCall(meta.streamerId);
            if (!head?.id)
    ...
    // PAY
    hunt2Router.post(`${prefix}/:slug/hunt/pay`, requireAuth, async (req, res) => {
        try {
            const slug = String(req.params.slug || "");
            const meta = await requireStreamerControl(req, res, slug);
            if (!meta)
    ...
    // START
    hunt2Router.post(`${prefix}/:slug/hunt/start`, requireAuth, async (req, res) => {
        try {
            const slug = String(req.params.slug || "");
            const meta = await requireStreamerControl(req, res, slug);
            if (!meta)
    ...
    // BET DEFAULT
    hunt2Router.post(`${prefix}/:slug/hunt/bet`, requireAuth, async (req, res) => {
        try {
            const slug = String(req.params.slug || "");
            const meta = await requireStreamerControl(req, res, slug);
            if (!meta)
    ...
    // RESET
    hunt2Router.post(`${prefix}/:slug/hunt/reset`, requireAuth, async (req, res) => {
        try {
            const slug = String(req.params.slug || "");
            const meta = await requireStreamerControl(req, res, slug);
            if (!meta)
         ...
         WHERE user_id=$1`, [meta.ownerUserId]);
            const payload = await buildCallsCompatPayload(meta);
            return res.json(payload);
        }
        catch (e) {
...
hunt2Router.get("/api/hunt2/share/state", async (req, res) => {
    try {
        ...
        const t = await pool.query(`SELECT token, user_id, revoked_at, expires_at
...
hunt2Router.post("/api/hunt2/add", async (req, res) => {
    try {
        ...
        if (sync.ok) {
            ...
            if (!add.ok)
                return res.status(400).json({ ok: false, error: add.error });
            return res.json({ ok: true, id: add.item.id });
        }
        const ins = await pool.query(`INSERT INTO hunt_session_items(user_id, pos, name, provider, image_url)
...
// reorder
hunt2Router.post("/api/hunt2/reorder", express.json(), async (req, res) => {
    try {
        const userId = Number(req.user?.id);
        if (!userId)
        ...
        const tasks = ids.map((id, i) => pool.query(`UPDATE hunt_session_items SET pos=$3 WHERE user_id=$1 AND id=$2`, [userId, id, i]));
...
hunt2Router.get("/api/hunt2/my-hunts", async (req, res) => {
    try {
        const userId = Number(req.user.id);
        await ensureSchema();
        const r = await pool.query(`SELECT id, created_at, title, start, total_pay, items_count, snapshot
       FROM hunt_archives
       WHERE user_id=$1
...
hunt2Router.post("/api/hunt2/load", async (req, res) => {
    try {
        ...
        for (let i = 0; i < items.length; i++) {
            const it = items[i];
            await pool.query(`INSERT INTO hunt_session_items(user_id, pos, name, provider, image_url, bet, pay, bounty, caller)
...
hunt2Router.post("/api/hunt2/save", async (req, res) => {
    try {
        ...
        const ins = await pool.query(`INSERT INTO hunt_archives(user_id, title, start, total_pay, items_count, snapshot)
       VALUES($1,$2,$3,$4,$5,$6)
       RETURNING id`, [userId, title, start || null, totalPay, itemsCount, snapshot]);
