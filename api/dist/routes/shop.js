...
async function spendRubisSink(userId, amount) {
    if (amount <= 0)
    ...
    try {
        ...
        // lock user row
        const u = await client.query(`SELECT rubis FROM users WHERE id=$1 FOR UPDATE`, [userId]);
        ...
        const lotsTable = await (async () => {
            ...
            for (const name of candidates) {
                const rr = await client.query(`SELECT to_regclass($1) AS reg`, [`public.${name}`]);
                if (rr.rows?.[0]?.reg)
                    return name;
            }
            return null;
...
function isBuyableShopItem(it) {
    ...
    const pre = typeof it.pricePrestige === "number" && Number.isFinite(it.pricePrestige) && it.pricePrestige > 0;
...
function shopSort(a, b) {
    ...
    const ap = typeof a.pricePrestige === "number" ? Number(a.pricePrestige) : Number.POSITIVE_INFINITY;
    const bp = typeof b.pricePrestige === "number" ? Number(b.pricePrestige) : Number.POSITIVE_INFINITY;
...
shopRouter.get("/shop/cosmetics", requireAuth, a(async (req, res) => {
    ...
    let equipped = { username: null, badge: null, title: null, frame: null, hat: null };
    if (await tableExists("user_equipped_cosmetics")) {
        const eq = await pool.query(`SELECT username_code, badge_code, title_code, frame_code, hat_code
...
shopRouter.post("/shop/cosmetics/buy", requireAuth, a(async (req, res) => {
    ...
    if (!isBuyableShopItem(it))
        return res.status(400).json({ ok: false, error: "not_shop_item" });
    // already owned ?
    ...
    if (ownedQ.rows?.[0]) {
        const u = await pool.query(`SELECT id, username, rubis FROM users WHERE id=$1 LIMIT 1`, [userId]);
        const ent = await pool.query(`SELECT kind, code FROM user_entitlements WHERE user_id=$1`, [userId]);
        const availablePrestige = await getTokenAmount(userId, PRESTIGE_TOKEN);
        return res.json({
            ok: true,
    ...
    const pricePrestige = typeof it.pricePrestige === "number" ? Number(it.pricePrestige) : null;
    ...
    try {
        ...
        if (msg === "insufficient_tokens")
            return res.status(400).json({ ok: false, error: "insufficient_prestige" });
        if (msg === "bad_amount")
            return res.status(400).json({ ok: false, error: "bad_amount" });
        if (msg === "tokens_unavailable")
            return res.status(500).json({ ok: false, error: "tokens_unavailable" });
        return res.status(500).json({ ok: false, error: "buy_failed" });
    }
    // grant entitlement
    await pool.query(`INSERT INTO user_entitlements (user_id, kind, code)
       VALUES ($1,$2,$3)
       ON CONFLICT DO NOTHING`, [userId, kind, code]);
    const u = await pool.query(`SELECT id, username, rubis FROM users WHERE id=$1 LIMIT 1`, [userId]);
    const ent = await pool.query(`SELECT kind, code FROM user_entitlements WHERE user_id=$1`, [userId]);
    const availablePrestige = await getTokenAmount(userId, PRESTIGE_TOKEN);
    res.json({
