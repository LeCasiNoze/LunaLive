...
adminXpRouter.get("/admin/xp/leaderboard", requireAdminKey, async (req, res, next) => {
    try {
        ...
        const r = await pool.query(`SELECT id, username, role, xp::bigint AS xp, last_login_at
         FROM users
...
adminXpRouter.post("/admin/xp/add", requireAdminKey, async (req, res, next) => {
    ...
    try {
        ...
        if (delta > 0) {
         ...
         VALUES ($1, $2, 'admin_grant', $3, $4::jsonb)`, [userId, newXp - oldXp, reason || "admin_remove", JSON.stringify({ by: "admin" })]);
            await client.query("COMMIT");
            return res.json({
                ok: true,
                delta: newXp - oldXp,
...
adminXpRouter.post("/admin/xp/set", requireAdminKey, async (req, res, next) => {
    ...
    try {
        ...
        await client.query(`INSERT INTO xp_events (user_id, delta, source, reason, meta)
       VALUES ($1, $2, 'admin_grant', $3, $4::jsonb)`, [userId, value - oldXp, reason || "admin_set", JSON.stringify({ by: "admin", target: value, oldXp })]);
        await client.query("COMMIT");
        return res.json({
            ok: true,
            delta: value - oldXp,
...
adminXpRouter.get("/admin/xp/events", requireAdminKey, async (req, res, next) => {
    try {
        ...
        const r = await pool.query(`SELECT e.id, e.user_id, u.username, e.delta, e.source, e.reason, e.meta, e.created_at
         FROM xp_events e
         LEFT JOIN users u ON u.id = e.user_id
         ${where}
         ORDER BY e.id DESC
         LIMIT $${where ? 2 : 1}`, params);
        return res.json({
            ok: true,
...
adminXpRouter.get("/admin/xp/balance", requireAdminKey, async (_req, res, next) => {
    ...
    `);
        ...
        return res.json({
            ...
            bySource30d: bySource.rows.map((r) => ({
