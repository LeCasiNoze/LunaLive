...
fsbTodosRouter.get("/fsb/todos", a(async (req, res) => {
    ...
    const r = await pool.query(`SELECT id, message, attachment_url, attachment_name, created_by_name,
...
fsbTodosRouter.post("/fsb/todos", a(async (req, res) => {
    ...
    const r = await pool.query(`INSERT INTO fsb_todos (message, attachment_url, attachment_name, created_by_user_id, created_by_name)
     VALUES ($1, $2, $3, $4, $5)
     RETURNING id, message, attachment_url, attachment_name, created_by_name, status, created_at, completed_at`, [message, attachmentUrl, attachmentName, userId, userName]);
...
fsbTodosRouter.patch("/fsb/todos/:id", a(async (req, res) => {
    ...
    const r = await pool.query(`UPDATE fsb_todos
     SET status = $1,
         completed_at = CASE WHEN $1 = 'done' THEN NOW() ELSE NULL END
     WHERE id = $2
     RETURNING id, message, attachment_url, attachment_name, created_by_name, status, created_at, completed_at`, [next, id]);
    if (r.rowCount === 0)
