...
export function defaultAvatarPath(userId) {
...
async function getUserWithWalletBits(userId) {
...
authRouter.post("/auth/register/verify", a(async (req, res) => {
    ...
    try {
        ...
        if (refSlug) {
            ...
            VALUES ($1,$2,'signup')
            ON CONFLICT (user_id) DO NOTHING`, [userId, streamerId]);
            }
        }
    }
    catch (e) {
        console.log("[auth/register/verify] referral lock failed", e?.message || e);
    }
