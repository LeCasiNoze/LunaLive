...
authRouter.post("/auth/register", a(async (req, res) => {
    ...
    const u2 = await pool.query(`SELECT 1 FROM pending_registrations WHERE lower(username)=lower($1) OR lower(email)=lower($2) LIMIT 1`, [username, email]);
    if (u2.rows[0])
    ...
    try {
        await pool.query(`INSERT INTO pending_registrations (username, email, password_hash, code_hash, expires_at, created_ip)
         VALUES ($1,$2,$3,$4,$5,$6)`, [username, email, passwordHash, codeHash, expiresAt, req.ip]);
    }
    catch {
        return res.status(400).json({ ok: false, error: "already_pending" });
    }
    ...
    try {
        ...
        await pool.query(`DELETE FROM pending_registrations
         WHERE lower(username)=lower($1) OR lower(email)=lower($2)`, [username, email]);
        return res.status(500).json({ ok: false, error: "mail_failed" });
    }
    res.json({ ok: true, needsVerify: true });
}));
authRouter.post("/auth/register/verify", a(async (req, res) => {
    const username = String(req.body.username || "").trim();
    ...
    const { rows } = await pool.query(`SELECT id, username, email, password_hash, code_hash, expires_at
    ...
    try {
        created = await pool.query(`INSERT INTO users (username, email, email_verified, password_hash, role, rubis, created_ip, last_login_ip, last_login_at)
         ...
         RETURNING id, username, rubis, role, email_verified AS "emailVerified"`, [p.username, p.email, p.password_hash, req.ip]);
    }
    catch {
        await pool.query(`DELETE FROM pending_registrations WHERE id=$1`, [p.id]);
        return res.status(400).json({ ok: false, error: "already_used" });
...
authRouter.post("/auth/register/resend", a(async (req, res) => {
    const username = String(req.body.username || "").trim();
    if (!username)
        return res.status(400).json({ ok: false, error: "username_required" });
    await pool.query(`DELETE FROM pending_registrations WHERE expires_at < NOW()`);
    const { rows } = await pool.query(`SELECT id, email FROM pending_registrations
       WHERE lower(username)=lower($1)
       LIMIT 1`, [username]);
    const p = rows[0];
    if (!p)
    ...
    await pool.query(`UPDATE pending_registrations
       ...
       WHERE id=$3`, [codeHash, expiresAt, p.id]);
    const NODE_ENV = process.env.NODE_ENV || "development";
    const IS_DEV = NODE_ENV !== "production";
    try {
...
authRouter.post("/auth/login", a(async (req, res) => {
    const username = String(req.body.username || "").trim();
    const password = String(req.body.password || "");
    const { rows } = await pool.query(`SELECT id, username, rubis, role, password_hash, email_verified
       FROM users
       WHERE lower(username) = lower($1)
       LIMIT 1`, [username]);
    const u = rows[0];
...
authRouter.get("/me", requireAuth, a(async (req, res) => {
    const { rows } = await pool.query(`SELECT id, username, rubis, role, email_verified AS "emailVerified"
       FROM users
       WHERE id = $1
       LIMIT 1`, [req.user.id]);
    if (!rows[0])
