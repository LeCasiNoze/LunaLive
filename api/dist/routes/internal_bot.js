...
// --------------------  
internalBotRouter.post("/internal/bot/clip/create", express.json(), async (req, res) => {
    const body = req.body || {};
...
// --------------------
internalBotRouter.patch("/internal/bot/clip/:id", express.json(), async (req, res) => {
    ...
    try {
        const r = await pool.query(`UPDATE bot_clips
         ...
         RETURNING id, title`, [clipId, newTitle.slice(0, 200)]);
        if (!r.rowCount) {
...
// --------------------
internalBotRouter.delete("/internal/bot/clip/:id", async (req, res) => {
    if (!requireBotKey(req, res))
        return;
    const clipId = Number(req.params.id || 0);
    if (!clipId) {
...
// --------------------
internalBotRouter.post("/internal/bot/chat/inject", express.json(), async (req, res) => {
    if (!requireBotKey(req, res))
        return;
    const body = req.body || {};
    ...
    const ins = await pool.query(`INSERT INTO chat_messages(streamer_slug, user_id, username, body, created_at)
       VALUES($1, $2, $3, $4, NOW())
       RETURNING id, created_at AS "createdAt"`, [slug, botUserId, botUsername, botMessage]);
...
internalBotRouter.post("/internal/bot/streamer/settings", express.json(), async (req, res) => {
    ...
    try {
        await pool.query(`INSERT INTO bot_settings(streamer_id, enabled, prefix, live_only)
         VALUES($1, $2, $3, $4)
         ON CONFLICT(streamer_id) DO UPDATE SET
           enabled = EXCLUDED.enabled,
           prefix = EXCLUDED.prefix,
           live_only = EXCLUDED.live_only`, [streamerId, enabled, prefix, liveOnly]);
        return res.json({ ok: true });
    }
    catch (e) {
...
// --------------------  
internalBotRouter.get("/internal/bot/streamer/:streamerIdOrSlug/settings", async (req, res) => {
    if (!requireBotKey(req, res))
        return;
    ...
    if (/^\d+$/.test(streamerIdOrSlug)) {
        ...
        const r = await pool.query(`SELECT id FROM streamers WHERE lower(slug)=lower($1) LIMIT 1`, [streamerIdOrSlug]);
        streamerId = Number(r.rows?.[0]?.id || 0);
    }
    if (!streamerId) {
        return res.status(404).json({ ok: false, error: "streamer not found" });
    ...
    try {
        const r = await pool.query(`SELECT enabled, prefix, live_only FROM bot_settings WHERE streamer_id=$1`, [streamerId]);
...
// --------------------  
internalBotRouter.post("/internal/bot/discord/send", express.json(), async (req, res) => {
    if (!requireBotKey(req, res))
        return;
    const body = req.body || {};
