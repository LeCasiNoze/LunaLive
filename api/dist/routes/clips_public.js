...
function setMp4Cors(req, res) {
    ...
    res.setHeader("Access-Control-Allow-Headers", "Authorization,Range,Content-Type,Accept,Origin");
    res.setHeader("Access-Control-Expose-Headers", "Content-Length,Content-Type,Content-Range,Accept-Ranges,Content-Disposition,ETag");
...
clipsPublicRouter.get("/clips/:id/mp4", async (req, res) => {
    ...
    const r = await pool.query(`SELECT mp4_key, title
     FROM bot_clips
     WHERE id=$1
...
clipsPublicRouter.get("/streamers/:slug/clips", async (req, res) => {
    ...
    if (!streamerId)
        return res.json({ ok: true, clips: [], pageInfo: { endCursor: null, hasNextPage: false } });
    // cursor
 ...
 */
clipsPublicRouter.post("/clips/:id/like", requireAuth, express.json(), async (req, res) => {
    ...
    if (!check.rows?.length)
        return res.status(404).json({ ok: false, error: "clip_not_found" });
    if (like) {
        await pool.query(`INSERT INTO clip_likes(clip_id, user_id, created_ts)
       VALUES($1,$2,$3)
       ON CONFLICT (clip_id, user_id) DO NOTHING`, [clipId, userId, nowMs()]);
    }
    else {
 ...
 */
clipsPublicRouter.post("/clips/:id/delete", requireAuth, async (req, res) => {
    const user = req.user;
    ...
    if (!Number.isFinite(clipId) || clipId <= 0)
        return res.status(400).json({ ok: false, error: "id_required" });
    const r = await pool.query(`SELECT s.user_id AS owner_user_id
     FROM bot_clips bc
