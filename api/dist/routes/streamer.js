...
streamerRouter.post("/streamer/apply", requireAuth, a(async (req, res) => {
    ...
    const { rows } = await pool.query(`INSERT INTO streamer_requests (user_id, status)
       VALUES ($1, 'pending')
       ON CONFLICT (user_id) DO UPDATE
         SET status='pending', updated_at = NOW()
       RETURNING id, status, created_at AS "createdAt"`, [userId]);
    res.json({ ok: true, request: rows[0] });
}));
streamerRouter.get("/streamer/request", requireAuth, a(async (req, res) => {
    const userId = req.user.id;
    const { rows } = await pool.query(`SELECT id, status, created_at AS "createdAt"
