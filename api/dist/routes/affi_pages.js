...
function affiSnapshotKey(slug) {
    return `static/affi_pages/${String(slug || "").trim().toLowerCase()}.json`;
}
async function publishAffiSnapshot(page) {
    if (!r2Enabled())
        return;
...
fsbAffiPagesRouter.post("/fsb/affi-pages", a(async (req, res) => {
    ...
    const { rows } = await pool.query(`INSERT INTO affi_landing_pages (
         owner_user_id,
         slug,
         model,
         variant,
...
fsbAffiPagesRouter.put("/fsb/affi-pages/:id", a(async (req, res) => {
    ...
    const { rows } = await pool.query(`UPDATE affi_landing_pages
          SET slug = $2,
              model = $3,
              variant = $4,
              brand_name = $5,
...
fsbAffiPagesRouter.delete("/fsb/affi-pages/:id", a(async (req, res) => {
    const id = Number(req.params.id || 0);
    if (!Number.isInteger(id) || id <= 0) {
        return res.status(400).json({ ok: false, error: "bad_id" });
    }
...
publicAffiPagesRouter.post("/public/affi-events", a(async (req, res) => {
    ...
    await pool.query(`INSERT INTO affi_landing_events
...
fsbAffiPagesRouter.get("/fsb/affi-pages/:id/stats", a(async (req, res) => {
    ...
    const { rows } = await pool.query(`WITH ev AS (
