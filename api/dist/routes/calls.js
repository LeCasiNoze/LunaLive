...
function canModOnStreamer(user, meta) {
    if (!user)
        return false;
    if (user.role === "admin")
        return true;
    if (meta.ownerUserId != null && Number(meta.ownerUserId) === Number(user.id))
        return true;
...
callsRouter.get("/:slug/list", requireAuth, async (req, res) => {
    try {
        const u = req.user;
        if (!u)
