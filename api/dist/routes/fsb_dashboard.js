...
function parseDateOnly(input, fallback) {
...
fsbDashboardRouter.get("/fsb/dashboard/instagram", a(async (req, res) => {
    await ensureBotClips();
    const today = new Date().toISOString().slice(0, 10);
    const from = parseDateOnly(req.query?.from, today);
    const to = parseDateOnly(req.query?.to, addDays(from, 30));
