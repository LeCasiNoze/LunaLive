...
async function safeScalar(sql, params = [], key = "v") {
    try {
        const r = await pool.query(sql, params);
...
adminPlatformStatsRouter.get("/admin/platform-stats", requireAdminKey, a(async (_req, res) => {
    ...
    const [totalUsers, newUsersMonth, newUsersToday, totalStreamers, streamersLive, publishedCasinos, currentViewers, uniqueViewersMonth, totalViewerSessionsMonth, avgSessionSecMonth, streamSessionsMonth, totalStreamMinutesMonth, mem, uptime,] = await Promise.all([
        // ── Users ──────────────────────────────────────
