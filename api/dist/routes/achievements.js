...
async function getMetrics(userId) {
  ...
  `;
    ...
    const winterWatchMinutes = hasStreamViewerMinutes
        ? await safeCount(`SELECT COUNT(*)::int AS n FROM stream_viewer_minutes
         WHERE user_id=$1
           AND EXTRACT(MONTH FROM (bucket_ts AT TIME ZONE 'Europe/Paris')) IN (12, 1, 2)`, [userId])
        : 0;
    const lateNightWatchNights = hasStreamViewerMinutes
        ? await safeCount(`SELECT COUNT(*)::int AS n FROM (
    ...
    const maxLiveDay = hasChatMessages
        ? await safeCount(`SELECT COALESCE(MAX(c),0)::int AS n FROM (
    ...
    const maxArchivedDay = hasChatMessageStats
        ? await safeCount(`SELECT COALESCE(MAX(messages_sent),0)::int AS n FROM chat_message_stats WHERE user_id=$1`, [userId])
        : 0;
    ...
    const ogOk = (await safeCount(`SELECT COUNT(*)::int AS n FROM users WHERE id=$1 AND created_at < $2::date`, [userId, OG_CUTOFF])) > 0;
    ...
    const wheelMaxGainHits = hasWheel
        ? await safeCount(`SELECT COUNT(*)::int AS n FROM daily_wheel_spins WHERE user_id=$1 AND segment_index = 8`, [userId])
        : 0;
    ...
    const bossDamageTotal = hasEvents && hasEventScores
        ? await safeSum(`SELECT COALESCE(SUM(s.points),0)::bigint AS s
         FROM event_scores s JOIN events e ON e.id = s.event_id
         WHERE s.user_id=$1 AND e.type='burn_boss'`, [userId])
        : 0;
    const bossDamageBestEdition = hasEvents && hasEventScores
        ? await safeCount(`SELECT COALESCE(MAX(s.points),0)::int AS n
         FROM event_scores s JOIN events e ON e.id = s.event_id
         WHERE s.user_id=$1 AND e.type='burn_boss'`, [userId])
        : 0;
    const chestEventContribTotal = hasChestDeposits
        ? await safeSum(`SELECT COALESCE(SUM(rubis),0)::bigint AS s FROM event_chest_deposits WHERE user_id=$1`, [userId])
        : 0;
    ...
    const viewerWeekWins = hasEvents && hasEventGrants
        ? await safeCount(`SELECT COUNT(*)::int AS n
         ...
         WHERE g.user_id=$1 AND g.tier='win' AND g.rank = 1 AND e.type='viewer_week'`, [userId])
        : 0;
    const podiumEventTypes = hasEvents && hasEventGrants
        ? await safeCount(`SELECT COUNT(DISTINCT e.type)::int AS n
         FROM event_reward_grants g JOIN events e ON e.id = g.event_id
         WHERE g.user_id=$1 AND g.tier='win' AND g.rank BETWEEN 1 AND 3
           AND e.type IN ${RANKED_EVENT_TYPES}`, [userId])
        : 0;
