...
const ALLOWED_KINDS = ["username", "badge", "title", "frame", "hat"];
...
// ──────────────────────────────────────────
const DEV_UNLOCK_ALL_SET = new Set(String(process.env.DEV_UNLOCK_ALL_FOR || "")
    .split(",")
    .map((s) => s.trim().toLowerCase())
    .filter(Boolean));
function devUnlockAll(username) {
    if (!username)
        return false;
...
async function getReqUsername(req) {
...
function buildOwnedAllActive(staticItems, extraItems = []) {
...
function catalogHas(staticItems, kind, code) {
    return staticItems.some((x) => x && x.active && x.kind === kind && x.code === code);
}
function isSubBadgeCode(code) {
    const p = "badge_sub_";
...
async function getActiveSubBadgesForUser(userId) {
    const q = await pool.query(`SELECT s.slug, s.display_name AS "displayName", s.appearance
     ...
     ORDER BY lower(s.slug) ASC`, [userId]);
    const out = [];
...
async function userHasActiveSubToSlug(userId, slug) {
...
// ──────────────────────────────────────────
cosmeticsRouter.get("/cosmetics/catalog", a(async (req, res) => {
    const staticItems = (COSMETICS_CATALOG || []).filter((x) => x && x.active);
...
// ──────────────────────────────────────────
cosmeticsRouter.get("/me/cosmetics", requireAuth, a(async (req, res) => {
    const userId = Number(req.user?.id);
    if (!Number.isFinite(userId) || userId <= 0)
    ...
    const eq = await pool.query(`SELECT username_code AS "username",
              badge_code    AS "badge",
              title_code    AS "title",
              frame_code    AS "frame",
              hat_code      AS "hat"
       ...
       WHERE user_id = $1
       LIMIT 1`, [userId]);
    const equipped = eq.rows?.[0] || { username: null, badge: null, title: null, frame: null, hat: null };
    // owned via entitlements
    const ent = await pool.query(`SELECT kind, code
    ...
    {
        const set = new Set(owned.badge);
        for (const it of subBadges)
            set.add(it.code);
    ...
    if (unlockAll) {
        ...
        for (const k of Object.keys(all)) {
            const cur = new Set(owned[k] || []);
            for (const code of all[k] || [])
                cur.add(code);
...
// ──────────────────────────────────────────
cosmeticsRouter.patch("/me/cosmetics/equip", requireAuth, a(async (req, res) => {
    const userId = Number(req.user?.id);
    if (!Number.isFinite(userId) || userId <= 0)
        return res.status(401).json({ ok: false, error: "unauthorized" });
    const username = await getReqUsername(req);
    ...
    // null => unequip
    if (code === null) {
        const upd = await pool.query(`UPDATE user_equipped_cosmetics
