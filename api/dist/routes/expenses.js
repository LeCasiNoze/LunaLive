...
function nextMonthStartDate(monthKey) {
    const [year, month] = monthKey.split("-").map(Number);
...
function getExpenseId(param) {
    const value = String(param || "").trim();
    return /^\d+$/.test(value) ? value : null;
}
function resolveRecurringOccurrenceDate(anchorDate, monthKey) {
    const [year, month] = monthKey.split("-").map(Number);
    const anchorDay = Number(anchorDate.slice(8, 10));
    const day = Math.min(anchorDay, daysInMonth(year, month));
    return `${monthKey}-${String(day).padStart(2, "0")}`;
}
function isOccurrenceValid(expense, occurrenceDate) {
    if (!expense.isRecurring)
        return false;
    if (occurrenceDate.slice(0, 7) < expense.date.slice(0, 7))
        return false;
    const expectedDate = resolveRecurringOccurrenceDate(expense.date, occurrenceDate.slice(0, 7));
    if (expectedDate !== occurrenceDate)
        return false;
    if (expense.recurrenceEndDate && occurrenceDate > expense.recurrenceEndDate)
        return false;
    return true;
}
...
function resolveAgencyPaymentDate(paymentDate, monthKey) {
...
async function syncAgencyExpensesForMonth(monthKey) {
    ...
    try {
      ...
      `, [monthStart, monthEnd]);
        ...
        for (const row of result.rows) {
            ...
            if (payouts.streamerTotal <= 0)
                continue;
            const assignmentId = Number(row.assignment_id);
            validAssignmentIds.push(assignmentId);
            const dueDate = resolveAgencyPaymentDate(normalizeDateOnly(row.payment_date) || normalizeDateOnly(row.start_date) || monthStart, monthKey);
            await client.query(`
...
function buildVisibleExpenseRows(expenses, paidMap, monthKey) {
    return expenses
        .map((expense) => {
        ...
        if (monthKey < expense.date.slice(0, 7))
            return null;
...
expensesRouter.get("/expenses", a(async (req, res) => {
    ...
    const dueThisMonthResult = await pool.query(`SELECT COALESCE(SUM(amount), 0) AS total FROM expenses
       WHERE source_type = 'agency_streamer_payout'
         AND date >= $1::date AND date < $2::date`, [monthStart, nextMonthStart]);
    ...
    if (recurringIds.length) {
        ...
        for (const row of occurrenceResult.rows) {
            paidMap.set(`${String(row.expense_id)}:${normalizeDateOnly(row.occurrence_date)}`, toIso(row.paid_at));
        }
    }
    const visibleExpenses = buildVisibleExpenseRows(expenses, paidMap, monthKey);
    const summary = visibleExpenses.reduce((acc, row) => {
        acc.total += Number(row.expense.amount || 0);
        if (row.paid)
            acc.paid += Number(row.expense.amount || 0);
        else
            acc.due += Number(row.expense.amount || 0);
        return acc;
...
expensesRouter.post("/expenses", a(async (req, res) => {
...
expensesRouter.put("/expenses/:id", a(async (req, res) => {
    const expenseId = getExpenseId(req.params?.id);
    if (!expenseId) {
        return res.status(400).json({ ok: false, error: "bad_id" });
    }
    const lockedResult = await pool.query(`SELECT id, source_type FROM expenses WHERE id = $1 LIMIT 1`, [expenseId]);
...
expensesRouter.put("/expenses/:id/payment", a(async (req, res) => {
    const expenseId = getExpenseId(req.params?.id);
    if (!expenseId) {
        return res.status(400).json({ ok: false, error: "bad_id" });
    }
...
expensesRouter.delete("/expenses/:id", a(async (req, res) => {
    const expenseId = getExpenseId(req.params?.id);
    if (!expenseId) {
        return res.status(400).json({ ok: false, error: "bad_id" });
    }
