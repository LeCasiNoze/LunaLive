...
streamerDliveLinkRouter.get("/", requireAuth, requireStreamer, async (req, res) => {
...
streamerDliveLinkRouter.post("/request", requireAuth, requireStreamer, async (req, res) => {
    const streamerId = req.streamerId;
    ...
    await pool.query(`UPDATE streamer_dlive_link_requests
     SET status='expired'
     WHERE streamer_id=$1 AND status='pending'`, [streamerId]);
    ...
    const ins = await pool.query(`INSERT INTO streamer_dlive_link_requests
     ...
     VALUES ($1,$2,$3,$4,$5)
     RETURNING id, created_at AS "createdAt", expires_at AS "expiresAt"`, [streamerId, displayname, info.username, code, expiresAt.toISOString()]);
    res.json({
        ok: true,
        request: ins.rows[0],
        code,
...
streamerDliveLinkRouter.post("/verify", requireAuth, requireStreamer, async (req, res) => {
    const streamerId = req.streamerId;
...
streamerDliveLinkRouter.post("/toggle", requireAuth, requireStreamer, async (req, res) => {
    const streamerId = req.streamerId;
...
streamerDliveLinkRouter.post("/unlink", requireAuth, requireStreamer, async (req, res) => {
    const streamerId = req.streamerId;
