...
function parseDliveDisplayname(input) {
...
async function getStreamerIdByUserId(userId) {
    const r = await pool.query(`SELECT id FROM streamers WHERE user_id=$1`, [userId]);
    const sid = r.rows?.[0]?.id;
    return sid ? Number(sid) : null;
}
async function maybeSyncUsersLinkToStreamer(userId, streamerId) {
    // Si le streamer n'a pas encore la link mais que users l'a, on copie (one-time sync)
    const s = await pool.query(`SELECT dlive_link_username AS u, dlive_link_displayname AS d, dlive_use_linked AS use
     FROM users WHERE id=$1`, [userId]);
    const u = s.rows?.[0];
    if (!u?.u && !u?.d)
        return;
    const t = await pool.query(`SELECT dlive_link_username AS u, dlive_link_displayname AS d
     FROM streamers WHERE id=$1`, [streamerId]);
    const st = t.rows?.[0];
    ...
    await pool.query(`UPDATE streamers
     SET dlive_link_displayname=$2,
         ...
         dlive_use_linked=$4
     WHERE id=$1`, [streamerId, u.d ?? null, u.u ?? null, !!u.use]);
}
// GET /streamer/me/dlive-link
streamerDliveLinkRouter.get("/", requireAuth, async (req, res) => {
...
streamerDliveLinkRouter.post("/request", requireAuth, async (req, res) => {
    const userId = Number(req.user?.id || 0);
    if (!userId)
        return res.status(401).json({ ok: false, error: "unauthorized" });
    ...
    if (!info.username)
        return res.status(400).json({ ok: false, error: "dlive_channel_not_found" });
    const streamerId = await getStreamerIdByUserId(userId);
    await pool.query(`UPDATE streamer_dlive_link_requests
     SET status='expired'
     WHERE user_id=$1 AND status='pending'`, [userId]);
    const code = `LL-${randomBytes(4).toString("hex").toUpperCase()}`;
    const expiresAt = new Date(Date.now() + 10 * 60_000);
    const ins = await pool.query(`INSERT INTO streamer_dlive_link_requests
      (streamer_id, user_id, requested_displayname, requested_username, code, expires_at)
     VALUES ($1,$2,$3,$4,$5,$6)
     RETURNING id, created_at AS "createdAt", expires_at AS "expiresAt"`, [streamerId, userId, displayname, info.username, code, expiresAt.toISOString()]);
    res.json({
        ok: true,
        request: ins.rows[0],
        code,
...
streamerDliveLinkRouter.post("/verify", requireAuth, async (req, res) => {
    const userId = Number(req.user?.id || 0);
    if (!userId)
        return res.status(401).json({ ok: false, error: "unauthorized" });
    const streamerId = await getStreamerIdByUserId(userId);
    ...
    const row = r.rows?.[0];
    if (!row)
        return res.status(400).json({ ok: false, error: "no_pending_request" });
    ...
    let wait = { ok: false, error: "timeout" };
    ...
    try {
        ...
        await client.query(`UPDATE streamer_dlive_link_requests
       ...
       WHERE id=$1`, [userId, row.requestedDisplayname, row.requestedUsername]);
        ...
        if (streamerId) {
         ...
         SET dlive_link_displayname=$2,
             ...
             dlive_linked_at=NOW(),
             dlive_use_linked=TRUE
         WHERE id=$1`, [streamerId, row.requestedDisplayname, row.requestedUsername]);
        }
...
streamerDliveLinkRouter.post("/toggle", requireAuth, async (req, res) => {
    const userId = Number(req.user?.id || 0);
    if (!userId)
        return res.status(401).json({ ok: false, error: "unauthorized" });
    ...
    const streamerId = await getStreamerIdByUserId(userId);
    if (useLinked) {
        const u = await pool.query(`SELECT dlive_link_displayname AS d, dlive_link_username AS u FROM users WHERE id=$1`, [userId]);
        const ok = !!String(u.rows?.[0]?.u || u.rows?.[0]?.d || "").trim();
        if (!ok)
            return res.status(400).json({ ok: false, error: "no_linked_channel" });
    }
...
streamerDliveLinkRouter.post("/unlink", requireAuth, async (req, res) => {
    const userId = Number(req.user?.id || 0);
    if (!userId)
        return res.status(401).json({ ok: false, error: "unauthorized" });
    const streamerId = await getStreamerIdByUserId(userId);
