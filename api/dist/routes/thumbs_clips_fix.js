...
export async function generateAndStoreThumbnail(clipId, mp4Url) {
...
export function setupClipsThumbsRoute(app) {
    app.get("/thumbs/clips/:id.jpg", async (req, res) => {
        const clipId = Number(req.params.id || 0);
        if (!Number.isFinite(clipId) || clipId <= 0)
            return res.status(400).end();
        // 1) Vérifier thumbnail_url en BDD
        const { rows } = await pool.query(`SELECT thumbnail_url, mp4_key FROM bot_clips WHERE id = $1 AND deleted_ts IS NULL LIMIT 1`, [clipId]);
        const clip = rows?.[0];
        if (!clip)
