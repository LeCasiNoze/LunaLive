...
earningsRouter.get("/streamer/me/earnings", requireAuth, async (req, res, next) => {
    try {
        ...
        const s = await pool.query(`SELECT id, slug, COALESCE(mods_percent_bp,0) AS mods_percent_bp
       FROM streamers
       WHERE user_id=$1
       LIMIT 1`, [req.user.id]);
        ...
        const lots = await pool.query(`SELECT weight_bp::int AS weight_bp, COALESCE(SUM(amount_remaining),0)::bigint AS n
       FROM rubis_lots
       WHERE user_id=$1 AND amount_remaining>0
       GROUP BY weight_bp
       ORDER BY weight_bp DESC`, [req.user.id]);
        const breakdownByWeight = {};
        let valueCents = 0;
        ...
        const last = await pool.query(`SELECT
