...
import { getLevelInfo, getUnlockedPerks, getUpcomingPerks, getShopGrade, MAX_LEVEL, XP_THRESHOLDS, } from "../economy/xp.js";
export const xpRouter = Router();
/**
 * GET /me/xp — état XP du user courant
 * Retourne level, progress, perks débloqués + à venir, grade shop.
 */
xpRouter.get("/me/xp", requireAuth, a(async (req, res) => {
 ...
 */
xpRouter.get("/me/xp/events", requireAuth, a(async (req, res) => {
    const userId = Number(req.user?.id);
    if (!Number.isFinite(userId) || userId <= 0) {
        return res.status(401).json({ ok: false, error: "unauthorized" });
    }
    const limit = Math.max(1, Math.min(200, Number(req.query?.limit) || 50));
    const r = await pool.query(`SELECT id, delta, source, reason, meta, created_at
         FROM xp_events
        WHERE user_id = $1
        ORDER BY id DESC
 ...
 */
xpRouter.get("/xp/curve", a(async (_req, res) => {
    return res.json({
        ok: true,
        maxLevel: MAX_LEVEL,
 ...
 */
xpRouter.get("/xp/leaderboard", a(async (req, res) => {
    const limit = Math.max(1, Math.min(100, Number(req.query?.limit) || 20));
    const r = await pool.query(`SELECT u.id, u.username, u.avatar_path, u.xp::bigint AS xp
         FROM users u
