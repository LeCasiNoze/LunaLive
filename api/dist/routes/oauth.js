...
const OAUTH_CALLBACK_BASE = String(process.env.OAUTH_CALLBACK_BASE || process.env.RENDER_EXTERNAL_URL || "https://lunalive-api.onrender.com").replace(/\/$/, "");
...
function buildDiscordAuthUrl(clientId, redirectUri, state) {
    const params = new URLSearchParams({
        client_id: clientId,
        redirect_uri: redirectUri,
        response_type: "code",
...
oauthRouter.get("/auth/oauth/:provider/start", a(async (req, res) => {
...
async function exchangeDiscord(code, clientId, clientSecret, redirectUri) {
...
async function generateAvailableUsername(seed) {
    ...
    if (base.length < 3)
        base = (base + randomBytes(3).toString("hex")).slice(0, 16);
...
async function loginOrCreateUser(provider, profile, state, req) {
    const existing = await pool.query(`SELECT u.id, u.username, u.role
     FROM user_oauth_identities oi
     JOIN users u ON u.id = oi.user_id
     WHERE oi.provider=$1 AND oi.provider_user_id=$2
     LIMIT 1`, [provider, profile.providerUserId]);
    if (existing.rows[0]) {
    ...
    if (profile.email && profile.emailVerified) {
        const match = await pool.query(`SELECT id, username, role FROM users WHERE lower(email)=lower($1) AND email_verified=TRUE LIMIT 1`, [profile.email]);
        if (match.rows[0]) {
            const u = match.rows[0];
            try {
                await pool.query(`INSERT INTO user_oauth_identities (user_id, provider, provider_user_id, email) VALUES ($1,$2,$3,$4)`, [u.id, provider, profile.providerUserId, profile.email]);
            }
            catch {
    ...
    try {
       ...
       RETURNING id`, [username, profile.email, emailVerified, passwordHash, getClientIp(req), state.utm]);
    }
    catch (e) {
        if (e?.code === "23505")
    ...
    try {
        ...
        if (refSlug) {
            ...
            if (streamerId > 0) {
                await pool.query(`INSERT INTO user_referrals(user_id, streamer_id, source) VALUES ($1,$2,'signup') ON CONFLICT (user_id) DO NOTHING`, [userId, streamerId]);
            }
        }
    }
    catch (e) {
        console.log("[oauth] referral lock failed", e?.message || e);
    }
    await pool.query(`UPDATE users SET avatar_path = COALESCE(avatar_path, $2) WHERE id=$1`, [userId, defaultAvatarPath(userId)]);
    await pool.query(`INSERT INTO user_oauth_identities (user_id, provider, provider_user_id, email) VALUES ($1,$2,$3,$4)`, [userId, provider, profile.providerUserId, profile.email]);
    return signToken({ id: userId, username, role: "viewer" });
}
oauthRouter.get("/auth/oauth/:provider/callback", a(async (req, res) => {
    const providerParam = String(req.params.provider || "").toLowerCase();
    if (!isOAuthProvider(providerParam))
    ...
    if (!clientId || !clientSecret)
        return res.redirect(302, doneUrl("error=oauth_not_configured"));
    try {
        const redirectUri = redirectUriFor(providerParam);
        const profile = providerParam === "google"
        ...
        if (e instanceof OAuthEmailConflictError)
            return res.redirect(302, doneUrl("error=email_conflict"));
        console.warn(`[oauth/${providerParam}] callback failed:`, e?.message || e);
        return res.redirect(302, doneUrl("error=oauth_failed"));
    }
}));
