...
publicRouter.get("/content/:key", a(async (req, res) => {
    const key = String(req.params.key || "").trim();
    if (!key)
        return res.status(400).json({ ok: false, error: "missing_key" });
    const { rows } = await pool.query(`SELECT key, title, html, updated_at
       FROM site_content
       WHERE key = $1`, [key]);
 ...
 * Body: { title: string }
 */
publicRouter.post("/streamers/:slug/title", requireAuth, a(async (req, res) => {
    ...
    const upd = await pool.query(`UPDATE streamers
       ...
       RETURNING id::text AS id, slug, display_name AS "displayName",
                 title, viewers, is_live AS "isLive", featured, user_id AS "ownerUserId"`, [title, Number(streamer.id)]);
 ...
 */
publicRouter.post("/streamers/:slug/host", requireAuth, a(async (req, res) => {
    const slug = String(req.params.slug || "").trim();
    if (!slug)
        return res.status(400).json({ ok: false, error: "bad_slug" });
    const requesterId = Number(req.user.id);
    ...
    if (!target.isLive)
        return res.status(400).json({ ok: false, error: "target_not_live" });
...
publicRouter.post("/streamers/:slug/follow", requireAuth, a(async (req, res) => {
    const slug = String(req.params.slug || "").trim();
    if (!slug)
        return res.status(400).json({ ok: false, error: "bad_slug" });
    ...
    if ((ins.rowCount ?? 0) > 0) {
        const tpl = (streamer.appearance?.chat?.followMessageTemplate &&
...
publicRouter.delete("/streamers/:slug/follow", requireAuth, a(async (req, res) => {
    const slug = String(req.params.slug || "").trim();
    if (!slug)
        return res.status(400).json({ ok: false, error: "bad_slug" });
...
publicRouter.patch("/streamers/:slug/follow/notify", requireAuth, a(async (req, res) => {
    const slug = String(req.params.slug || "").trim();
    if (!slug)
        return res.status(400).json({ ok: false, error: "bad_slug" });
    ...
    if (!streamer)
        return res.status(404).json({ ok: false, error: "streamer_not_found" });
    const upd = await pool.query(`UPDATE streamer_follows
       SET notify_enabled = $3
       WHERE streamer_id = $1 AND user_id = $2
       RETURNING notify_enabled`, [Number(streamer.id), Number(req.user.id), notifyEnabled]);
    if (!upd.rows?.[0])
