...
async function getStreamerBySlug(slug) {
    ...
    if (!s)
        return null;
    const r = await pool.query(`SELECT id, slug, user_id AS "userId", is_live AS "isLive"
     FROM streamers
     WHERE lower(slug)=lower($1)
     LIMIT 1`, [s]);
    return r.rows?.[0] || null;
}
async function getChestBalance(client, streamerId) {
    const r = await client.query(`SELECT COALESCE(SUM(amount_remaining),0)::int AS balance
     FROM streamer_chest_lots
     WHERE streamer_id=$1`, [streamerId]);
    return Number(r.rows?.[0]?.balance || 0);
}
async function getChestBreakdown(client, streamerId) {
    const r = await client.query(`SELECT weight_bp, COALESCE(SUM(amount_remaining),0)::int AS amount
     FROM streamer_chest_lots
     WHERE streamer_id=$1
     GROUP BY weight_bp
     ORDER BY weight_bp DESC`, [streamerId]);
    const out = {};
    for (const row of r.rows || [])
        out[String(row.weight_bp)] = Number(row.amount || 0);
    return out;
}
async function ensureChest(client, streamerId) {
    await client.query(`INSERT INTO streamer_chests (streamer_id)
     VALUES ($1)
     ON CONFLICT (streamer_id) DO NOTHING`, [streamerId]);
}
async function getOpenOpening(client, streamerId) {
    const r = await client.query(`SELECT id, status, opens_at AS "opensAt", closes_at AS "closesAt", min_watch_minutes AS "minWatchMinutes"
...
async function getCurrentLiveSessionId(client, streamerId) {
    const r = await client.query(`SELECT id
     FROM live_sessions
...
async function hasRecentHeartbeat(client, liveSessionId, viewerKey) {
    const r = await client.query(`SELECT 1
     ...
     LIMIT 1`, [liveSessionId, viewerKey, HEARTBEAT_TTL_SECONDS]);
    return !!r.rows?.[0];
}
async function getWatchedMinutes(client, liveSessionId, viewerKey) {
    const r = await client.query(`SELECT COUNT(*)::int AS n
     FROM stream_viewer_minutes
     WHERE live_session_id=$1
       AND viewer_key=$2`, [liveSessionId, viewerKey]);
 ...
 */
async function depositToChest(streamerId, fromUserId, amount, note) {
    ...
    try {
        ...
        const used = [];
        while (remaining > 0) {
            const r = await client.query(`SELECT id, origin, weight_bp AS "weightBp", amount_remaining AS "amountRemaining"
         FROM rubis_lots
        ...
        // ledger tx
        const txIns = await client.query(`INSERT INTO rubis_tx
       ...
       RETURNING id`, [
        ...
        // entries
        await client.query(`INSERT INTO rubis_tx_entries (tx_id, entity, user_id, delta)
       VALUES
       ($1, 'user',  $2, $3),
       ($1, 'chest', NULL, $4)`, [txId, fromUserId, -amount, amount]);
        // map used lots in ledger
        for (const u of used) {
            await client.query(`INSERT INTO rubis_tx_lots (tx_id, lot_id, origin, weight_bp, amount_used)
         VALUES ($1,$2,$3,$4,$5)
         ON CONFLICT DO NOTHING`, [txId, u.lotId, u.origin, u.weightBp, u.used]);
...
async function openChest(streamerId, byUserId, durationSec, minWatchMinutes) {
    const client = await pool.connect();
    try {
        await client.query("BEGIN");
        await ensureChest(client, streamerId);
        ...
        const ins = await client.query(`INSERT INTO streamer_chest_openings
...
async function closeOpeningAndPayout(openingId, closedBy) {
    const client = await pool.connect();
    try {
        await client.query("BEGIN");
        // lock opening
        const o = await client.query(`SELECT id, streamer_id AS "streamerId", status, closes_at AS "closesAt", min_watch_minutes AS "minWatchMinutes"
       FROM streamer_chest_openings
       WHERE id=$1
       FOR UPDATE`, [openingId]);
        const opening = o.rows?.[0];
        if (!opening)
            throw new Error("opening_not_found");
        if (String(opening.status) !== "open") {
            // idempotent: return existing payouts
            const p = await client.query(`SELECT user_id AS "userId", amount, breakdown, tx_id AS "txId"
         FROM streamer_chest_payouts
         WHERE opening_id=$1
         ORDER BY id ASC`, [openingId]);
            await client.query("COMMIT");
            return { alreadyClosed: true, payouts: p.rows || [] };
        }
        const streamerId = Number(opening.streamerId);
        // lock chest lots (best value first)
        const lotsRes = await client.query(`SELECT id, weight_bp AS "weightBp", amount_remaining AS "amountRemaining"
       FROM streamer_chest_lots
       WHERE streamer_id=$1 AND amount_remaining > 0
       ORDER BY weight_bp DESC, created_at ASC, id ASC
       FOR UPDATE`, [streamerId]);
        const lots = (lotsRes.rows || []).map((x) => ({
            id: Number(x.id),
        ...
        // participants
        const partRes = await client.query(`SELECT user_id AS "userId"
       FROM streamer_chest_participants
       WHERE opening_id=$1
       ORDER BY joined_at ASC`, [openingId]);
        const participants = (partRes.rows || []).map((x) => Number(x.userId));
        if (participants.length === 0 || total <= 0) {
            await client.query(`UPDATE streamer_chest_openings
         SET status='closed', closed_at=NOW(), meta = meta || $2::jsonb
         WHERE id=$1`, [openingId, JSON.stringify({ closedBy })]);
            await client.query("COMMIT");
        ...
        const bonusSet = new Set(idx.slice(0, rem));
        // allocate from chest lots
        let lotPtr = 0;
        const payouts = [];
        const takeFromLots = (need) => {
            const breakdown = {};
            while (need > 0) {
                if (lotPtr >= lots.length)
                    throw new Error("chest_empty_race");
        ...
        for (let i = 0; i < participants.length; i++) {
            ...
            for (const [w, a] of Object.entries(breakdown)) {
                ...
                await client.query(`INSERT INTO rubis_lots (user_id, origin, weight_bp, amount_total, amount_remaining, meta)
         ...
         VALUES
         ($1, 'chest', NULL, $2),
         ($1, 'user',  $3, $4)`, [txId, -amount, userId, amount]);
            // payout record
            await client.query(`INSERT INTO streamer_chest_payouts (opening_id, user_id, amount, breakdown, tx_id)
         VALUES ($1,$2,$3,$4::jsonb,$5)
         ON CONFLICT (opening_id, user_id) DO NOTHING`, [openingId, userId, amount, JSON.stringify(breakdown), txId]);
...
chestRouter.get("/streamers/:slug/chest", async (req, res) => {
    ...
    try {
        ...
        let joined = false;
        if (opening) {
            const pc = await client.query(`SELECT COUNT(*)::int AS n FROM streamer_chest_participants WHERE opening_id=$1`, [opening.id]);
 ...
 * streamer only (owner)
 */
chestRouter.post("/streamers/:slug/chest/deposit", requireAuth, async (req, res) => {
    const slug = String(req.params.slug || "").trim();
    const s = await getStreamerBySlug(slug);
    if (!s)
        return res.status(404).json({ ok: false, error: "streamer_not_found" });
 ...
 * streamer owner only
 */
chestRouter.post("/streamers/:slug/chest/open", requireAuth, async (req, res) => {
    const slug = String(req.params.slug || "").trim();
    const s = await getStreamerBySlug(slug);
    if (!s)
        return res.status(404).json({ ok: false, error: "streamer_not_found" });
 ...
 */
chestRouter.post("/streamers/:slug/chest/join", requireAuth, async (req, res) => {
    const slug = String(req.params.slug || "").trim();
    const s = await getStreamerBySlug(slug);
    if (!s)
        return res.status(404).json({ ok: false, error: "streamer_not_found" });
    ...
    try {
        ...
        await client.query(`INSERT INTO streamer_chest_participants (opening_id, user_id)
       VALUES ($1,$2)
       ON CONFLICT (opening_id, user_id) DO NOTHING`, [opening.id, meId]);
        await client.query("COMMIT");
 ...
 */
chestRouter.post("/streamers/:slug/chest/close", requireAuth, async (req, res) => {
    const slug = String(req.params.slug || "").trim();
    const s = await getStreamerBySlug(slug);
    if (!s)
        return res.status(404).json({ ok: false, error: "streamer_not_found" });
