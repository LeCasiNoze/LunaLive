...
function parseRumbleUsername(input) {
...
function isValidRumbleUsername(s) {
...
async function getStreamerByUserId(userId) {
    const r = await pool.query(`SELECT id, slug FROM streamers WHERE user_id=$1 LIMIT 1`, [userId]);
...
streamerRumbleLinkRouter.get("/", requireAuth, async (req, res) => {
    const userId = Number(req.user?.id || 0);
    if (!userId)
        return res.status(401).json({ ok: false, error: "unauthorized" });
    const streamer = await getStreamerByUserId(userId);
    if (!streamer)
        return res.json({ ok: true, linked: false, streamer: null, account: null });
    const acc = await pool.query(`SELECT id, username, assigned_at, api_key IS NOT NULL AS has_api_key
 ...
 */
streamerRumbleLinkRouter.post("/", requireAuth, async (req, res) => {
    const userId = Number(req.user?.id || 0);
    if (!userId)
        return res.status(401).json({ ok: false, error: "unauthorized" });
    const streamer = await getStreamerByUserId(userId);
    ...
    const inUse = await pool.query(`SELECT assigned_to_streamer_id FROM rumble_accounts
     WHERE api_key = $1 AND assigned_to_streamer_id IS NOT NULL AND assigned_to_streamer_id <> $2
     LIMIT 1`, [apiKey, streamer.id]);
    if (inUse.rows[0]) {
    ...
    const existing = await pool.query(`SELECT id FROM rumble_accounts WHERE assigned_to_streamer_id = $1 LIMIT 1`, [streamer.id]);
    if (existing.rows[0]) {
        await pool.query(`UPDATE rumble_accounts
       SET username = $2, api_key = $3, assigned_at = NOW(), updated_at = NOW()
       WHERE id = $1`, [existing.rows[0].id, probe.username, apiKey]);
    }
    else {
        await pool.query(`INSERT INTO rumble_accounts (username, api_key, assigned_to_streamer_id, assigned_at)
       VALUES ($1, $2, $3, NOW())`, [probe.username, apiKey, streamer.id]);
    }
...
streamerRumbleLinkRouter.delete("/", requireAuth, async (req, res) => {
    const userId = Number(req.user?.id || 0);
    if (!userId)
        return res.status(401).json({ ok: false, error: "unauthorized" });
    const streamer = await getStreamerByUserId(userId);
    ...
    await pool.query(`UPDATE rumble_accounts
     ...
     WHERE assigned_to_streamer_id = $1`, [streamer.id]);
