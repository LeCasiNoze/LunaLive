...
import { FABIO_GUILD_ID, FABIO_CAT_MODO_ID, FABIO_ROLE_MOD_ID, FABIO_ROLE_CHEF_ID, FABIO_ROLE_NOTIF_STREAM, FABIO_ROLE_NOTIF_INSTA, FABIO_ROLE_NOTIF_YT, FABIO_ROLE_NOTIF_TW, FABIO_NOTIF_CHANNEL_ID, FABIO_STATS_CHANNEL_ID, } from "./constants.js";
export const CID_STATS_LIST = "fabio:stats:list:"; // + casino slug
let statsChannelId = null;
let statsMsgId = null;
// ─── Setup ────────────────────────────────────────────────────────────────────
export async function ensureStatsChannel(client, ctx) {
    ...
    if (FABIO_STATS_CHANNEL_ID) {
        ch = await client.channels.fetch(FABIO_STATS_CHANNEL_ID).catch(() => null);
    }
    if (!ch) {
        const channels = await guild.channels.fetch().catch(() => null);
        const found = channels?.find((c) => c?.parentId === FABIO_CAT_MODO_ID && c?.name?.includes("stats"));
        if (found) {
            ch = await client.channels.fetch(found.id).catch(() => null);
        }
    }
    if (!ch) {
    ...
    try {
        const notifCh = await client.channels.fetch(FABIO_NOTIF_CHANNEL_ID).catch(() => null);
    ...
    try {
        ...
        const existing = msgs?.find((m) => m.author.id === client.user?.id && m.embeds[0]?.title?.includes("Statistiques"));
        if (existing) {
...
export async function refreshStatsMessage(client, ctx) {
    if (!statsChannelId || !statsMsgId)
        return;
    try {
        const guild = await client.guilds.fetch(FABIO_GUILD_ID).catch(() => null);
        if (!guild)
        ...
        const countRole = (roleId) => guild.members.cache.filter((m) => m.roles.cache.has(roleId)).size;
    ...
    `, [FABIO_GUILD_ID]);
      ...
      ORDER BY total DESC
    `, [FABIO_GUILD_ID]);
        const casinoLines = byCasino.rows.map((r) => `▸ **${r.casino}** — ✅ ${r.approved} · ❌ ${r.rejected} · ⏳ ${r.pending} en attente — déposé **${r.deposited}€** · remboursé **${r.reimbursed}€**`).join("\n") || "_Aucun ticket pour l'instant._";
        // ── Giveaways actifs ─────────────────────────────────────────────────────
    ...
    `, [FABIO_GUILD_ID]);
        ...
        const embed = {
            ...
            fields: [
                {
                    name: "👥 Serveur",
                    value: `Total membres : **${totalMembers}**\n` +
                        `🔴 Stream : **${nbStream}** · 📸 Insta : **${nbInsta}** · ▶️ YT : **${nbYT}** · 🐦 Twitter : **${nbTW}**`,
                    inline: false,
                },
                {
                    name: "🎫 Tickets",
                    value: `Total : **${ts.total}** — ` +
                        `✅ Validés : **${ts.approved}** · ❌ Refusés : **${ts.rejected}** · ⏳ En attente : **${ts.pending}**`,
                    inline: false,
                },
                {
                    name: "💰 Montants",
                    value: `Déposé (validés) : **${ts.total_deposited}€** · Remboursé : **${ts.total_reimbursed}€**`,
                    inline: false,
                },
                {
                    name: "🎰 Par casino",
        ...
        const ch = await client.channels.fetch(statsChannelId).catch(() => null);
        if (!ch)
            return;
...
export async function handleStatsList(interaction, casino, ctx) {
    ...
    `, [FABIO_GUILD_ID, casino]);
        ...
        const lines = rows.rows.map((r, i) => `**${i + 1}.** ${r.discord_username} — \`${r.casino_email}\` — ` +
