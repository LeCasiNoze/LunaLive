...
import { APPLY_CHANNEL_ID, CID_APPLY_DECIDE_PREFIX, CID_APPLY_MODAL, CID_APPLY_OPEN, STAFF_DECISIONS_CHANNEL_ID, STAFF_DECISIONS_PING_ROLE_ID, STAFF_ROLE_IDS, STAFF_TICKETS_CATEGORY_ID, } from "./constants.js";
import { normalizeAccept, safeDm } from "./utils.js";
...
export function buildApplyModal() {
    ...
    const rulesInput = new TextInputBuilder()
        ...
        .setRequired(true)
        .setMaxLength(20);
    modal.addComponents(new ActionRowBuilder().addComponents(discordInput), new ActionRowBuilder().addComponents(dliveInput), new ActionRowBuilder().addComponents(linksInput), new ActionRowBuilder().addComponents(expInput), new ActionRowBuilder().addComponents(rulesInput));
    return modal;
}
function ticketName(username, discordId) {
...
export async function ensureApplyMessage(guild, ctx) {
    try {
...
export async function createTicketChannel(guild, member, ctx) {
    ...
    const topic = clamp(`Streamer apply ticket | user=${member.user.tag} (${member.id})`, LIMITS.channelTopic);
...
export async function dbUpsertStreamerRequest(discordUserId, payload) {
    ...
    return { ok: true, request: up.rows[0], userId };
}
export async function sendDecisionLog(guild, content, ctx) {
    try {
...
export async function approveRequest(requestId) {
    ...
    try {
        ...
        const upd = await client.query(`UPDATE streamer_requests SET status='approved', updated_at=NOW() WHERE id=$1 RETURNING user_id`, [requestId]);
        if (!upd.rows[0]) {
      ...
      `, [slug, username, userId]);
        ...
        if (!streamerId) {
            await client.query("ROLLBACK");
            return { ok: false, error: "streamer_missing" };
        }
        ...
        try {
            ...
            console.warn("[discord/apply] DLive assignment skipped:", e?.message || e);
        }
        await client.query("COMMIT");
        return { ok: true, userId, username, streamer: { id: streamerId, slug: String(s.rows[0]?.slug || slug) } };
    }
    catch (e) {
        try {
            await client.query("ROLLBACK");
...
export async function rejectRequest(requestId) {
    ...
    try {
        await client.query("BEGIN");
        const upd = await client.query(`UPDATE streamer_requests SET status='rejected', updated_at=NOW() WHERE id=$1 RETURNING user_id`, [requestId]);
        if (!upd.rows[0]) {
            await client.query("ROLLBACK");
            return { ok: false, error: "not_found" };
        }
...
export function validateRulesInput(rulesField) {
    return normalizeAccept(rulesField) === normalizeAccept("J'ACCEPTE");
}
export function buildStaffActionsRow(requestId) {
...
export function buildDisabledActionsRowApproved() {
    return new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId("noop1").setLabel("✅ Approuvé").setStyle(ButtonStyle.Success).setDisabled(true), new ButtonBuilder().setCustomId("noop2").setLabel("❌ Refuser").setStyle(ButtonStyle.Danger).setDisabled(true));
}
export function buildDisabledActionsRowRejected() {
    return new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId("noop1").setLabel("✅ Approuver").setStyle(ButtonStyle.Success).setDisabled(true), new ButtonBuilder().setCustomId("noop2").setLabel("❌ Refusé").setStyle(ButtonStyle.Danger).setDisabled(true));
}
export async function handleStaffDecisionButton(opts) {
    ...
    if (action === "approve") {
        ...
        await sendDecisionLog(guild, `✅ **APPROUVÉ** — request #${requestId} — par <@${staffMember.id}>` +
            (applicantDiscordUserId ? ` — user <@${applicantDiscordUserId}>` : ""), ctx);
        ...
        if (applicantDiscordUserId) {
            await safeDm(client, applicantDiscordUserId, `✅ Ta demande Streamer LunaLive a été **approuvée**.\nTu peux suivre le ticket si besoin : <#${interactionChannelId}>`, ctx);
        }
        return { ok: true, applicantDiscordUserId, kind: "approved" };
    }
    const res = await rejectRequest(requestId);
    if (!res.ok)
        return { ok: false, error: res.error };
    await sendDecisionLog(guild, `❌ **REFUSÉ** — request #${requestId} — par <@${staffMember.id}>` +
        (applicantDiscordUserId ? ` — user <@${applicantDiscordUserId}>` : ""), ctx);
    await interactionMessageEdit({
        content: `✅ Décision traitée. (Log envoyé dans <#${STAFF_DECISIONS_CHANNEL_ID}>)`,
    ...
    if (applicantDiscordUserId) {
        await safeDm(client, applicantDiscordUserId, `❌ Ta demande Streamer LunaLive a été **refusée**.\nTu peux répondre dans le ticket si tu veux des précisions : <#${interactionChannelId}>`, ctx);
    }
    return { ok: true, applicantDiscordUserId, kind: "rejected" };
}
export function staffCanDecide(member) {
