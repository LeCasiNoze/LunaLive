...
import { FABIO_GUILD_ID, FABIO_CAT_MODO_ID, FABIO_ROLE_MOD_ID, FABIO_ROLE_CHEF_ID, CID_FABIO_OFFER_ADD, CID_FABIO_OFFER_EDIT, CID_FABIO_OFFER_TOGGLE, CID_FABIO_OFFER_EVENT, CID_FABIO_OFFER_ADD_MODAL, CID_FABIO_OFFER_EVENT_MODAL, } from "./constants.js";
...
function casinoColor(slug) {
    return CASINO_COLORS[slug.toLowerCase()] ?? 0x2ECC71;
}
// ─── DB ops ───────────────────────────────────────────────────────────────────
export async function getActiveOffers(guildId) {
    const r = await pool.query(`SELECT * FROM discord_casino_offers
     WHERE guild_id = $1 AND active = true
       AND (type = 'permanent' OR event_end_at > now())
     ORDER BY created_at ASC`, [guildId]);
    return r.rows;
}
export async function getAllOffers(guildId) {
    const r = await pool.query(`SELECT * FROM discord_casino_offers WHERE guild_id = $1 ORDER BY active DESC, created_at ASC`, [guildId]);
    return r.rows;
}
export async function getOfferById(id) {
    const r = await pool.query(`SELECT * FROM discord_casino_offers WHERE id = $1`, [id]);
    return r.rows[0] ?? null;
}
export async function createOffer(data, client) {
    ...
    const r = await pool.query(`INSERT INTO discord_casino_offers
...
export async function updateOffer(id, data) {
    ...
    const r = await pool.query(`UPDATE discord_casino_offers SET ${sets}, updated_at = now() WHERE id = $1 RETURNING *`, [id, ...vals]);
    return r.rows[0] ?? null;
}
// ─── Dashboard embed ──────────────────────────────────────────────────────────
let dashboardChannelId = null;
let dashboardMessageId = null;
export async function ensureOfferDashboard(client, ctx) {
    if (String(FABIO_GUILD_ID) !== String(FABIO_GUILD_ID))
        return; // safety
    ...
    let ch = channels?.find((c) => c?.parentId === FABIO_CAT_MODO_ID && c?.name?.includes("gestion-offres"));
    if (!ch) {
...
export async function refreshDashboard(client, ctx) {
    ...
    try {
        ...
        if (dashboardMessageId) {
            await ch.messages.fetch(dashboardMessageId).then((m) => m.edit(payload)).catch(() => { dashboardMessageId = null; });
        }
        if (!dashboardMessageId) {
...
function buildDashboardPayload(offers) {
...
export function buildEventModal(offerId) {
    return {
...
export function parseAddOfferModal(fields) {
    ...
    if (!casino || !label)
        return { error: "Casino et nom requis." };
    const parts = amountStr.split("/");
    const min_deposit = parseInt(parts[0]);
    const reimburse_amt = parseInt(parts[1]);
    if (isNaN(min_deposit) || isNaN(reimburse_amt))
        return { error: "Format montants invalide. Ex: 50/25" };
    let reimburse_type = "wallet";
    let crypto;
