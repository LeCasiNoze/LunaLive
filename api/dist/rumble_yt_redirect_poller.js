...
function eligibleSlugs() {
    const raw = String(process.env.YT_REDIFF_STREAMER_SLUGS || "lecasinoze,fabiozsis");
    return raw.split(",").map((s) => s.trim().toLowerCase()).filter(Boolean);
}
async function listPendingVods() {
    const slugs = eligibleSlugs();
    if (slugs.length === 0)
        return [];
    const r = await pool.query(`SELECT v.id, v.streamer_id, s.slug, s.display_name,
...
async function nextStreamNumber(streamerId) {
    const r = await pool.query(`SELECT COUNT(*)::int AS n FROM rumble_vods WHERE streamer_id = $1 AND yt_uploaded_at IS NOT NULL`, [streamerId]);
    return Number(r.rows[0]?.n || 0) + 1;
}
async function processOne(vod) {
    ...
    await pool.query(`UPDATE rumble_vods SET yt_upload_attempts = yt_upload_attempts + 1 WHERE id = $1`, [vod.id]);
    try {
        ...
        WHERE id = $1`, [vod.id, videoId, streamNumber]);
        ...
        await pool.query(`UPDATE rumble_vods SET yt_upload_error = $2 WHERE id = $1`, [vod.id, msg]).catch(() => { });
    }
}
async function tick() {
    if (!process.env.YT_REFRESH_TOKEN)
        return; // pas configure -> on dort
    try {
