...
function normalizeRestream(senderDisplay, senderUsername, content) {
    const isRestream = /^restreambot$/i.test(senderDisplay) ||
        /^restreambot$/i.test(senderUsername) ||
        /restream/i.test(senderDisplay);
    if (!isRestream)
        return { username: senderDisplay, body: content, restreamFrom: null };
...
async function resolveImmutableUsername(displaynameOrUsername) {
    ...
    if (displayname.toLowerCase().startsWith("dlive-"))
        return displayname;
...
export function ensureDliveBridge(opts) {
    ...
    const broadcast = (m) => {
        ...
        if (dliveId && seenIds.has(dliveId))
            return;
        ...
        const isBot = (BOT_DLIVE_USERNAME && senderUsername.toLowerCase() === BOT_DLIVE_USERNAME) ||
            (BOT_DLIVE_DISPLAY && senderDisplay.toLowerCase() === BOT_DLIVE_DISPLAY);
        if (isBot)
            return; // ✅ ignore messages du bot pour éviter doublons
    ...
    const startWs = (immutableUsername) => {
        ...
        ws.on("error", (e) => {
            console.warn("[dlive_bridge] ws error", key, e?.message || e);
            closeWs();
            scheduleReconnect(1200);
        });
    ...
    const start = async () => {
        ...
        try {
            ...
            console.warn("[dlive_bridge] resolve/start error", key, e?.message || e);
            closeWs();
            scheduleReconnect(2500);
        }
    };
