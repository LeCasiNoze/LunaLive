...
export function xpTotalForLevel(n) {
    if (n <= 0)
        return 0;
    let sum = 0;
    for (let i = 1; i <= Math.min(n, MAX_LEVEL); i++)
        sum += xpForLevel(i);
    return sum;
}
...
export function getBlackjackCooldownMs(level) {
...
export function getDailyBonusMultiplier(level) {
    if (level >= 25)
        return 1.10;
    if (level >= 15)
        return 1.05;
...
export function getWeeklyWheelTicketsBonus(level) {
    if (level >= 40)
        return 2;
    if (level >= 5)
        return 1;
...
export function getShopGrade(level) {
    let g = 0;
    for (const k of Object.keys(LEVEL_PERKS).map(Number)) {
        if (k > level)
            continue;
        for (const perk of LEVEL_PERKS[k]) {
            if (perk.kind === "shop_grade")
                g = Math.max(g, perk.grade);
        }
    }
    return g;
}
 ...
 */
export async function awardXpTx(client, userId, amount, source, reason, meta) {
    ...
    if (cap != null && cap > 0) {
        const r = await client.query(`SELECT COALESCE(SUM(delta), 0)::int AS used
         FROM xp_events
        WHERE user_id = $1 AND source = $2
          AND day = (NOW() AT TIME ZONE 'Europe/Paris')::date`, [userId, source]);
    ...
    await client.query(`INSERT INTO xp_events (user_id, delta, source, reason, meta)
     VALUES ($1, $2, $3, $4, $5::jsonb)`, [userId, delta, source, reason || null, meta ? JSON.stringify(meta) : null]);
    return {
        delta,
        capped,
        newXp,
