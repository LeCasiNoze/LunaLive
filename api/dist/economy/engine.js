...
import { ORIGIN_WEIGHT_BP } from "./config.js";
function floorInt(n) {
    return Math.floor(n);
}
function valueCentsFrom(amountRubis, weightBp) {
...
async function selectLots(client, userId, kind) {
    const order = kind === "support" ? "weight_bp DESC, id ASC" : "weight_bp ASC, id ASC";
    const { rows } = await client.query(`SELECT id, origin, weight_bp, amount_remaining
     FROM rubis_lots
...
function allocateFromLots(lots, amount) {
    const out = [];
    let left = amount;
    ...
    if (left > 0)
        throw new Error("insufficient_lots");
    return out;
}
async function applyAllocations(client, alloc) {
    for (const a of alloc) {
        await client.query(`UPDATE rubis_lots
       SET amount_remaining = amount_remaining - $2
       WHERE id=$1`, [a.lotId, a.amount]);
    }
}
async function createLot(client, userId, origin, amount, meta = {}) {
    const wbp = ORIGIN_WEIGHT_BP[origin] ?? 0;
    await client.query(`INSERT INTO rubis_lots (user_id, origin, weight_bp, amount_total, amount_remaining, meta)
     VALUES ($1,$2,$3,$4,$4,$5::jsonb)`, [userId, origin, wbp, amount, JSON.stringify(meta)]);
}
/** Helper: écrit un breakdown lots */
async function insertTxLots(client, txId, alloc) {
    for (const a of alloc) {
        await client.query(`INSERT INTO rubis_tx_lots (tx_id, lot_id, origin, weight_bp, amount_used)
       VALUES ($1,$2,$3,$4,$5)`, [txId, a.lotId, a.origin, a.weightBp, a.amount]);
    }
}
/** Helper: entries comptables (optionnel mais utile) */
async function insertEntry(client, txId, entity, userId, delta) {
    await client.query(`INSERT INTO rubis_tx_entries (tx_id, entity, user_id, delta)
     VALUES ($1,$2,$3,$4)`, [txId, entity, userId, delta]);
}
export async function mintRubis(params) {
    ...
    try {
        ...
        const tx = await client.query(`INSERT INTO rubis_tx (kind, purpose, status, from_user_id, to_user_id, amount, meta)
       VALUES ('mint',$2,'succeeded',NULL,$1,$3,$4::jsonb)
       RETURNING id`, [userId, origin, amount, JSON.stringify(meta || {})]);
...
export async function spendSink(params) {
    ...
    try {
        ...
        const tx = await client.query(`INSERT INTO rubis_tx (kind, purpose, status, from_user_id, to_user_id, amount, support_value, streamer_amount, platform_amount, burn_amount, meta)
       VALUES ('sink',$2,'succeeded',$1,NULL,$3,0,0,0,$4,$5::jsonb)
       RETURNING id`, [userId, purpose, amount, burnAmount, JSON.stringify(meta || {})]);
        const txId = Number(tx.rows[0].id);
...
export async function spendSupport(params) {
    ...
    try {
        ...
        const s = await client.query(`SELECT COALESCE(mods_percent_bp,0)::int AS mods_percent_bp
       FROM streamers
       WHERE id=$1
       LIMIT 1`, [streamerId]);
        ...
        let modsPaid = [];
        if (modsTotal > 0) {
            const mods = await client.query(`SELECT user_id
         FROM streamer_mods
         WHERE streamer_id=$1 AND removed_at IS NULL
         ORDER BY user_id ASC`, [streamerId]);
            const ids = mods.rows.map((r) => Number(r.user_id)).filter(Boolean);
            if (ids.length > 0) {
        ...
        const tx = await client.query(`INSERT INTO rubis_tx (kind, purpose, status, from_user_id, to_user_id, amount,
       ...
       RETURNING id`, [
        ...
        await insertEntry(client, txId, "user", userId, -amount);
        if (platformAmount > 0)
            await insertEntry(client, txId, "platform_fee", null, +platformAmount);
        if (burnAmount > 0)
            await insertEntry(client, txId, "platform_burn", null, +burnAmount);
        if (streamerAmount > 0)
            await insertEntry(client, txId, "user", streamerOwnerUserId, +streamerAmount);
        for (const m of modsPaid) {
            if (m.amount > 0)
                await insertEntry(client, txId, "user", m.userId, +m.amount);
        }
        await client.query("COMMIT");
        return {
            ok: true,
...
export async function cashoutRequest(params) {
    ...
    if (!Number.isFinite(eurosCents) || eurosCents <= 0)
        throw new Error("bad_amount");
    const client = await pool.connect();
    try {
        await client.query("BEGIN");
        ...
        if (leftCents > 0)
            throw new Error("insufficient_value");
       ...
       RETURNING id`, [streamerOwnerUserId, rubisToDebit, eurosCents, JSON.stringify({ ...(meta || {}), streamerId })]);
        const txId = Number(tx.rows[0].id);
        await insertTxLots(client, txId, alloc);
        await insertEntry(client, txId, "user", streamerOwnerUserId, -rubisToDebit);
        // on garde ton cashout_requests (déjà dans DB)
        await client.query(`INSERT INTO cashout_requests (streamer_id, amount_rubis, status, tx_id, note)
       VALUES ($1,$2,'pending',$3,$4)`, [streamerId, rubisToDebit, txId, meta?.note ? String(meta.note) : null]);
        await client.query("COMMIT");
