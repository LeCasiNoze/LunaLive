...
export async function uploadVodToYouTube(input) {
    const privacy = input.privacyStatus || process.env.YT_PRIVACY || "unlisted";
    ...
    if (!uploadUrl)
        throw new Error("YT resumable init: header Location manquant");
    ...
    const videoId = j?.id;
    if (!videoId)
        throw new Error(`YT upload: reponse sans id (${JSON.stringify(j).slice(0, 300)})`);
    return { videoId, uploadUrl };
}
/**
 * Formate "Rediff stream #N (JJ/MM)" en se basant sur la date de fin du live (Europe/Paris).
 */
export function formatRediffTitle(streamNumber, endedAt) {
