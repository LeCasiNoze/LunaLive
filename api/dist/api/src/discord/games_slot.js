 ...
 *  - max    :  0.20% (9980..10000)
 */
export function rollSlotOutcome(rnd = Math.random()) {
    const x = Math.floor(rnd * 10000);
    const pick = (from, to) => x >= from && x < to;
    if (pick(0, 500)) {
        return {
            code: "fucked",
            label: "[FUCKED] -X3",
 ...
 */
export async function slotCheckAndTouchCooldownTx(client, discordUserId, cooldownMs = SLOT_COOLDOWN_MS) {
    try {
        const r = await client.query(`SELECT last_play_at FROM discord_slot_cooldowns WHERE discord_user_id=$1 FOR UPDATE`, [discordUserId]);
        const now = Date.now();
        ...
        if (!row) {
            await client.query(`INSERT INTO discord_slot_cooldowns (discord_user_id, last_play_at, updated_at)
         VALUES ($1, NOW(), NOW())`, [discordUserId]);
        }
        else {
            await client.query(`UPDATE discord_slot_cooldowns
...
export async function discordSlotPlayTxClient(client, discordUserId) {
...
export function buildSpinFrames(finalArt) {
