...
function allowedStakesFromLevel(level) {
...
async function getBetLevel(userId) {
    // 1) NEW: talents
    try {
...
async function getLastStake(userId) {
    try {
...
async function getStreamerIdFromReq(req) {
    ...
    if (slug)
        return await resolveStreamerIdFromSlug(String(slug));
    return null;
}
/**
...
predictionsRouter.get("/api/bot/predictions/stake", requireAuth, async (req, res) => {
    try {
        ...
        if (!isAllowedStake(selected, allowed))
            selected = allowed[0];
...
predictionsRouter.post("/api/bot/predictions/create", requireAuth, async (req, res) => {
    try {
        ...
        const pred = await createPredictionSafe(pool, Number(streamerId), String(question).trim(), String(option1).trim(), String(option2).trim(), Math.max(30, Number(durationSec) || 180), Math.max(10, Number(fixedStake) || 10));
...
predictionsRouter.post("/api/bot/predictions/bet", requireAuth, async (req, res) => {
    try {
        ...
        if (!isAllowedStake(stake, allowed)) {
            if (bodyStake != null)
                return res.status(400).json({ ok: false, reason: "stake_not_allowed", allowed, level });
            stake = allowed[0];
        }
        // 💸 spend rubis (SINK)
        await spendRubis({
        ...
        await addBet(pool, pred, userId, choice, stake);
...
predictionsRouter.post("/api/bot/predictions/resolve", requireAuth, async (req, res) => {
    try {
        ...
        await resolvePrediction(pool, pred, winning);
