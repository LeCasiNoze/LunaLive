...
/* Streamer request */
streamerRouter.post("/streamer/apply", requireAuth, a(async (req, res) => {
    ...
    const dliveDisplayname = String(req.body?.dliveDisplayname ?? req.body?.dlive_displayname ?? "").trim() || null;
    ...
    const { rows } = await pool.query(`INSERT INTO streamer_requests
       ...
       RETURNING
         ...
         rules_accepted AS "rulesAccepted"`, [userId, discord, channelUrl, hasChannel, hasDlive, dliveDisplayname, rulesAccepted, payload]);
    res.json({ ok: true, request: rows[0] });
}));
streamerRouter.get("/streamer/request", requireAuth, a(async (req, res) => {
    const userId = req.user.id;
...
/* Streamer (dashboard) */
streamerRouter.get("/streamer/me", requireAuth, a(async (req, res) => {
    const { rows } = await pool.query(`SELECT id::text AS id, slug, display_name AS "displayName",
    ...
    const s = rows[0] || null;
    if (!s)
        return res.json({ ok: true, streamer: null });
    s.appearance = normalizeAppearance(s.appearance || {});
    res.json({ ok: true, streamer: s });
}));
streamerRouter.patch("/streamer/me", requireAuth, a(async (req, res) => {
    ...
    const { rows } = await pool.query(`UPDATE streamers
       SET title = $1, updated_at = NOW()
       WHERE user_id = $2
       RETURNING id::text AS id, slug, display_name AS "displayName",
                 title, viewers, is_live AS "isLive", featured, appearance`, [title, req.user.id]);
    if (!rows[0])
        return res.status(404).json({ ok: false, error: "not_found" });
    rows[0].appearance = normalizeAppearance(rows[0].appearance || {});
    res.json({ ok: true, streamer: rows[0] });
}));
streamerRouter.get("/streamer/me/appearance", requireAuth, a(async (req, res) => {
    if (req.user.role !== "streamer" && req.user.role !== "admin") {
        return res.status(403).json({ ok: false, error: "forbidden" });
    }
    const cur = await pool.query(`SELECT slug, appearance, offline_bg_path
       FROM streamers WHERE user_id=$1 LIMIT 1`, [req.user.id]);
    if (!cur.rows[0])
        return res.status(404).json({ ok: false, error: "not_found" });
...
streamerRouter.patch("/streamer/me/appearance", requireAuth, a(async (req, res) => {
    if (req.user.role !== "streamer" && req.user.role !== "admin") {
        return res.status(403).json({ ok: false, error: "forbidden" });
    }
    ...
    const upd = await pool.query(`UPDATE streamers
       SET appearance = $1, updated_at = NOW()
       WHERE user_id = $2
       RETURNING id::text AS id, slug, display_name AS "displayName",
                 title, viewers, is_live AS "isLive", featured, appearance`, [merged, req.user.id]);
...
streamerRouter.get("/streamer/me/connection", requireAuth, a(async (req, res) => {
    if (req.user.role !== "streamer" && req.user.role !== "admin") {
        return res.status(403).json({ ok: false, error: "forbidden" });
    }
    const { rows } = await pool.query(`SELECT pa.provider,
