...
async function touchChest(client, streamerId) {
    // best-effort
    try {
...
async function getParticipantsCount(client, openingId) {
    const r = await client.query(`SELECT COUNT(*)::int AS n
     FROM streamer_chest_participants
     WHERE opening_id=$1`, [openingId]);
    return Number(r.rows?.[0]?.n || 0);
}
async function hasJoined(client, openingId, userId) {
    const r = await client.query(`SELECT 1
     FROM streamer_chest_participants
     WHERE opening_id=$1 AND user_id=$2
     LIMIT 1`, [openingId, userId]);
    return !!r.rows?.[0];
}
/**
 ...
 */
async function hasRecentHeartbeatByStreamer(client, streamerId, viewerKey) {
    const r = await client.query(`SELECT 1
     ...
     LIMIT 1`, [streamerId, viewerKey, HEARTBEAT_TTL_SECONDS]);
    return !!r.rows?.[0];
}
/**
 ...
 */
async function getWatchedMinutesByStreamer(client, streamerId, viewerKey) {
    const r = await client.query(`SELECT COUNT(*)::int AS n
     FROM stream_viewer_minutes
     WHERE streamer_id=$1
       AND viewer_key=$2
       AND bucket_ts >= (NOW() - ($3::int * INTERVAL '1 hour'))`, [streamerId, viewerKey, WATCH_WINDOW_HOURS]);
    return Number(r.rows?.[0]?.n || 0);
}
/* =========================
...
async function closeOpeningAndPayout(openingId, closedBy) {
    ...
    try {
        ...
        const o = await client.query(`SELECT id, streamer_id AS "streamerId", status
       FROM streamer_chest_openings
       WHERE id=$1
       ...
       FOR UPDATE`, [streamerId]);
        ...
        // participants
        const parts = await client.query(`SELECT user_id AS "userId"
       FROM streamer_chest_participants
       WHERE opening_id=$1
       ORDER BY joined_at ASC`, [openingId]);
        const users = parts.rows.map((r) => Number(r.userId));
        ...
        if (!users.length) {
         ...
         WHERE id=$1`, [openingId, JSON.stringify({ closedBy, reason: "no_participants" })]);
            await client.query("COMMIT");
        ...
        if (total <= 0) {
            await client.query(`UPDATE streamer_chest_openings
         SET status='closed', closed_at=NOW(), meta = meta || $2::jsonb
         WHERE id=$1`, [openingId, JSON.stringify({ closedBy, reason: "empty_chest" })]);
            await client.query("COMMIT");
            return { alreadyClosed: false, payouts: [] };
        }
        // =========================
      ...
      `, [users]);
        const premiumSet = new Set((premRes.rows || []).map((r) => Number(r.user_id)));
        ...
        try {
            ...
            if (opening) {
                ...
                try {
                    ...
                    if (au?.id)
                        joined = await hasJoined(client, Number(opening.id), Number(au.id));
                }
                catch { }
                openingOut = {
...
chestRouter.post("/streamers/:slug/chest/open", requireAuth, async (req, res) => {
    try {
        ...
        if (!streamer.isLive)
            return res.status(400).json({ ok: false, error: "stream_offline" });
...
chestRouter.post("/streamers/:slug/chest/join", requireAuth, async (req, res) => {
    try {
        ...
        try {
            const opening = await getOpenOpening(client, Number(streamer.id));
            if (!opening)
                return res.status(400).json({ ok: false, error: "no_opening" });
            ...
            if (!hb)
                return res.status(400).json({ ok: false, error: "not_watching" });
            ...
            await client.query(`INSERT INTO streamer_chest_participants(opening_id, user_id, joined_at)
        VALUES ($1,$2,NOW())
        ON CONFLICT (opening_id, user_id) DO NOTHING`, [Number(opening.id), userId]);
...
chestRouter.post("/streamers/:slug/chest/deposit", requireAuth, async (req, res) => {
    try {
        ...
        try {
           ...
           VALUES ($1,$2,$3,$4,$5::jsonb)`, [Number(streamer.id), "chest_deposit", w, nn, JSON.stringify(lotMeta)]);
            }
