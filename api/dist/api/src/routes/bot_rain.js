...
const RAIN_TALENT_CODE = "talent_rain_boost";
// +10% / +20% / +30% (niveaux 1..3)
const RAIN_BOOST_MULT = {
    0: 1.0,
    1: 1.10,
    2: 1.20,
    3: 1.30,
};
function applyRainBoost(base, level) {
...
async function getPremiumViewerSet(client, userIds) {
...
async function hasActiveStreamerSub(userId) {
...
function applyPremiumBonus(payoutAfterTalent) {
...
async function getRainBoostLevels(client, userIds) {
    const ids = (userIds || []).map((x) => Number(x)).filter((x) => x > 0);
    const map = new Map();
    if (!ids.length)
        return map;
    try {
        const r = await client.query(`SELECT user_id, level
       FROM user_talents
       WHERE talent_code = $1
         AND user_id = ANY($2::bigint[])`, [RAIN_TALENT_CODE, ids]);
        for (const row of r.rows || []) {
...
async function getStreamerBySlug(slug) {
    ...
    if (!s)
        return null;
    const r = await pool.query(`SELECT id, slug, user_id AS "ownerUserId", is_live AS "isLive", appearance
     FROM streamers
...
async function canManage(req, streamer) {
    ...
    if (role === "admin")
        return true;
    if (streamer.ownerUserId != null && Number(streamer.ownerUserId) === Number(u.id))
        return true;
...
async function sendBotChatAndToast(opts) {
    ...
    const ins = await pool.query(`INSERT INTO chat_messages (streamer_id, user_id, username, body)
     VALUES ($1,$2,$3,$4)
     RETURNING id, created_at AS "createdAt"`, [opts.streamerId, botUserId, botUsername, String(opts.body || "").slice(0, 500)]);
    ...
    if (io) {
        ...
        if (opts.toast?.title)
            io.to(`chat:${opts.slug}`).emit("ui:toast", opts.toast);
    }
}
async function ensureCfg(streamerId) {
    const r = await pool.query(`SELECT enabled, interval_min AS "intervalMin", rubies_per_user AS "rubiesPerUser",
    ...
    await pool.query(`INSERT INTO bot_rain_cfg(streamer_id, enabled, interval_min, rubies_per_user, join_window_sec, offline_reset_sec)
     VALUES ($1,false,$2,$3,120,120)
     ON CONFLICT (streamer_id) DO NOTHING`, [streamerId, intervalMin, rubiesPerUser]);
    return {
        enabled: false,
        intervalMin,
        rubiesPerUser,
...
async function ensureState(streamerId, intervalMin) {
    const r = await pool.query(`SELECT phase, next_at_ms AS "nextAtMs", open_at_ms AS "openAtMs", close_at_ms AS "closeAtMs",
    ...
    await pool.query(`INSERT INTO bot_rain_state(streamer_id, phase, next_at_ms, round_id)
     VALUES ($1,'waiting',$2,0)
     ON CONFLICT (streamer_id) DO NOTHING`, [streamerId, nextAtMs]);
    return {
        phase: "waiting",
        nextAtMs,
        openAtMs: null,
...
async function countParticipants(streamerId, roundId) {
    const r = await pool.query(`SELECT COUNT(*)::int AS n
     FROM bot_rain_joins
     WHERE streamer_id=$1 AND round_id=$2`, [streamerId, roundId]);
    return Number(r.rows?.[0]?.n || 0);
}
async function hasJoined(streamerId, roundId, userId) {
    const r = await pool.query(`SELECT 1 FROM bot_rain_joins WHERE streamer_id=$1 AND round_id=$2 AND user_id=$3 LIMIT 1`, [streamerId, roundId, userId]);
    return !!r.rows?.[0];
}
/**
 ...
 */
async function advanceIfNeeded(req, streamer, cfg) {
    const now = Date.now();
    // disabled => juste mettre phase=disabled (sans casser nextAt)
    if (!cfg.enabled) {
        await pool.query(`UPDATE bot_rain_state SET phase='disabled', updated_at=NOW() WHERE streamer_id=$1`, [streamer.streamerId]);
        return;
    }
    ...
    try {
        ...
        // lock state
        const stRes = await client.query(`SELECT phase, next_at_ms AS "nextAtMs", open_at_ms AS "openAtMs", close_at_ms AS "closeAtMs",
              round_id AS "roundId", offline_since_ms AS "offlineSinceMs"
       FROM bot_rain_state
       WHERE streamer_id=$1
       FOR UPDATE`, [streamer.streamerId]);
        let st = stRes.rows?.[0];
        if (!st) {
            const nextAtMs = now + cfg.intervalMin * 60_000;
            await client.query(`INSERT INTO bot_rain_state(streamer_id, phase, next_at_ms, round_id)
         VALUES ($1,'waiting',$2,0)`, [streamer.streamerId, nextAtMs]);
            st = {
                phase: "waiting",
                nextAtMs,
                openAtMs: null,
        ...
        if (!streamer.isLive) {
            ...
            await client.query(`UPDATE bot_rain_state
         SET phase='offline',
             offline_since_ms = COALESCE(offline_since_ms, $2),
             updated_at=NOW()
         WHERE streamer_id=$1`, [streamer.streamerId, offSince]);
           ...
           SET phase='waiting',
               ...
               offline_since_ms=$3,
               updated_at=NOW()
           WHERE streamer_id=$1`, [streamer.streamerId, resetNext, offSince]);
            }
        ...
        if (offlineSinceMs != null) {
            await client.query(`UPDATE bot_rain_state SET offline_since_ms=NULL, updated_at=NOW() WHERE streamer_id=$1`, [streamer.streamerId]);
        }
        // transitions
        if ((phase === "waiting" || phase === "offline" || phase === "disabled") && now >= nextAtMs) {
         ...
         SET phase='open',
             ...
             close_at_ms=$4,
             updated_at=NOW()
         WHERE streamer_id=$1`, [streamer.streamerId, newRoundId, openAt, closeAt]);
            await client.query("COMMIT");
        ...
        if (phase === "open" && closeAtMs != null && now >= closeAtMs) {
            ...
            const joins = await client.query(`SELECT user_id, username FROM bot_rain_joins WHERE streamer_id=$1 AND round_id=$2`, [streamer.streamerId, curRound]);
            const userIds = (joins.rows || []).map((x) => Number(x.user_id)).filter((x) => x > 0);
         ...
         SET phase='waiting',
             ...
             close_at_ms=NULL,
             updated_at=NOW()
         WHERE streamer_id=$1`, [streamer.streamerId, next]);
            await client.query("COMMIT");
 ...
 */
botRainRouter.get("/public", async (req, res) => {
    try {
        await ensureSchema();
        ...
        const phase = !cfg.enabled ? "disabled" : !streamer.isLive ? "offline" : st.phase === "open" ? "open" : "waiting";
        return res.json({
            ok: true,
            slug: streamer.slug,
            enabled: cfg.enabled,
 ...
 * body: { slug }
 */
botRainRouter.post("/join", async (req, res) => {
    try {
        await ensureSchema();
        ...
        await advanceIfNeeded(req, streamer, cfg);
        if (!cfg.enabled)
            return res.status(409).json({ ok: false, error: "disabled" });
        if (!streamer.isLive)
            return res.status(409).json({ ok: false, error: "offline" });
        ...
        try {
            await client.query("BEGIN");
            const stRes = await client.query(`SELECT phase, close_at_ms AS "closeAtMs", round_id AS "roundId"
         FROM bot_rain_state
         WHERE streamer_id=$1
         FOR UPDATE`, [streamer.streamerId]);
            ...
            await client.query(`INSERT INTO bot_rain_joins(streamer_id, round_id, user_id, username)
         VALUES ($1,$2,$3,$4)
         ON CONFLICT (streamer_id, round_id, user_id) DO NOTHING`, [streamer.streamerId, roundId, userId, username]);
            await client.query("COMMIT");
 ...
 */
botRainRouter.post("/config", async (req, res) => {
    try {
        await ensureSchema();
        ...
        await pool.query(`INSERT INTO bot_rain_cfg(streamer_id, enabled, interval_min, rubies_per_user, updated_at)
       VALUES ($1,$2,$3,$4,NOW())
       ON CONFLICT (streamer_id) DO UPDATE
         SET enabled=EXCLUDED.enabled,
             interval_min=EXCLUDED.interval_min,
             rubies_per_user=EXCLUDED.rubies_per_user,
             updated_at=NOW()`, [streamer.streamerId, enabled, intervalMin, rubiesPerUser]);
        ...
        if (st.phase !== "open") {
            ...
            await pool.query(`UPDATE bot_rain_state
         SET phase=$2,
             next_at_ms=$3,
             updated_at=NOW()
         WHERE streamer_id=$1`, [streamer.streamerId, enabled ? "waiting" : "disabled", nextAtMs]);
        }
