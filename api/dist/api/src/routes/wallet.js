...
walletRouter.get("/wallet", requireAuth, async (req, res) => {
    ...
    const tot = await pool.query(`SELECT COALESCE(SUM(amount_remaining),0)::bigint AS n
     FROM wallet_lots
     WHERE user_id=$1 AND amount_remaining>0`, [userId]);
    const by = await pool.query(`SELECT origin, COALESCE(SUM(amount_remaining),0)::bigint AS n
     FROM wallet_lots
     WHERE user_id=$1 AND amount_remaining>0
     GROUP BY origin
     ORDER BY origin ASC`, [userId]);
    res.json({
        ok: true,
