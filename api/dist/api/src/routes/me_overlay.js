...
function safeExtFromMime(kind, mime) {
...
async function getOwnedStreamerSlugByUserId(userId) {
...
async function dliveGetFollowers(username) {
 ...
 *  ========================= */
meOverlayRouter.get("/followers", a(async (req, res) => {
 ...
 *  ========================= */
meOverlayRouter.post("/alert", a(async (req, res) => {
    const uid = getUserId(req);
    if (!uid)
 ...
 *  ========================= */
meOverlayRouter.post("/alerts/upload", upload.single("file"), a(async (req, res) => {
    const uid = getUserId(req);
    if (!uid)
        return res.status(401).json({ ok: false, error: "unauthorized" });
