...
const socketsBySlugUser = new Map();
...
async function sendBotChat(io, meta, body) {
    ...
    const ins = await pool.query(`INSERT INTO chat_messages (streamer_id, user_id, username, body)
     VALUES ($1,$2,$3,$4)
     RETURNING id, created_at AS "createdAt"`, [meta.id, botUserId, botUsername, text]);
...
async function safeInitDliveBridge(io, streamerId, slug) {
    try {
...
export function attachChat(io) {
    ...
    io.on("connection", (socket) => {
        ...
        socket.on("stream:join", async ({ slug, token }, cb) => {
            try {
                const s = String(slug || "").trim();
                if (!s)
                ...
                if (token) {
                    socket.handshake.auth = { ...socket.handshake.auth, token: String(token) };
                    tryAuth(socket);
                }
        ...
        socket.on("chat:settings_set", async ({ slug, patch }, cb) => {
            try {
                ...
                if (old.dliveSyncPublic !== next.dliveSyncPublic)
                    changed.dliveSyncPublic = next.dliveSyncPublic;
                if (old.dliveSyncPopup !== next.dliveSyncPopup)
                    changed.dliveSyncPopup = next.dliveSyncPopup;
