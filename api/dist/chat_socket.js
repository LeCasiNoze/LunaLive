...
function tryAuth(socket) {
    ...
    if (!token)
        return;
    try {
        data.user = jwt.verify(String(token), getJwtSecret());
    }
    catch {
        // token invalide => guest
    }
}
async function getStreamerMetaBySlug(slug) {
...
async function isStreamerMod(streamerId, userId) {
...
async function isBanned(streamerId, userId) {
    const r = await pool.query(`SELECT 1
     FROM chat_bans
...
async function getActiveTimeout(streamerId, userId) {
    const r = await pool.query(`SELECT expires_at AS "expiresAt"
     FROM chat_timeouts
...
async function computeRolePerms(streamerId, ownerUserId, user) {
...
const socketsBySlugUser = new Map(); // key = `${slug}:${userId}` => socketIds
function keySlugUser(slug, userId) {
    return `${String(slug).toLowerCase()}:${Number(userId)}`;
}
function trackSocket(slug, userId, socketId) {
    const key = keySlugUser(slug, userId);
    const set = socketsBySlugUser.get(key) || new Set();
    set.add(socketId);
    socketsBySlugUser.set(key, set);
}
function untrackSocket(slug, userId, socketId) {
    const key = keySlugUser(slug, userId);
    ...
    set.delete(socketId);
    if (set.size === 0)
        socketsBySlugUser.delete(key);
}
async function pushPermsUpdate(io, slug, streamerId, ownerUserId, userId) {
    const key = keySlugUser(slug, userId);
    const set = socketsBySlugUser.get(key);
    if (!set || set.size === 0)
        return;
    ...
    if (!row)
        return;
    const u = { id: Number(row.id), username: String(row.username), role: String(row.role) };
    const rp = await computeRolePerms(streamerId, ownerUserId, u);
    for (const sid of set) {
        const s = io.sockets.sockets.get(sid);
        if (!s)
            continue;
        s.data.perms = rp.perms;
        s.data.state = rp.state;
        s.data.role = rp.role;
...
async function isFollowing(streamerId, userId) {
    const r = await pool.query(`SELECT 1 FROM streamer_follows WHERE streamer_id=$1 AND user_id=$2 LIMIT 1`, [streamerId, userId]);
    return !!r.rows?.[0];
}
async function isActiveSub(streamerId, userId) {
    const r = await pool.query(`SELECT expires_at
...
async function readSettings(streamerId) {
    const now = Date.now();
...
export function attachChat(io) {
    ...
    io.on("connection", (socket) => {
        ...
        socket.on("chat:join", async ({ slug }, cb) => {
            try {
                const s = String(slug || "").trim();
                if (!s)
                ...
                if (!meta)
                    return cb?.({ ok: false, error: "streamer_not_found" });
                data.slug = meta.slug;
        ...
        socket.on("stream:join", async (payload, cb) => {
            try {
        ...
        socket.on("chat:settings_get", async ({ slug }, cb) => {
            try {
                const u = data.user;
                if (!u)
        ...
        socket.on("chat:settings_set", async ({ slug, patch }, cb) => {
            try {
                const u = data.user;
                if (!u)
                    return cb?.({ ok: false, error: "auth_required" });
                ...
                const changed = {};
                if (old.allowLinks !== next.allowLinks)
                    changed.allowLinks = next.allowLinks;
                if (old.followOnly !== next.followOnly)
                    changed.followOnly = next.followOnly;
                if (old.subOnly !== next.subOnly)
                    changed.subOnly = next.subOnly;
        ...
        socket.on("chat:send", async ({ slug, body }, cb) => {
            try {
                ...
                if (await isBanned(meta.id, u.id))
                    return cb?.({ ok: false, error: "banned" });
                const timeout = await getActiveTimeout(meta.id, u.id);
                if (timeout)
                    return cb?.({ ok: false, error: "timed_out", expiresAt: timeout.expiresAt });
                ...
                if (settings.followOnly) {
                    ...
                    if (!ok)
                        return cb?.({ ok: false, error: "follow_only" });
                }
                if (settings.subOnly) {
                    const ok = await isActiveSub(meta.id, u.id);
                    if (!ok)
                        return cb?.({ ok: false, error: "sub_only" });
                }
        ...
        socket.on("chat:clear", async ({ slug }, cb) => {
            try {
                const u = data.user;
                if (!u)
                    return cb?.({ ok: false, error: "auth_required" });
        ...
        socket.on("chat:refresh", async ({ slug }, cb) => {
            try {
                const s = String(slug || data.slug || "").trim();
                if (!s)
                    return cb?.({ ok: false, error: "bad_slug" });
        ...
        // ─────────────────────────────────────────────
        socket.on("chat:delete", async ({ slug, messageId }, cb) => {
            try {
                const u = data.user;
                if (!u)
                    return cb?.({ ok: false, error: "auth_required" });
                ...
                if (!meta)
                    return cb?.({ ok: false, error: "streamer_not_found" });
                const rp = await computeRolePerms(meta.id, meta.ownerUserId, u);
                if (!rp.perms.canDelete)
                    return cb?.({ ok: false, error: "forbidden" });
                const mid = Number(messageId || 0);
                if (!mid)
                    return cb?.({ ok: false, error: "bad_message" });
                const upd = await pool.query(`UPDATE chat_messages
             SET deleted_at = NOW(),
                 deleted_by = $3
             WHERE id = $1
               AND streamer_id = $2
               AND deleted_at IS NULL
             RETURNING id`, [mid, meta.id, u.id]);
                if (!upd.rows?.[0])
                    return cb?.({ ok: false, error: "message_not_found" });
        ...
        socket.on("chat:timeout", async ({ slug, userId, seconds, reason }, cb) => {
            try {
                const u = data.user;
                if (!u)
                    return cb?.({ ok: false, error: "auth_required" });
                ...
                if (!meta)
                    return cb?.({ ok: false, error: "streamer_not_found" });
                const rp = await computeRolePerms(meta.id, meta.ownerUserId, u);
                if (!rp.perms.canTimeout)
                    return cb?.({ ok: false, error: "forbidden" });
                const targetId = Number(userId || 0);
                if (!targetId)
                    return cb?.({ ok: false, error: "bad_user" });
                if (targetId === u.id)
                    return cb?.({ ok: false, error: "cannot_self_timeout" });
                const sec = clampInt(seconds, 1, 7 * 24 * 3600);
                const expiresAt = new Date(Date.now() + sec * 1000);
                const r = String(reason || "").trim();
                await pool.query(`INSERT INTO chat_timeouts (streamer_id, user_id, expires_at, created_by, reason)
             VALUES ($1,$2,$3,$4,$5)`, [meta.id, targetId, expiresAt, u.id, r || null]);
        ...
        socket.on("chat:ban", async ({ slug, userId, reason }, cb) => {
            try {
                const u = data.user;
                if (!u)
                    return cb?.({ ok: false, error: "auth_required" });
                ...
                if (!targetId)
                    return cb?.({ ok: false, error: "bad_user" });
                if (targetId === u.id)
                    return cb?.({ ok: false, error: "cannot_self_ban" });
                const r = String(reason || "").trim();
                await pool.query(`INSERT INTO chat_bans (streamer_id, user_id, created_by, reason)
             VALUES ($1,$2,$3,$4)
             ON CONFLICT (streamer_id, user_id)
             DO UPDATE SET created_at=NOW(), created_by=EXCLUDED.created_by, reason=EXCLUDED.reason`, [meta.id, targetId, u.id, r || null]);
                const targetUsername = await getUsernameById(targetId);
        ...
        socket.on("chat:untimeout", async ({ slug, userId }, cb) => {
            try {
                const u = data.user;
                if (!u)
                    return cb?.({ ok: false, error: "auth_required" });
                ...
                if (!targetId)
                    return cb?.({ ok: false, error: "bad_user" });
                await pool.query(`UPDATE chat_timeouts
           SET expires_at = NOW()
           WHERE streamer_id=$1 AND user_id=$2 AND expires_at > NOW()`, [meta.id, targetId]);
                const targetUsername = await getUsernameById(targetId);
        ...
        socket.on("chat:unban", async ({ slug, userId }, cb) => {
            try {
                const u = data.user;
                if (!u)
                    return cb?.({ ok: false, error: "auth_required" });
        ...
        socket.on("chat:mod_status", async ({ slug, userId }, cb) => {
            try {
                const u = data.user;
                if (!u)
                    return cb?.({ ok: false, error: "auth_required" });
        ...
        socket.on("chat:timeout_status", async ({ slug, userId }, cb) => {
            try {
                const u = data.user;
                if (!u)
                    return cb?.({ ok: false, error: "auth_required" });
        ...
        socket.on("chat:mod_set", async ({ slug, userId, enabled }, cb) => {
            try {
                const u = data.user;
                if (!u)
                    return cb?.({ ok: false, error: "auth_required" });
                ...
                if (enabled) {
                    await pool.query(`INSERT INTO streamer_mods (streamer_id, user_id, created_by, created_at, removed_at, removed_by)
               VALUES ($1,$2,$3,NOW(),NULL,NULL)
               ON CONFLICT (streamer_id, user_id) DO UPDATE
                 SET created_at = NOW(),
                     created_by = EXCLUDED.created_by,
                     removed_at = NULL,
                     removed_by = NULL`, [meta.id, targetId, u.id]);
                }
                else {
                    await pool.query(`UPDATE streamer_mods
               SET removed_at = NOW(),
                   removed_by = $3
               WHERE streamer_id=$1 AND user_id=$2 AND removed_at IS NULL`, [meta.id, targetId, u.id]);
                }
                const targetUsername = await getUsernameById(targetId);
