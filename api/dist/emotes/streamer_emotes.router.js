...
streamerEmotesRouter.get("/me/streamer/emotes", async (req, res) => {
    try {
        ...
        const r = await pool.query(`SELECT id, kind, scope, streamer_id, name, label, url, mime, size_bytes, status, created_at
       FROM emotes
       WHERE scope='channel' AND streamer_id=$1 AND status <> 'deleted'
       ORDER BY kind ASC, name ASC`, [streamerId]);
...
streamerEmotesRouter.post("/me/streamer/emotes", 
...
streamerEmotesRouter.delete("/me/streamer/emotes/:id", async (req, res) => {
    try {
        ...
        await pool.query(`UPDATE emotes SET status='deleted', updated_at=now()
       WHERE id=$1 AND scope='channel' AND streamer_id=$2`, [id, streamerId]);
        res.json({ ok: true });
    }
    catch (e) {
        res.status(500).json({ ok: false, error: String(e?.message || e) });
