...
const recentRumbleSends = new Map();
function rumbleSendDedupKey(videoId, text) {
    return `${videoId}|${text.trim().slice(0, 200)}`;
}
function shouldSkipRumbleSend(key) {
    const now = Date.now();
    const last = recentRumbleSends.get(key) ?? 0;
    if (now - last < RUMBLE_SEND_DEDUP_MS)
        return true;
 ...
 */
export async function sendRumbleMessage(videoIdNumeric, text) {
 ...
 */
async function connectChatSse(videoIdNumeric, onMessage, onClose) {
    const session = await getRumbleBotSession();
    if (!hasRumbleBotSession(session)) {
    ...
    function ingestUsers(arr) {
        if (!Array.isArray(arr))
            return;
        for (const u of arr) {
            if (u?.id && u?.username)
                userById.set(String(u.id), String(u.username));
        }
    }
    ...
    function emitOne(m, reqId, isOwn) {
        if (!m?.id || !m?.user_id)
            return;
        if (isOwn)
            return; // ignore le rebond de nos propres messages
...
export function ensureRumbleBridge(opts) {
    ...
    async function persistMessage(msgId, rumbleUserId, username, body, createdAt) {
        try {
            // 1. Archive dédiée Rumble (avec rumble_user_id pour cross-ref)
            await opts.pool.query(`INSERT INTO rumble_chat_messages (streamer_id, rumble_msg_id, rumble_user_id, username, body, created_at)
         VALUES ($1, $2, $3, $4, $5, $6)
         ON CONFLICT (streamer_id, rumble_msg_id) WHERE rumble_msg_id IS NOT NULL DO NOTHING`, [opts.streamerId, msgId, rumbleUserId, username, body, createdAt]);
            ...
            await opts.pool.query(`INSERT INTO chat_messages (streamer_id, user_id, username, body, created_at, external_source, external_msg_id)
         VALUES ($1, 0, $2, $3, $4, 'rumble', $5)
         ON CONFLICT (streamer_id, external_source, external_msg_id) WHERE external_source IS NOT NULL AND external_msg_id IS NOT NULL DO NOTHING`, [opts.streamerId, username, body, createdAt, msgId]);
        }
        catch (e) {
    ...
    function broadcast(m) {
        if (m.msgId && seenIds.has(m.msgId))
            return;
        if (m.msgId)
            addSeen(m.msgId);
        ...
        if (publicOn)
            opts.io.to(`chat:${opts.slug}:public`).emit("chat:message", payload);
        if (popupOn)
            opts.io.to(`chat:${opts.slug}:popup`).emit("chat:message", payload);
        ...
        if (bang) {
            ...
            if (bang.cmd === "clip") {
                ...
                }).catch((e) => console.warn("[rumble_chat] clip error", opts.slug, e?.message || e));
            }
            else {
                ...
                handleCallsCommand({
                    ...
                    cmd: bang.cmd,
                    arg: bang.arg,
                }).catch((e) => console.warn("[rumble_chat] handleCallsCommand error", opts.slug, e?.message || e));
            }
        }
    }
    ...
    async function start() {
        ...
        stopStream = await connectChatSse(videoIdNumeric, (m) => { consecutiveFailures = 0; broadcast(m); }, (noChatAvailable) => {
            stopStream = null;
            if (!alive)
                return;
            consecutiveFailures++;
...
export function stopRumbleBridge(slug) {
