...
// ─────────────────────────────────────────────────────────────────────────────
async function sendInstagramDm(senderId, text) {
...
// ─────────────────────────────────────────────────────────────────────────────
async function processIncomingDm(senderId, text) {
  ...
  `, [textLower]);
    ...
    const already = await pool.query(`SELECT 1 FROM ig_dm_replies WHERE sender_id = $1 AND streamer_slug = $2 LIMIT 1`, [senderId, cfg.streamer_slug]);
    ...
    await pool.query(`INSERT INTO ig_dm_replies (sender_id, streamer_slug)
     VALUES ($1, $2)
     ON CONFLICT (sender_id, streamer_slug) DO NOTHING`, [senderId, cfg.streamer_slug]);
}
// ─────────────────────────────────────────────────────────────────────────────
// Routes webhook
// ─────────────────────────────────────────────────────────────────────────────
...
igWebhookRouter.post("/webhook/instagram", (req, res) => {
    ...
    for (const entry of entries) {
        ...
        for (const event of messagingEvents) {
            ...
            processIncomingDm(senderId, messageText).catch((e) => {
