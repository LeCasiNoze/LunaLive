...
import { ensureBotClips, getClipForStreamer, listClipsForStreamer, removeClipForStreamer, setClipMp4Error, setClipMp4Success, } from "./store.js";
...
async function requireStreamer(req, res, next) {
    ...
    if (!streamerId)
        return res.status(403).json({ ok: false, reason: "not_streamer" });
...
function newJob(streamerId, clipId, secondsTotal) {
...
function startFfmpegToMp4(job, vodUrl, startSec, durSec, title, r2Key) {
    ...
    proc.stderr.on("data", (chunk) => {
        stderrTail += chunk;
        if (stderrTail.length > 20_000)
            stderrTail = stderrTail.slice(-20_000);
        let outMs = null;
        let outTimeText = null;
        let progressFlag = null;
        ...
        if (Number.isFinite(outMs) && outMs > 0) {
            done = Math.floor(outMs / 1_000_000);
        }
        else if (outTimeText) {
...
/* (1) LIST */
botClipsRouter.get("/list", async (req, res) => {
    ...
    const items = rows.map((r) => {
        ...
        return {
            ...
            mp4_key,
            mp4_url,
            mp4_ready_ts: r.mp4_ready_ts ?? null,
            mp4_size: r.mp4_size ?? null,
            mp4_error: r.mp4_error ?? null,
            mp4_rendering: !!r.mp4_rendering,
        };
    });
    return res.json({ ok: true, items });
});
/* (2) DELETE (hide) */
botClipsRouter.post("/delete", express.json(), async (req, res) => {
    const streamerId = Number(req.streamerId || 0);
 ...
 */
botClipsRouter.post("/download/start", express.json(), async (req, res) => {
    try {
        if (!r2Enabled())
            return res.status(409).json({ ok: false, reason: "r2_not_configured" });
        const streamerId = Number(req.streamerId || 0);
        ...
        if (!clip)
            return res.status(404).json({ ok: false, reason: "clip_not_found" });
        if (!clip.vod_url)
            return res.status(409).json({ ok: false, reason: "vod_not_ready" });
...
botClipsRouter.get("/download/progress", async (req, res) => {
    ...
    if (!job)
        return res.status(404).json({ ok: false, reason: "job_not_found" });
    ...
    const onTick = (payload) => send("tick", payload);
 ...
 */
botClipsRouter.get("/download/file", async (req, res) => {
    try {
        const id = String(req.query?.job || "");
        const job = JOBS.get(id);
        if (!job)
        ...
        if (!url)
            return res.status(409).json({ ok: false, reason: "r2_not_ready", job: snapshot(job) });
