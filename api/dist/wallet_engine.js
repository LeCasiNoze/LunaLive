...
export async function earnRubisTx(client, userId, origin, amount, meta = {}) {
    const amt = assertIntAmount(amount);
    await ensureUserExists(client, userId);
    await client.query(`INSERT INTO wallet_lots (user_id, origin, amount_remaining, meta)
     VALUES ($1,$2,$3,$4::jsonb)`, [userId, String(origin || "event_platform"), amt, JSON.stringify(meta ?? {})]);
    await client.query(`UPDATE users SET rubis = rubis + $2 WHERE id=$1`, [userId, amt]);
    await client.query(`INSERT INTO wallet_tx (user_id, kind, origin, amount, meta)
     VALUES ($1,'earn',$2,$3,$4::jsonb)`, [userId, String(origin || "event_platform"), amt, JSON.stringify(meta ?? {})]);
    return { ok: true };
}
export async function earnRubis(userId, origin, amount, meta = {}) {
...
export async function spendRubisTx(client, opts) {
    ...
    if (need > 0)
        throw new Error("insufficient_rubis");
    // apply updates
    for (const t of take) {
    ...
    await client.query(`INSERT INTO wallet_tx (user_id, kind, spend_kind, spend_type, streamer_id, amount, breakdown,
    ...
    if (spendKind === "support" && streamerId) {
        await client.query(`INSERT INTO streamer_earnings_ledger
       ...
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8::jsonb,$9::jsonb)`, [
        ...
        await client.query(`INSERT INTO streamer_wallets (streamer_id, available_rubis, lifetime_rubis)
       VALUES ($1,$2,$2)
       ON CONFLICT (streamer_id) DO UPDATE
         SET available_rubis = streamer_wallets.available_rubis + EXCLUDED.available_rubis,
             ...
             updated_at = NOW()`, [streamerId, streamerEarnRubis]);
    }
    return { spent: amount, breakdown, supportRubis, streamerEarnRubis, platformCutRubis };
}
export async function spendRubis(opts) {
    const client = await pool.connect();
    try {
        await client.query("BEGIN");
...
export async function cashoutFromStreamerWallet(params) {
    ...
    try {
        await client.query("BEGIN");
        const w = await client.query(`SELECT available_rubis FROM streamer_wallets WHERE streamer_id=$1 FOR UPDATE`, [streamerId]);
        const available = Number(w.rows?.[0]?.available_rubis ?? 0);
        if (!w.rows?.[0] || available < rubisNeeded)
            throw new Error("insufficient_balance");
        await client.query(`UPDATE streamer_wallets
       SET available_rubis = available_rubis - $2, updated_at = NOW()
       WHERE streamer_id=$1`, [streamerId, rubisNeeded]);
        const ins = await client.query(`INSERT INTO cashout_requests (streamer_id, amount_rubis, status, note)
       VALUES ($1,$2,'pending',$3)
       RETURNING id, amount_rubis, status, created_at`, [streamerId, rubisNeeded, params.note ?? null]);
        await client.query("COMMIT");
