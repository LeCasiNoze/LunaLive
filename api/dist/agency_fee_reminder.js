...
function normalLine(f) {
    return `• **${eur(f.amount)}** — ${f.description}\n   *${frenchDate(f.due_date)}*`;
}
function buildEmbed(fees, hasLogo) {
    const grouped = {
        overdue: [], today: [], j2: [], j7: [], upcoming: [],
    };
    for (const f of fees)
        grouped[bucketOf(f.days_until)].push(f);
    ...
    const embed = new EmbedBuilder()
        ...
        .setDescription((grouped.overdue.length > 0
    ...
    const orderedKeys = ["overdue", "today", "j2", "j7", "upcoming"];
...
function buildButtons() {
    return new ActionRowBuilder().addComponents(new ButtonBuilder().setStyle(ButtonStyle.Link).setLabel("📊  Ouvrir le FSB Board").setURL(FSB_BOARD_URL));
}
// ── Tick principal ───────────────────────────────────────────────────────────
async function tryClaimRun(todayParis, itemsCount) {
    const r = await pool.query(`INSERT INTO agency_fee_reminder_runs (run_date, items_count)
     VALUES ($1::date, $2)
     ON CONFLICT (run_date) DO NOTHING
     RETURNING run_date`, [todayParis, itemsCount]);
    return r.rowCount === 1;
}
async function tick() {
    const { date: todayParis, hour } = parisNow();
    if (hour < SEND_HOUR_PARIS)
        return;
    // Court-circuit : si déjà envoyé aujourd'hui, sortir sans query inutile.
    const already = await pool.query(`SELECT 1 FROM agency_fee_reminder_runs WHERE run_date = $1::date LIMIT 1`, [todayParis]);
    ...
    try {
        ...
        console.log(`${LOG} ✅ rappel envoyé dans #${channel.name || FEES_CHANNEL_ID} — ${fees.length} échéance(s) — ${todayParis}`);
    }
    catch (e) {
...
// ── Boot ─────────────────────────────────────────────────────────────────────
export function startAgencyFeeReminder() {
    let running = false;
