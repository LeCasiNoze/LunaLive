...
import { ChannelType, PermissionFlagsBits, PermissionsBitField, EmbedBuilder, ButtonBuilder, ButtonStyle, ActionRowBuilder, } from "discord.js";
import * as cfg from "./config.js";
import { kvSet } from "./db.js";
import { ensureOpenBatch } from "./refill.js";
import { ensureWatcherBoard } from "./watcher.js";
const log = (...a) => console.log("[aurix.setup]", ...a);
...
function findTextChannel(category, name) {
    const norm = name.toLowerCase().replace(/\s+/g, "-");
    return category.children.cache.find((c) => c.type === ChannelType.GuildText && c.name === norm);
}
...
async function getOrCreateTextChannel(guild, category, name, topic, overwrites) {
...
export async function runSetup(guild) {
    ...
    if (!me)
        throw new Error("guild.members.me unavailable");
    // ─── Rôles ───
    ...
    // ─── Info salons ───
    const chReglement = await getOrCreateTextChannel(guild, catInfo, cfg.CHANNELS.REGLEMENT, "📌 Règlement de l'agence Aurix. À lire impérativement.", ow.public);
    const chAnnonces = await getOrCreateTextChannel(guild, catInfo, cfg.CHANNELS.ANNONCES, "📣 Annonces officielles Aurix.", ow.public);
    const chBienvenue = await getOrCreateTextChannel(guild, catInfo, cfg.CHANNELS.BIENVENUE, "🎉 Bienvenue dans la famille Aurix.", ow.public);
    const chALire = await getOrCreateTextChannel(guild, catInfo, cfg.CHANNELS.A_LIRE, "📖 Panneau d'information — comment fonctionne l'agence Aurix.", ow.public);
    ...
    // ─── Ouvrir-ticket ───
    const chOpenTicket = await getOrCreateTextChannel(guild, catTickets, cfg.CHANNELS.OUVRIR_TICKET, "✅ Clique sur le bouton ci-dessous pour ouvrir un ticket privé avec la direction Aurix.", ow.openTicketPanel);
    await kvSet("channel_open_ticket_id", chOpenTicket.id);
    // ─── Streamers ───
    await getOrCreateTextChannel(guild, catStreamers, cfg.CHANNELS.ANNONCES_STREAMERS, "📢 Annonces réservées aux streamers Aurix.", ow.streamersReadOnly);
    await getOrCreateTextChannel(guild, catStreamers, cfg.CHANNELS.CHAT_STREAMERS, "💬 Discutez entre streamers Aurix.", ow.streamersOnly);
    await getOrCreateTextChannel(guild, catStreamers, cfg.CHANNELS.PROMOTIONS, "🎁 Promotions, deals et codes exclusifs Aurix.", ow.streamersReadOnly);
    const chBotStreamers = await getOrCreateTextChannel(guild, catStreamers, cfg.CHANNELS.BOT_STREAMERS, "🤖 Invite le bot Aurix sur ton Discord pour offrir /celsius à tes viewers.", ow.streamersReadOnly);
    await kvSet("channel_bot_streamers_id", chBotStreamers.id);
    // ─── Staff ───
    const chStaffChat = await getOrCreateTextChannel(guild, catStaff, cfg.CHANNELS.STAFF_CHAT, "💬 Discussions internes staff Aurix.", ow.staffOnly);
    const chRefills = await getOrCreateTextChannel(guild, catStaff, cfg.CHANNELS.REFILLS, "📋 Liste des demandes de refill en cours. Mise à jour en temps réel.", ow.staffOnly);
    const chLogs = await getOrCreateTextChannel(guild, catStaff, cfg.CHANNELS.LOGS, "🔔 Audit log Aurix.", ow.staffOnly);
    const chGestion = await getOrCreateTextChannel(guild, catStaff, cfg.CHANNELS.GESTION, "🗂️ Gestion interne.", ow.staffOnly);
    const chWatcher = await getOrCreateTextChannel(guild, catStaff, cfg.CHANNELS.WATCHER, "🕵️ Supervision des inscriptions /celsius (validation / refus).", ow.staffOnly);
...
async function postTicketPanel(channel) {
    ...
    const btnApply = new ButtonBuilder()
        ...
        .setEmoji("📝");
    const row = new ActionRowBuilder().addComponents(btnDeal, btnApply);
    await channel.send({ embeds: [embed], components: [row] });
}
async function postBotInvitePanel(channel) {
    await purgeBotMessages(channel);
    const appId = channel.client.application?.id ?? channel.client.user?.id ?? "";
    const inviteUrl = `https://discord.com/api/oauth2/authorize?client_id=${appId}` +
    ...
    const btnInvite = new ButtonBuilder()
        ...
        .setURL(inviteUrl)
        .setEmoji("🤖");
    const row = new ActionRowBuilder().addComponents(btnInvite);
    await channel.send({ embeds: [embed], components: [row] });
}
async function postWelcomePanel(channel, refs) {
    await purgeBotMessages(channel);
...
async function postRulesPanel(channel, refs) {
    await purgeBotMessages(channel);
    const annoncesMention = `<#${refs.annoncesId}>`;
    const embed = new EmbedBuilder()
        .setTitle(`📌  Règlement ${cfg.BRAND.NAME}`)
        .setDescription("Règles à respecter pour tous les membres de l'agence. Le non-respect peut entraîner des sanctions allant du simple rappel à la **rupture immédiate du contrat**.")
