...
export async function startAurixBot() {
    ...
    client.once(Events.ClientReady, async (c) => {
        ...
        try {
            ...
            if (guildId) {
                ...
                console.log(`[aurix] Guild commands enregistrées sur ${guildId} : ${guildCommandDefinitions.length}`);
            }
            await rest.put(Routes.applicationCommands(env.DISCORD_APP_ID), {
                body: globalCommandDefinitions,
            });
            console.log(`[aurix] Global commands enregistrées : ${globalCommandDefinitions.length} (celsius / aurix)`);
        }
        catch (e) {
        ...
        if (guildId) {
            ...
            if (guild) {
                try {
                    ...
                    const flag = await one("SELECT value FROM aurix_kv WHERE key=$1", [
                        "initial_setup_done",
                    ]);
    ...
    client.on(Events.GuildMemberAdd, async (member) => {
        ...
        if (env.GUILD_ID && member.guild.id !== env.GUILD_ID)
            return;
    ...
    client.on(Events.GuildMemberUpdate, async (before, after) => {
        if (env.GUILD_ID && after.guild.id !== env.GUILD_ID)
            return;
        await onMemberUpdate(before, after);
    });
