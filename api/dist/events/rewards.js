 ...
 */
async function distributeViewerStreamers(client, eventId, cfg) {
    ...
    for (const tier of cfg.tiers) {
        ...
        if (tier.rubisToChest > 0) {
            await client.query(`INSERT INTO streamer_chests (streamer_id) VALUES ($1) ON CONFLICT (streamer_id) DO NOTHING`, [st.streamerId]);
            await client.query(`INSERT INTO streamer_chest_lots (streamer_id, origin, weight_bp, amount_remaining, meta)
         VALUES ($1, 'event_viewer_week', $2, $3, $4::jsonb)`, [st.streamerId, cfg.chestLotWeightBp, tier.rubisToChest, JSON.stringify({ eventId, eventType: "viewer_week", rank: tier.rank })]);
        }
        if (tier.rank === 1 && cfg.featuredDays > 0) {
            await client.query(`UPDATE streamers SET featured=true, featured_until=NOW() + ($2::int * INTERVAL '1 day'), updated_at=NOW() WHERE id=$1`, [st.streamerId, cfg.featuredDays]);
        }
        if (tier.subs > 0 && st.userId) {
            await client.query(`INSERT INTO user_coupons (user_id, code, qty, updated_at)
         VALUES ($1, 'sub_ticket', $2, NOW())
         ON CONFLICT (user_id, code) DO UPDATE SET qty = user_coupons.qty + $2, updated_at = NOW()`, [st.userId, tier.subs]);
        }
        if (st.userId) {
            await client.query(`INSERT INTO event_reward_grants (event_id, user_id, tier, rank, rubis, extras)
...
async function distributeBoss(client, eventId, event, cfg) {
    ...
    const rubisForRank = (rank) => {
        for (const b of cfg.rubisBrackets)
            if (rank <= b.maxRank)
                return b.rubis;
        return 0;
    };
    ...
    for (const row of ranked) {
        ...
        if (cfg.allWheelTickets > 0) {
         ...
         ON CONFLICT (user_id) DO UPDATE SET amount = LEAST(user_event_tickets.amount + $2, 50)`, [row.userId, cfg.allWheelTickets]);
        }
        ...
        if (cfg.allPremiumDays > 0)
            extras.premiumDays = cfg.allPremiumDays;
        if (isTop3) {
            await grantEntitlement(client, row.userId, "skin", cfg.topSkinCode);
...
async function distributeClipRace(client, eventId, event, cfg) {
    ...
    for (const stTier of cfg.streamerTiers.tiers) {
        ...
        await client.query(`INSERT INTO streamer_chests (streamer_id) VALUES ($1) ON CONFLICT (streamer_id) DO NOTHING`, [st.streamerId]);
        await client.query(`INSERT INTO streamer_chest_lots (streamer_id, origin, weight_bp, amount_remaining, meta)
       VALUES ($1, 'event_clip_race', $2, $3, $4::jsonb)`, [st.streamerId, cfg.streamerTiers.chestLotWeightBp, stTier.rubisToChest, JSON.stringify({ eventId, eventType: "clip_race", rank: stTier.rank })]);
        if (stTier.rank === 1 && cfg.streamerTiers.featuredDays > 0) {
            await client.query(`UPDATE streamers SET featured=true, featured_until=NOW() + ($2::int * INTERVAL '1 day'), updated_at=NOW() WHERE id=$1`, [st.streamerId, cfg.streamerTiers.featuredDays]);
        }
        if (st.userId) {
            if (stTier.subs > 0) {
                await client.query(`INSERT INTO user_coupons (user_id, code, qty, updated_at) VALUES ($1,'sub_ticket',$2,NOW())
           ON CONFLICT (user_id, code) DO UPDATE SET qty = user_coupons.qty + $2, updated_at = NOW()`, [st.userId, stTier.subs]);
            }
            const extras = { kind: "streamer_win", streamerId: st.streamerId, streamerSlug: st.slug, rank: stTier.rank, subs: stTier.subs, chestDeposit: stTier.rubisToChest };
            if (stTier.rank === 1) {
