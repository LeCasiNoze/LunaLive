 ...
 */
export async function getRankedClips(db, eventId, limit = 10) {
 ...
 */
export async function getRankedStreamers(db, eventId, limit = 10) {
    const r = await db.query(`
    SELECT
 ...
 */
export async function resolveClipCreatorUserId(db, author) {
    const name = String(author || "").trim();
    if (!name || name.toLowerCase() === "lunaclip")
        return null;
 ...
 */
export async function recomputeClipRace(eventId) {
