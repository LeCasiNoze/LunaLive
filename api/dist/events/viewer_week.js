...
export const VIEWER_WEEK_PALIERS = [
...
export async function getViewerPaliersState(db, eventId, userId) {
    ...
    const claimed = (claimsRes.rows || []).map((r) => Number(r.palier_index));
    return { points, claimed };
}
// Récupère tous les paliers atteints non encore réclamés (points >= seuil).
export async function claimViewerPaliers(eventId, userId) {
    ...
    try {
        ...
        for (const p of VIEWER_WEEK_PALIERS) {
            if (points < p.threshold || claimed.has(p.index))
                continue;
...
`;
export async function recomputeViewerWeek(eventId) {
    // charge fenêtre + type
