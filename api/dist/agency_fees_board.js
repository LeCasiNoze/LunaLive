...
const BUCKET_ORDER = ["overdue", "today", "j2", "j7", "upcoming"];
...
function buildBoardEmbed(unpaid, paid, hasLogo) {
    ...
    const embed = new EmbedBuilder()
        ...
        .setDescription(`**Vue d'ensemble** — actualisé toutes les 24h, ou via le bouton ci-dessous.\n` +
    ...
    if (paid.rows.length === 0) {
        ...
        const lines = paid.rows.map(p => `• **${eur(p.amount)}** — ${p.description}\n   *Payé le ${frenchDate(p.paid_date)}*`);
        embed.addFields({
...
export async function refreshAgencyFeesBoard() {
    if (_refreshInFlight) {
    ...
    try {
        ...
        if (existingMsgId) {
            ...
            try {
                ...
                await pool.query(`UPDATE agency_fees_board SET last_refreshed_at = NOW() WHERE id = 1`);
...
export async function handleFeesBoardRefreshButton(interaction) {
    try {
...
// ── Boot ─────────────────────────────────────────────────────────────────────
export function startAgencyFeesBoard() {
