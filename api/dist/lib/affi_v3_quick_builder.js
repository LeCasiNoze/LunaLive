...
export function defaultV3QuickInputs() {
    return {
        modelKind: "M1",
        pseudo: "",
        affiLink: "",
...
// ─── Style helper ───────────────────────────────────────────────────────────
function lineStyleToV2(s, fallbackColor = "#ffffff") {
...
export function buildV3PageFromQuickInputs(inputs) {
    ...
    const last = aboveCards[aboveCards.length - 1];
    if (last)
        last.marginBottom = "8px";
    ...
    const cardsContainer = page.zones.cards[0];
    if (cardsContainer && cardsContainer.type === "container") {
        const [c1, c2] = cardsContainer.children;
