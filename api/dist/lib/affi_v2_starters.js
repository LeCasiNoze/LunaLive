...
// ─────────────────────────────────────────────────────────────────────────────
// Copié depuis web/src/lib/editor_v2_starters.ts pour usage server-side.
// Si les types évoluent côté web, refresh ce fichier.
import { newV2Page, makeV2BlockId, } from "./affi_v2_types.js";
...
export const M4_DEFAULT_IMAGES = [
...
// Card M4 V2 — duplication visuelle pixel-exact de la card V1.
// Le rendu est délégué au composant <M4V1Card /> (cf. components/M4V1Card.tsx)
// qui copie 1:1 le HTML/CSS de model4.html. Aucun bloc primitif ici.
function buildM4Card(label, bonusPct, defaultImg, depositAmount, bonusAmount, animationDelay = "0s") {
    return {
...
function buildM4ReviewCard(name, gain, text, when) {
    return ct("stack", [
...
export function buildM4V2Starter() {
    ...
    page.zones.cards.push(ct("stack", [
    ...
    // REVIEWS ZONE
    page.zones.reviews.push(txt("AVIS DES JOUEURS", {
        tag: "h2",
        align: "center",
        style: {
            fontFamily: "Inter",
        ...
        buildM4ReviewCard("Pierre L.", "400€", "Offre VIP validée. Le bonus s'est activé en 1 minute après mon dépôt. Cashout de 400€ reçu par virement SEPA. Rapide et discret.", "il y a 2 jours"),
        buildM4ReviewCard("Sophie M.", "180€", "Interface haute qualité sur téléphone, on se croirait dans un casino VIP. Pas de bugs sur les mini-jeux. Le bonus est un vrai plus.", "il y a 5 jours"),
        buildM4ReviewCard("Karim B.", "100€", "Inscription et vérification ultra-rapide. Dépôt sécurisé, j'ai posé 50€, j'ai eu 100€ de capital. Service client très réactif.", "il y a 1 semaine"),
    ], {
        gap: "16px",
        maxWidth: "440px",
        marginBottom: "40px",
    ...
    // FAQ ZONE
    page.zones.faq.push(txt("QUESTIONS FRÉQUENTES", {
        tag: "h2",
        align: "center",
        style: {
            fontFamily: "Inter",
        ...
        buildM4FaqItem("Quelles sont les garanties de sécurité des dépôts ?", "La plateforme utilise un cryptage SSL 256 bits certifiant l'anonymat et la sécurité totale de vos transactions. Vos fonds sont stockés sur des serveurs régulés."),
        buildM4FaqItem("Comment retirer mes gains ?", "Les demandes de retrait SEPA sont traitées sous 24h ouvrées. Le délai bancaire légal s'ajoute pour que les fonds apparaissent sur votre compte."),
        buildM4FaqItem("Le bonus est-il garanti ?", "Oui. Dès que ton premier dépôt est validé, le bonus est crédité automatiquement. Aucune condition cachée."),
    ], {
        maxWidth: "440px",
    }));
    // FOOTER
    page.zones.footer.push(txt("Les jeux d'argent comportent des risques : endettement, isolement, dépendance. Pour être aidé, appelez le 09-74-75-13-13. Cette offre est strictement interdite aux mineurs.", {
        tag: "p",
        align: "center",
...
export function buildM5V2Starter() {
    const page = newV2Page("M5V2");
    page.zones.aboveCards.push(img("", {
    ...
    page.zones.belowCards.push(btn("RÉCLAME 20€ OFFERTS", {
...
// ─── Dispatch ────────────────────────────────────────────────────────────────
export function getStarterTemplateV2(model) {
    if (model === "M4V2")
        return buildM4V2Starter();
