...
async function fetchHlsPlaylist(m3u8Url, timeoutMs = 15_000) {
...
async function prepareRumbleClipPlaylist(p) {
    ...
    console.log(`[clips-mp4] rumble seek strategy=${strategyUsed} ` +
    ...
    if (startIdx === -1)
        return null;
    if (endIdx === -1)
        endIdx = playlist.segments.length - 1; // pas assez de post → prend tout ce qu'on a
...
async function renderAndUploadClip(clip) {
    ...
    const isRumbleLiveHls = String(clip.platform || "").toLowerCase() === "rumble" &&
        /\.m3u8(\?|$)/i.test(vodUrl);
    if (isRumbleLiveHls) {
        ...
        console.log(`[clips-mp4] rumble live HLS clip=${clip.id} ` +
    ...
    try {
        ...
        const HLS_HEADERS = "Origin: https://rumble.com\r\n" +
        ...
        if (localM3u8ToCleanup) {
            try {
                if (fs.existsSync(localM3u8ToCleanup))
                    fs.unlinkSync(localM3u8ToCleanup);
            }
            catch { }
        }
    }
