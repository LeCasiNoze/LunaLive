...
const byKind = (kind) => COSMETICS_CATALOG.filter((x) => x.active && x.kind === kind);
function tryGetAuthUser(req) {
    ...
    try {
        ...
        if (!secret)
            return null;
        const au = jwt.verify(m[1], secret);
...
async function getActiveSubBadgesForUser(userId) {
    const q = await db.query(`SELECT s.slug, s.display_name AS "displayName", s.appearance
...
cosmeticsRouter.get("/cosmetics/catalog", async (req, res) => {
    const staticItems = COSMETICS_CATALOG.filter((x) => x.active);
    const au = tryGetAuthUser(req);
    let dyn = [];
    if (au?.id)
        dyn = await getActiveSubBadgesForUser(au.id);
    // merge unique
    const map = new Map();
...
cosmeticsRouter.get("/me/cosmetics", requireAuth, async (req, res) => {
    ...
    // ensure row exists
    await db.query(`INSERT INTO user_equipped_cosmetics (user_id) VALUES ($1) ON CONFLICT (user_id) DO NOTHING`, [user.id]);
    const row = await db.query(`SELECT username_code, badge_code, title_code, frame_code, hat_code
    ...
    const owned = {
        username: unlockAll ? byKind("username").map((x) => x.code) : [],
        badge: [],
        title: unlockAll ? byKind("title").map((x) => x.code) : [],
        frame: unlockAll ? byKind("frame").map((x) => x.code) : [],
        hat: unlockAll ? byKind("hat").map((x) => x.code) : [],
    };
    if (unlockAll) {
...
cosmeticsRouter.patch("/me/cosmetics/equip", requireAuth, async (req, res) => {
    const user = req.user;
    const unlockAll = cosmeticsUnlockAllFor(user.username);
    ...
    if (code) {
        ...
        if (kind === "badge") {
            ...
            if (slug && !unlockAll) {
                ...
                if (item.kind !== kind)
                    return res.status(400).json({ ok: false, error: "kind_mismatch" });
                if (!unlockAll)
                    return res.status(403).json({ ok: false, error: "not_owned" });
            }
        }
        ...
        : kind === "badge"
            ...
            : kind === "title"
                ...
                : kind === "frame"
                    ? "frame_code"
                    : "hat_code";
    await db.query(`UPDATE user_equipped_cosmetics
     SET ${col} = $2, updated_at = NOW()
     WHERE user_id = $1`, [user.id, code]);
    const row = await db.query(`SELECT username_code, badge_code, title_code, frame_code, hat_code
     FROM user_equipped_cosmetics
     WHERE user_id=$1`, [user.id]);
    const equipped = row.rows?.[0] ?? {};
