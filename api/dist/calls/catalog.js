...
export async function resolveSlot(pool, input) {
    ...
    if (!qKey)
        return null;
    const exact = await pool.query(`SELECT name, provider_norm AS provider, image_url AS "imageUrl" FROM slots_catalog WHERE name_key=$1 LIMIT 1`, [qKey]);
    if (exact.rows?.[0]) {
        return {
    ...
    if (!cand.length) {
        ...
        if (toks[0])
            cand = await searchSlots(pool, toks[0], 10);
    }
    if (!cand.length)
        return null;
    if (qKey.length < 3)
