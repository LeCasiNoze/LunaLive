...
import { addCall, countUserCalls, effectiveLimit, getCallsSettings, isProviderBanned, isSlotBanned, isUserBannedFromCalls, listCalls, resetCalls, } from "./queue.js";
...
export async function handleCallsCommand(opts) {
    ...
    if (cmd !== "call" && cmd !== "listec" && cmd !== "resetc")
    ...
    try {
        ...
        if (settings.showAcceptPublic) {
            const sys = chatStore.addSystem(slug, `🎰 Call ajouté : "${add.item.slotName}"${add.item.provider ? ` (${add.item.provider})` : ""} — @${actorUsername}`);
            io.to(`chat:${slug}`).emit("chat:message", sys);
        }
