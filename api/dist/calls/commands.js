...
const CALLS_TALENT_CODE = "talent_calls_limit";
// lvl 1 => +1, lvl 2/3 => +2
function callsLimitBonusFromLevel(level) {
...
async function getUserTalentLevel(pool, userId, code) {
    try {
...
async function sendBotChat(pool, io, opts, body) {
    ...
    const ins = await pool.query(`INSERT INTO chat_messages (streamer_id, user_id, username, body)
     VALUES ($1,$2,$3,$4)
     RETURNING id, created_at AS "createdAt"`, [opts.streamerId, botUserId, botUsername, text]);
...
async function getPcallNextAt(pool, streamerId, userId) {
...
async function setPcallCooldown(pool, streamerId, userId, nextAtMs) {
    await ensurePcallSchema(pool);
    await pool.query(`INSERT INTO calls_pcall_cooldowns(streamer_id, user_id, next_at_ms)
     VALUES ($1,$2,$3)
     ON CONFLICT (streamer_id, user_id) DO UPDATE SET next_at_ms = EXCLUDED.next_at_ms`, [streamerId, userId, nextAtMs]);
}
// minimal (évite “relation hunt_sessions does not exist”)
async function ensureHuntSchema(pool) {
    await pool.query(`
...
async function getHuntMeta(pool, ownerUserId) {
...
async function setHuntPhase(pool, ownerUserId, phase) {
    await ensureHuntSchema(pool);
    await pool.query(`
...
async function hasActivePremiumViewer(pool, userId) {
