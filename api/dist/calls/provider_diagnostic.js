...
export async function diagnoseProvider(pool, providerRequested) {
    ...
    try {
        ...
        const exactMatch = allProviders.find((p) => p.toLowerCase() === providerRequested.toLowerCase());
        const partialMatches = allProviders.filter((p) => p.toLowerCase().includes(providerRequested.toLowerCase()));
        if (exactMatch) {
            ...
            console.log(`⚠️  No exact match, but found similar: ${partialMatches.map((p) => `"${p}"`).join(', ')}`);
...
export async function diagnoseMultipleProviders(pool, providers) {
    const results = [];
