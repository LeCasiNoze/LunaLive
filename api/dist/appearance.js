...
function pickHex(input, fallback) {
...
export function mergeAppearance(base, patch) {
    const b = normalizeAppearance(base);
    const p = patch ?? {};
    const out = JSON.parse(JSON.stringify(b));
    const chat = p?.chat ?? p;
    if (chat && typeof chat === "object") {
        if (chat.viewerSkinsLevel !== undefined)
            out.chat.viewerSkinsLevel = chat.viewerSkinsLevel;
        if (chat.usernameColor !== undefined)
            out.chat.usernameColor = chat.usernameColor;
        if (chat.messageColor !== undefined)
            out.chat.messageColor = chat.messageColor;
        const sub = chat.sub;
        if (sub && typeof sub === "object") {
            if (sub.usernameColor !== undefined)
                out.chat.sub.usernameColor = sub.usernameColor;
            if (sub.messageColor !== undefined)
                out.chat.sub.messageColor = sub.messageColor;
            const badge = sub.badge;
            if (badge && typeof badge === "object") {
                if (badge.enabled !== undefined)
                    out.chat.sub.badge.enabled = badge.enabled;
                if (badge.text !== undefined)
                    out.chat.sub.badge.text = badge.text;
                if (badge.borderColor !== undefined)
                    out.chat.sub.badge.borderColor = badge.borderColor;
                if (badge.textColor !== undefined)
                    out.chat.sub.badge.textColor = badge.textColor;
            }
            const hat = sub.hat;
            if (hat && typeof hat === "object") {
                if (hat.id !== undefined)
                    out.chat.sub.hat.id = hat.id;
            }
        }
    }
