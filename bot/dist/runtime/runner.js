...
export class StreamerRunner {
    ...
    start() {
        ...
        const sendBotText = async (t, reason = "send") => {
        ...
        const sendDliveText = async (message, trigger) => {
        ...
        this.transport.start(async (msg) => {
            ...
            if (Number(msg.userId) === botUserId)
                return; // évite boucles
            ...
            try {
                const handledLuna = await tryHandleLunaCommand({
                    ...
                    sendDlive: async (t, meta) => {
                        await sendDliveText(t, meta?.trigger);
                    },
                    publicBase: this.env.LUNALIVE_PUBLIC_BASE, // optionnel
                });
                if (handledLuna)
                    return;
            }
        ...
        const isLiveNow = async () => {
            try {
                const r = await this.pool.query(`SELECT is_live
          FROM streamers
          WHERE id=$1
          LIMIT 1`, [this.streamer.id]);
                if (!r.rowCount)
                    return false;
    ...
    stop() {
        ...
        if (this.cfgReloadTimer)
            clearInterval(this.cfgReloadTimer);
