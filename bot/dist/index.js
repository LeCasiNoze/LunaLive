...
function getCgroupCpuLimitCores() {
...
function getCgroupCpuUsageUsec() {
 ...
 */
function startLunaClipDbIpc(pool) {
    ...
    const flush = async () => {
      ...
      `, [JSON.stringify(payload)]);
        ...
        if (logs.length) {
            ...
            await pool.query(`INSERT INTO lunaclip_admin_logs (slug, source, msg) VALUES ${chunks.join(",")}`, values);
            // ring DB
            await pool.query(`
            ...
            try {
                if (action === "force_switch") {
                    if (!payload.streamer_id)
                        throw new Error("missing streamer_id");
                ...
                await pool.query(`UPDATE lunaclip_admin_commands
           SET status='done', handled_at=NOW(), result=$2::jsonb
           WHERE id=$1`, [id, JSON.stringify({ ok: true })]);
            }
            catch (e) {
                await pool.query(`UPDATE lunaclip_admin_commands
           SET status='error', handled_at=NOW(), result=$2::jsonb
           WHERE id=$1`, [id, JSON.stringify({ ok: false, error: String(e?.message ?? e) })]);
            }
        }
    };
