...
export async function addLunaClip(pool, streamerId, title, atSec, 
...
liveStartTs) {
    ...
    try {
       ...
       WHERE streamer_id=$1
         ...
         AND ABS(at_sec - $3) <= $4
         AND created_ts >= $5
       LIMIT 1`, [streamerId, LUNACLIP_AUTHOR, at, DEDUP_WINDOW_SEC, nowMs - DEDUP_HORIZON_MS]);
        if (dup.rows?.[0]?.id) {
       ...
       RETURNING id`, [streamerId, title.slice(0, 140), LUNACLIP_AUTHOR, at,
