// bot/src/modules/wheel/wheel.ts
import type { Pool } from "pg";
import type { StreamerRow } from "../../core/types.js";
...
export async function tryHandleWheelCommand(args: {
  ...
  prefix: string; // souvent "!"
  msg: any; // message transport (userId, username, body...)
  ...
  const isJoin =
    new RegExp(`^${escapeRegex(p)}join\\b`, "i").test(body) ||
    /^!join\b/i.test(body);

  const isWheel =
    new RegExp(`^${escapeRegex(p)}wheel\\b`, "i").test(body) ||
    /^!wheel\b/i.test(body);

  if (!isJoin && !isWheel) return false;

  // cfg
  await ensureCfg(args.pool, args.streamer.id);

  const cfgR = await args.pool.query(
    `SELECT enroll_open
     FROM bot_wheel_cfg
     WHERE streamer_id=$1
     LIMIT 1`,
  ...
  if (isWheel && !isJoin) {
    const countR = await args.pool.query(
  ...
  try {
    const ins = await args.pool.query(
      `INSERT INTO bot_wheel_entries (streamer_id, user_id, username, username_lc)
       ...
       RETURNING id`,
      [args.streamer.id, userId, username, usernameLc]
    );
    joined = !!ins.rows?.[0];
  } catch {
    ...
    const ins = await args.pool.query(
      `INSERT INTO bot_wheel_entries (streamer_id, username, username_lc)
       ...
       RETURNING id`,
      [args.streamer.id, username, usernameLc]
    );
    joined = !!ins.rows?.[0];
  }
...
async function ensureCfg(pool: Pool, streamerId: number) {
  await pool.query(
